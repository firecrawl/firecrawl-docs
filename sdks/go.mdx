...
Large PDFs that outlive the request timeout return `408` with `details.state: "processing_continues"` and a `Retry-After` header. Honor that delay and retry the same URL — the retry attaches to the in-flight job. See [Large PDFs](/features/parse#large-pdfs).
