...
const session = await firecrawl.browser({ ttl: 300, streamWebView: true });
console.log(session.id);          // session ID
console.log(session.cdpUrl);      // WebSocket URL for CDP
console.log(session.liveViewUrl); // real-time view URL
```
