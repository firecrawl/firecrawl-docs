...
<Note>
**Large PDFs:** if a document outlives the request timeout, the API returns `408` with `details.state: "processing_continues"`. `scrape()` (SDK ≥ 4.37) sleeps the advertised delay and retries automatically so the call still returns the finished document. Pass `autoResume: false` to surface the timeout instead. See [Large PDFs](/features/parse#large-pdfs).
</Note>
