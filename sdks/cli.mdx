...
Use `npx firecrawl-cli@alexandria` for Alexandria discovery and provider execution. The beta supports progressive catalogue browsing: category, provider, then a selected tool's full contract. It uses your existing Firecrawl login or `FIRECRAWL_API_KEY`.
...
These commands only discover tools. See [Alexandria](/features/alexandria#browse-the-catalogue-progressively) for setup, semantic search, full-contract output, pricing, and execution with `scrape --alexandria`.

The beta also supports `alexandria terms show <provider>` and `alexandria terms accept <provider> --terms-version <version> --digest <digest> --confirm`. Read the agreement and obtain explicit authorization before accepting it for your organization. See [Provider terms](/features/alexandria#provider-terms) for the complete flow.
