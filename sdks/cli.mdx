...
Run an AI agent to extract structured data from the web using natural language prompts.
...
| `--model <model>` | Model to use: `spark-1-mini` (default, cheaper) or `spark-1-pro` (higher accuracy) |
...
firecrawl agent "Find all pricing plans and their features" --urls https://example.com/pricing --wait

# Use a schema for consistent output
firecrawl agent "Get company info" --urls https://example.com --schema '{"name": "string", "employees": "number"}' --wait --pretty
```
