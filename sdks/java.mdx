...
Use a scrape job ID to execute follow-up browser code in the same replayed context:
...
BrowserExecuteResponse scrapeRun = client.scrapeExecute(
    scrapeJobId,
...
BrowserDeleteResponse deleted = client.deleteScrapeBrowser(scrapeJobId);
