...
| `httpClient` | `ClientInterface` | Built from timeout | PSR-18 compatible HTTP client |
...
You can pass a pre-configured PSR-18 compatible HTTP client (such as Guzzle) to control connection pooling, middleware, proxy settings, and other HTTP features. When provided, the `timeoutSeconds` setting is ignored in favor of the client's own configuration.
