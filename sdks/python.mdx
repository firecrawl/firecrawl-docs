...
import AgentPythonBase from '/snippets/v2/agent/base/python.mdx'
import AgentPythonZdr from '/snippets/v2/agent/zdr/python.mdx'
...
Set `zero_data_retention=True` to disable stored prompts and activity logs for the run.
...
Firecrawl endpoints for crawl and batch return a `next` URL when more data is available. The Python SDK auto-paginates by default and aggregates all documents; in that case `next` will be `None`. You can disable auto-pagination or set limits. The `next` URL is opaque; pass it back to the SDK to fetch the next page.
...
- Start a job, then fetch one page at a time with `auto_paginate=False` and `get_crawl_status_page`.
...
- Start a job, then fetch one page at a time with `auto_paginate=False` and `get_batch_scrape_status_page`.
