...
Convert a local document into markdown, HTML, or structured JSON with `parse`. Supported file types are `.html`, `.htm`, `.pdf`, `.docx`, `.doc`, `.odt`, `.rtf`, `.xlsx`, and `.xls`. The file uploads as multipart form data, and the call returns a `Document` populated with the formats you requested.

`ParseOptions` omits scrape-only fields that `/v2/parse` rejects, such as `actions`, `waitFor`, `location`, `mobile`, `screenshot`, `branding`, and `changeTracking`.
