...
Each check records page-level results as `same`, `new`, `changed`, `removed`, or `error`. You can receive a webhook for every completed check, email summaries when changes or errors happen, or both.
...
When a monitor has a `webhook`, Firecrawl sends `monitor.check.completed` after each completed check. The webhook contains the check summary and up to 100 page-level results.
...
The check detail response includes summary counts and a paginated `pages` array. Changed pages include inline diff data when available.
...
- [Monitor webhook payload](/api-reference/endpoint/webhook-monitor-check-completed)
