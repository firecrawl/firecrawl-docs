...
Add a plain-language `goal` when you only want to be alerted for meaningful changes. If `goal` is present and `judgeEnabled` is omitted, Firecrawl enables judging automatically. Judging runs on changed pages and returns a `judgment` with `meaningful`, `confidence`, `reason`, and `fields`.
...
<Note>
  Meaningful-change judging adds one judge credit per judged page. `estimatedCreditsPerMonth` includes this cost when judging is enabled.
</Note>
...
The minimum interval is 15 minutes. API responses always return the normalized cron expression. Text schedules are spread by monitor ID before they are converted to cron so many monitors do not all run at the same instant.
