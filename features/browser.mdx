...
Firecrawl Browser Sandbox gives your agents a secure, fully managed browser environment — no local setup, no Chromium installs, no driver compatibility issues. Each session runs in an isolated, disposable sandbox, so you can scale from one browser to hundreds without managing infrastructure.

Browser works alongside Firecrawl's [scrape](/features/scrape) and [search](/features/search) endpoints. Use scrape for fast, single-page extraction. Use Browser when your workflow requires navigation, form fills, multi-step flows, or interactive content that scraping alone can't reach.

Available via API, CLI, and SDK.
