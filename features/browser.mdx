...
Files downloaded inside a session are saved to the sandbox filesystem. Use Playwright's download API via the execute endpoint to capture them:
...
const [download] = await Promise.all([
  ...
  page.click('a#download-link') // Click element that triggers download
]);
