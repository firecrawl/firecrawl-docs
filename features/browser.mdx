---
title: "Browser"
description: "Launch cloud browser sessions and remotely execute Python, JavaScript, or bash. Use agent-browser for simple CLI-driven automation."
...
Available via [API](/api-reference/endpoint/browser-create), [CLI](/sdks/cli#browser) (Bash / agent-browser), [Node SDK](/sdks/node#browser), [Python SDK](/sdks/python#browser), and [Vercel AI SDK](/developer-guides/llm-sdks-and-frameworks/vercel-ai-sdk).
...
[agent-browser](https://github.com/vercel-labs/agent-browser) is a headless browser CLI with 40+ commands. It's **pre-installed in every sandbox**. Instead of writing Playwright code, agents send simple bash commands that run remotely inside the sandbox. The CLI auto-injects `--cdp` so agent-browser connects to your active session automatically.

To use browsers within an agent harness like Claude Code, Codex, Open Code, or Cursor, add the Firecrawl skill:
...
```python Python SDK
result = app.browser_execute(session_id, {
    "code": "agent-browser snapshot",
    "language": "bash",
})
```
...
The core agent-browser pattern: **snapshot** the page to get an accessibility tree with `@ref` IDs, then use those refs to **click**, **type**, or **fill** elements.

```bash
# 1. Take a snapshot - returns an accessibility tree with @ref IDs
firecrawl browser execute --bash "agent-browser snapshot"
...
# 2. Interact using the @ref IDs from the snapshot
firecrawl browser execute --bash "agent-browser fill @e3 'search query'"
firecrawl browser execute --bash "agent-browser click @e8"

# 3. Snapshot again to see updated state
firecrawl browser execute --bash "agent-browser snapshot"
```
...
<Tip>
Run `agent-browser --help` remotely to see all 40+ available commands:
