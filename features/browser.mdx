...
Firecrawl Browser Sandbox gives your agents a secure, fully managed browser environment. No local setup, no Chromium installs, no driver compatibility issues. Agent browser and playwright are pre-installed. Each session runs in an isolated, disposable sandbox that scales without managing infrastructure.
...
By default, each browser session starts with a clean slate. With `persistentSession`, you can save cookies, localStorage, and other browser state so it carries over to future sessions. This is useful for staying logged in, preserving preferences, or resuming multi-step workflows.
...
| `writeMode` | `"readwrite"` | `"readwrite"` saves changes back. `"readonly"` loads existing data without writing — useful when you need multiple concurrent readers. |

<Note>
Only one session can write to a profile at a time. If another session is already writing, you'll get a `409` error. You can still open the same profile in `"readonly"` mode, or try again later.
</Note>
