...
Every session is recorded by default and can be replayed after it ends — in the dashboard under [Activity Logs](https://firecrawl.dev/app/logs), or via the API as standard HLS video. Recordings capture what the browser actually rendered and are retained for 30 days.
...
# Fetch the HLS playlist for a tab
curl "https://api.firecrawl.dev/v2/interact/$SESSION_ID/replay/0" \
  -H "Authorization: Bearer $FIRECRAWL_API_KEY"
```

To disable recording for a session, pass `recordSession: false` when creating it. See the [Interact session replay docs](/features/interact#session-replay) for playback details.
...
- [Get Session Replay](/api-reference/endpoint/browser-replay)
- [Get Session Replay Playlist](/api-reference/endpoint/browser-replay-page)
