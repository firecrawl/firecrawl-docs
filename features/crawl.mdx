...
<Note>
  **Regex flavor for `includePaths`/`excludePaths`:** patterns are compiled with the Rust [`regex`](https://docs.rs/regex) engine (RE2-style). Anchors (`^`, `$`), character classes, alternation (`a|b`), quantifiers, and non-capturing/named groups (`(?:...)`, `(?<name>...)`) are supported, but **look-around (`(?=...)`, `(?!...)`, `(?<=...)`, `(?<!...)`) and backreferences are not**. A pattern using an unsupported construct is rejected with a `400`. Patterns match against the URL **pathname** only, unless `regexOnFullURL` is set. To express "everything except a few paths", prefer an `includePaths` allowlist over a negative-lookahead `excludePaths` pattern.
</Note>
