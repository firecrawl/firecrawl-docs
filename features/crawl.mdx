...
<Note>
  The `limit` parameter controls the maximum number of pages crawled, not the total credits used. If crawled pages contain PDFs, each PDF page adds 1 credit. To control PDF costs, use `scrapeOptions.parsers: []` to skip PDF parsing (returns base64 for 1 credit per file) or `scrapeOptions.parsers: [{"type": "pdf", "maxPages": 10}]` to limit parsed pages.
</Note>
