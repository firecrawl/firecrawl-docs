...
<Note>
  The crawl endpoint automatically strips URL hash/anchor fragments (e.g., `#section-name`) and deduplicates the resulting URLs. This means single-page sites that use only anchor-based navigation will return few or no results, since all anchor links resolve to the same base URL. For these sites, use the [scrape](/features/scrape) endpoint instead to extract the full page content.
</Note>
