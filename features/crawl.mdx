...
The `limit` parameter controls the number of pages crawled, not the total credits consumed. Credit usage can vary based on the content of each page:

- **Basic scrape**: 1 credit per webpage
- **PDF parsing**: 1 credit per PDF page (a 50-page PDF costs 50 credits)

To control costs when crawling sites with large PDFs, use `scrapeOptions.parsers`:
- Set `parsers: []` to skip PDF parsing entirely (returns base64-encoded PDF for 1 credit)
- Set `parsers: [{"type": "pdf", "maxPages": 10}]` to limit parsed pages per PDF
