...
<Info>
  Each page crawled consumes 1 credit (plus additional credits for options like JSON mode, enhanced proxy, or PDF parsing). The default `limit` is 10,000 pages. Set a lower `limit` to control credit usage - for example, `limit: 100` to cap the crawl at 100 pages.
</Info>
...
The `limit` parameter controls the number of URLs crawled, not total credits. Each crawled page costs 1 credit, but if the page contains a PDF, parsing costs 1 credit per PDF page. This means total credits can exceed your `limit` if PDFs are encountered.

To control costs when crawling sites with PDFs, set `parsers: []` in your `scrapeOptions` to skip PDF parsing (returns base64 for 1 credit flat), or use `parsers: [{"type": "pdf", "maxPages": 10}]` to limit PDF pages parsed.
