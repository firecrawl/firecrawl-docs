...
Crawl billing is based on the content of each page scraped:

- **Basic scrape**: 1 credit per page crawled
- **PDF parsing**: 1 credit per PDF page (PDFs are parsed by default)

<Note>
  The `limit` parameter controls how many pages are crawled, not the total credits consumed. If crawled pages contain PDFs, credit usage may exceed the page limit since each PDF page costs 1 credit. To control PDF costs, use `scrapeOptions.parsers` to limit PDF pages or disable PDF parsing. See [Document Parsing](/features/document-parsing) for details.
</Note>
