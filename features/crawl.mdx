...
- **Crawl errors**: The `data` array contains pages Firecrawl successfully scraped. Use the [Get Crawl Errors](/api-reference/endpoint/crawl-get-errors) endpoint to retrieve pages that failed due to network errors, timeouts, or robots.txt blocks. A large PDF that outlived the scrape window may appear there as `SCRAPE_TIMEOUT` with `details.state: "processing_continues"` — retrying that URL with [`/scrape`](/features/scrape) attaches to the in-flight job. See [Large PDFs](/features/parse#large-pdfs).
