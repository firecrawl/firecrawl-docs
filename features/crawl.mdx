...
<Info>
  Each page crawled consumes 1 credit (plus additional credits for options like JSON mode, stealth proxy, or PDF parsing). The default `limit` is 10,000 pages. Set a lower `limit` to control credit usage - for example, `limit: 100` to cap the crawl at 100 pages.
</Info>
