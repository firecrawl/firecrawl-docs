...
The `limit` parameter controls the number of pages crawled, not the total credits consumed. Credit usage depends on what each page contains:
...
To control PDF costs, use the `parsers` option in `scrapeOptions`:
- Set `parsers: []` to skip PDF parsing entirely (returns base64 for 1 credit flat)
- Set `parsers: [{"type": "pdf", "maxPages": 10}]` to limit PDF pages parsed
