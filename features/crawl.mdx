...
<Note>
  Crawl strips URL hash fragments (e.g., `#section`) and deduplicates URLs, so sites that use only anchor links for navigation (single-page documentation sites, SPAs) may return few or no results. For these sites, use the [Scrape](/features/scrape) endpoint instead to get the full page content.
</Note>
