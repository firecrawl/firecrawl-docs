...
### Combining actions with JSON extraction

Actions can be combined with the `json` format to interact with a page (e.g., clicking a button to reveal content) and then extract structured data from the result. This is useful when the data you need only appears after a user interaction, such as opening a modal or expanding a section.
...
curl -X POST https://api.firecrawl.dev/v2/scrape \
    ...
    -d '{
      ...
      "formats": [{
        "type": "json",
        "prompt": "Extract the product details shown in the modal.",
        "schema": {
          "type": "object",
          "properties": {
            "name": { "type": "string" },
      ...
      "actions": [
        ...
        { "type": "click", "selector": ".product-item" },
