...
The `zeroDataRetention` option applies to the [Scrape](/api-reference/endpoint/scrape), [Crawl](/api-reference/endpoint/crawl), and [Batch Scrape](/api-reference/endpoint/batch-scrape) endpoints. For ZDR on the Search endpoint, see [ZDR Search](/features/search#zdr), which uses a separate `enterprise` parameter.
