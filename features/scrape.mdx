...
- Audio (`audio`) - extract MP3 audio from YouTube videos (returns a signed GCS URL, expires after 1 hour)
...
The `audio` format extracts audio from YouTube videos as MP3 files and returns a signed Google Cloud Storage URL. This is useful for building audio processing pipelines, transcription services, or podcast tools.

The audio format only works with YouTube URLs (`youtube.com/watch?v=...` or `youtu.be/...`). Using it with non-YouTube URLs will return a validation error. The signed URL expires after 1 hour.
