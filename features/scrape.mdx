...
`maxAge` controls whether Firecrawl may reuse indexed content; it does not tell you whether the state represented by a page is current. When freshness matters, use `maxAge: 0` to bypass index reuse, then inspect the returned content and any redirect or source-specific status signals. A successful response alone is not proof of current state.
