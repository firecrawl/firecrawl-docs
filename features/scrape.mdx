...
<Note>
**Large PDFs:** a long document can outlive the request `timeout`. Firecrawl keeps processing it and returns HTTP `408` with `code: "SCRAPE_TIMEOUT"` and `details.state: "processing_continues"`. Retry the same URL after the `Retry-After` header — the retry attaches to the in-flight job and returns the finished result without re-processing. The JS and Python SDKs do this automatically on `scrape()`. See [Large PDFs](/features/parse#large-pdfs) and [Errors](/api-reference/errors#large-pdf-timeouts).
</Note>
