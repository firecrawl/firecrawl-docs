...
- Video (`video`) - extract best-quality video from supported video URLs, e.g. YouTube or TikTok (returns a signed GCS URL, expires after 1 hour)
...
The `video` format extracts best-quality video from supported websites (e.g. YouTube and TikTok) and returns a signed Google Cloud Storage URL. This is useful for building video processing pipelines, moderation tools, or media archiving workflows.
