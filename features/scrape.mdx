...
`maxAge` controls whether Firecrawl may reuse indexed content; it does not tell you whether the item described by a page is still active. When freshness matters, use `maxAge: 0` to bypass index reuse, then inspect the returned content and any redirect or source-specific status signals. A successful response alone is not proof that a job, product, or listing is still available.
