...
import ScrapeQueryPython from "/snippets/v2/scrape/query/python.mdx";
import ScrapeQueryNode from "/snippets/v2/scrape/query/js.mdx";
import ScrapeQueryCURL from "/snippets/v2/scrape/query/curl.mdx";
...
<Info>
  Each scrape consumes 1 credit. Additional credits apply for certain options: JSON mode costs 4 additional credits per page, the query format costs 4 additional credits per page, enhanced proxy costs 4 additional credits per page, PDF parsing costs 1 credit per PDF page, and audio extraction costs 4 additional credits per page.
</Info>
...
The `query` format lets you ask a natural-language question about the page. Firecrawl scrapes the page and runs an LLM over the content, returning the answer in the response's `answer` field. Use it for one-shot Q&A over a URL without writing a JSON schema.

<Info>
  The query format costs 5 credits per page (1 base + 4 additional for the LLM call).
</Info>

Options inside the format object:

- `prompt` (required): the question to answer. Maximum 10,000 characters.
- `mode` (optional): `"freeform"` answers the question using the page content. `"directQuote"` returns the relevant source text from the page. Defaults to `"freeform"`.

You can combine `query` with other formats — for example, request `markdown` and `query` together to get both the page content and the answer in a single call.
...
The query format is also available in `/search` via `scrapeOptions`, which runs the same Q&A across each search result.
