...
If `metadata.cache` is absent, Firecrawl cannot attest cache provenance for that response. Absence does not prove a cache miss, a fresh scrape, cache bypass, or that the source page is still live. The legacy `metadata.cacheState`/`cachedAt` fields may still appear for compatibility, but new agent workflows should prefer `metadata.cache`.
