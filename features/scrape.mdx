...
- **Performance**: This can reduce latency when data does not need to be ultra-fresh.
- **Request fresh retrieval**: Set `maxAge` to `0` to skip Firecrawl's index cache and use the full scraping pipeline. This can take longer and be more likely to fail. Use a non-zero `maxAge` if freshness on every request is not critical.
...
- **Credits**: Cached results still cost 1 credit per page. Caching improves latency, not credit usage.

### Cache provenance metadata

When Firecrawl can attest that a response was served from its own index cache, the document metadata includes:
...
If `metadata.cache` is absent, Firecrawl cannot attest cache provenance for that response. Absence does not prove a cache miss, a fresh scrape, cache bypass, or that the source page is still live. The legacy `metadata.cacheState`/`cachedAt` fields may still appear for compatibility, but new agent workflows should prefer `metadata.cache`. `metadata.indexId`, when present, identifies an index record but does not by itself prove that the response was served from cache.

### Agent liveness checks for job and ATS pages

HTTP `200`, rendered HTML, and `metadata.cache` only describe how Firecrawl obtained the response. They do not prove that a business object on the page is still active. For job postings, event pages, inventory pages, or other stateful records, add an application-level liveness gate immediately before downstream action:

- Prefer the ATS, job board, marketplace, or product API when one is available.
- Treat redirects to generic listing pages, missing apply buttons, closed/expired labels, and changed canonical IDs as not-live signals.
- Use `maxAge: 0` for the final verification scrape if you must verify through HTML, then validate the page content itself.
- Do not use `metadata.cache` or legacy `metadata.cacheState` as the only signal that a posting is still open.
