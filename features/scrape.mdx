...
- **Always fetch fresh**: Set `maxAge` to `0`. Note that this bypasses the cache entirely, so every request goes through the full scraping pipeline. This increases latency and the likelihood of hitting the default 30-second timeout, especially on slower or heavily-protected sites. Consider increasing `timeout` (e.g. `60000` ms) if you use `maxAge: 0`.
