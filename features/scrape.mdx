...
Customize scraping behavior with these options:
...
- **Type**: `boolean`
- **Default**: `true`
- **Description**: By default, Firecrawl returns only the main content of the page. Set to `false` to return the full page content including headers, footers, and sidebars.

#### `includeTags`
- **Type**: `array`
- **Description**: Specify HTML tags, classes, or IDs to include in the output. Only matching elements will be scraped.
- **Example**: `["h1", "p", "a", ".main-content", "#article"]`

#### `excludeTags`
- **Type**: `array`
- **Description**: Specify HTML tags, classes, or IDs to exclude from the output.
- **Example**: `["#ad", "#footer", ".sidebar", "nav"]`
...
- **Default**: `0`
- **Description**: Extra wait time before scraping, in addition to Firecrawl's smart wait feature. Use sparingly for pages with delayed content loading.

#### `timeout`
- **Type**: `integer` (milliseconds)
- **Default**: `30000` (30 seconds)
- **Description**: Maximum duration before the scrape request is aborted.

### PDF Handling

#### `parsers`
- **Type**: `array`
- **Description**: Control PDF parsing behavior.
- **Options**:
  - `["pdf"]` - Parse PDF content (1 credit per page)
  - `[]` - Skip PDF parsing, return as base64 (1 credit flat)
  - `[{"type": "pdf", "maxPages": 10}]` - Limit parsing to first N pages
...
doc = firecrawl.scrape('https://example.com', {
    ...
    'excludeTags': ['#cookie-banner', 'footer'],
    'waitFor': 2000,
    'timeout': 60000
})
```
...
|--------|-------------|------------|
| `wait` | Wait for page to load | `milliseconds` or `selector` |
| `click` | Click an element | `selector`, `all?` (click all matches) |
| `write` | Type text into a field | `text` (element must be focused first) |
| `press` | Press a keyboard key | `key` |
| `scroll` | Scroll the page | `direction` ("up"/"down"), `selector?` |
| `screenshot` | Capture screenshot | `fullPage?`, `quality?`, `viewport?` |
| `scrape` | Scrape current state | - |
| `executeJavascript` | Run JavaScript | `script` |
| `pdf` | Generate PDF | `format?`, `landscape?`, `scale?` |

### Example

Here is an example of how to use actions to navigate to google.com, search for Firecrawl, click on the first result, and take a screenshot.

<Tip>Always use the `wait` action before/after executing other actions to give the page time to load.</Tip>
...
- **Write action**: You must first focus the element using a `click` action before using `write`. Text is typed character by character to simulate real keyboard input.
- **Scroll with selector**: To scroll a specific element instead of the whole page, provide the `selector` parameter.
- **Wait with selector**: Use `wait` with a `selector` to wait for a specific element to appear, or use `milliseconds` for a fixed delay.
- **Sequential execution**: Actions run in order, and Firecrawl waits for each interaction to complete before proceeding.

### Advanced Action Examples

**Taking a screenshot after clicking:**
...
For more details about action parameters, refer to the [API Reference](https://docs.firecrawl.dev/api-reference/endpoint/scrape).
