...
Firecrawl supports Zero Data Retention for teams with strict data handling requirements. When enabled, Firecrawl will not persist any page content, screenshots, or extracted data beyond the lifetime of the request.
...
The `zeroDataRetention` option applies to the [Scrape](/api-reference/endpoint/scrape), [Crawl](/api-reference/endpoint/crawl), and [Batch Scrape](/api-reference/endpoint/batch-scrape) endpoints. For ZDR on the Search endpoint, see [ZDR Search](/features/search#zero-data-retention-zdr-search), which uses a separate `enterprise` parameter.
