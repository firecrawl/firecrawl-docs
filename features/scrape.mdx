...
<Info>
  Each scrape consumes 1 credit by default (markdown format). JSON mode costs 4 additional credits per page (5 total) as it uses LLM extraction. Other add-ons like enhanced proxy (+4 credits) and PDF parsing (+1 credit per PDF page) also increase cost. See [Pricing](https://www.firecrawl.dev/pricing) for full details.
</Info>
