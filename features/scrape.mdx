...
<Note>
**Screenshot for Anti-Bot Protection**: When scraping websites protected by Cloudflare or similar anti-bot mechanisms, adding the `screenshot` format triggers full browser rendering, which significantly improves success rates. This executes JavaScript and creates a more complete browser fingerprint, helping bypass detection systems. If you're encountering 403 errors or "Just a moment..." pages, try adding `formats: ["markdown", "screenshot"]` to your request.
</Note>
...
Some websites use anti-bot mechanisms (like Cloudflare) that can return 403 errors or block-detection pages. Here are strategies to improve scraping reliability:

- **Enable screenshot rendering**: Add `formats: ["screenshot"]` to trigger full browser execution, which creates a more complete browser fingerprint and handles JavaScript challenges. This is often the most effective solution.
- **Disable ad blocking**: Set `blockAds: false` in your scrape options. This allows more natural page loading patterns and can improve success rates on some sites.
- **Use stealth mode**: Enable the `proxy: "stealth"` option to use stealth proxies. Learn more about [Stealth Mode](/features/stealth-mode).
- **Try location-based proxies**: Some sites require traffic from specific geographic regions. Use the `location.country` parameter to specify a country proxy. See [Proxies documentation](/features/proxies) for available locations.

If you're consistently getting 403 errors despite trying these approaches, the site may have strengthened its detection mechanisms.

## Stealth Mode

For complex websites, Firecrawl offers stealth mode that provides better success rates while maintaining privacy.
