...
<Note>
This is separate from the `zeroDataRetention` option on [Scrape](/api-reference/endpoint/scrape), [Crawl](/api-reference/endpoint/crawl), and [Batch Scrape](/api-reference/endpoint/batch-scrape), which controls ZDR for scraping operations. The `enterprise` parameter only applies to the search portion of the request.
</Note>
