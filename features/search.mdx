...
Use the `country` parameter to target specific countries using ISO country codes (defaults to "us"):
...
Common country codes: `"us"` (United States, default), `"de"` (Germany), `"fr"` (France), `"jp"` (Japan), `"uk"` (United Kingdom), `"ca"` (Canada).
