...
Developer results come back in their own `developer` group next to `web`, each carrying `category: "developer"`. For ranked results with the matched passages, and for the repository and documentation-source filters, use the [developer search endpoint](/features/developer#developer-search-endpoint).
