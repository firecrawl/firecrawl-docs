...
Use the `sources` parameter to choose what kind of results you want back. You can mix sources in a single call to get web pages, news articles, and images side by side.

- `web`: standard web results (default)
- `news`: news-focused results
- `images`: image search results

When you request multiple sources (e.g. `sources: ["web", "news"]`), the `limit` parameter applies **per source type**. With `limit: 5` and two sources, you get up to 5 results of each, for 10 total. If you need different `limit` values or different `scrapeOptions` per source, make separate calls instead.
