...
<Note>
**SDK users:** search results are grouped by source type, not under a generic `.data` array. Access web results with `result.web`, news with `result.news`, and images with `result.images`. When you request the `developer` category, read those results with `result.developer`.
...
- `pdf`: Search for PDFs
- `developer`: Add results from the [Developer Index](/features/developer), covering GitHub issues, merged pull requests, repository READMEs, and curated documentation sites
...
Search the developer index for programming questions:
...
Unlike the other categories, `developer` returns its results in a separate `developer` group rather than mixed into `web`. See the [Developer Index](/features/developer) for the dedicated endpoint, its filters, and the matched passages it returns.
