...
## Zero Data Retention (ZDR) Search

For teams with strict data handling requirements, Firecrawl offers Zero Data Retention (ZDR) options for the `/search` endpoint via the `enterprise` parameter. ZDR search must be enabled for your team by contacting [support@firecrawl.com](mailto:support@firecrawl.com).

<Note>
ZDR search is separate from the team-level `zeroDataRetention` scrape option. Teams with forced ZDR at the team level cannot use the search endpoint — contact support to configure search-specific ZDR access.
</Note>
