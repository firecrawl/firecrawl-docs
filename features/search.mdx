...
Use the `tbs` parameter to filter results by time. Note that the `tbs` parameter only applies to `web` source results and is not supported for `news` sources. If you need time-filtered news, consider using `web` results with the `site:` operator to target specific news domains.
