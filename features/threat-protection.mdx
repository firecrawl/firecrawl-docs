...
Threat Protection has four modes, set at the organization level:

- **Off** (default) — no checks are performed.
- **Manual only** — only your own policy is enforced: the custom blacklist, whitelist, and blocked TLDs described below. No classifier is consulted, so nothing leaves your policy and there are **no scan fees**.
...
If Threat Protection is **enforced** for your team, an override may tighten the policy but never weaken it: `"mode": "off"` is always rejected, and the mode may not be lowered below the organization's mode (the order is `off` < `manual-only` < `normal` = `zscaler`). A request that tries is rejected with a `403`. The organization policy itself also cannot be set to `off` or `manual-only` while enforced.
...
In Normal mode a scan costs **+2 credits per URL scanned**, on top of the base cost of the request. **Manual only and Zscaler modes have no scan fees**: Manual only never calls a classifier, and Zscaler classification runs against your own ZIA tenant, with your credentials and your API quota. The scan-fee details below apply to Normal mode only. A few details:
...
| `403` | A `threatProtection` override sets `mode: "off"`, or lowers the mode below the organization's mode, while Threat Protection is enforced for the team. |
| `403` | The organization policy is updated with `mode: "off"` or `mode: "manual-only"` while Threat Protection is enforced for the team. |
