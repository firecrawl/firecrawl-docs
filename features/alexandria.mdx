...
<AccordionGroup>
  ...
  <Accordion title="Retries">
    SDKs reuse the request ID automatically when retrying. For manual retries, send the same payload and request ID (`x-request-id` over HTTP). Completed requests return the saved result without running or charging again.

    If a request is still running or its outcome is uncertain, keep the same ID. Use a new ID when changing the payload.
  </Accordion>
</AccordionGroup>
