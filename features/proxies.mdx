...
<Info>If you request a country where a proxy is not available, Firecrawl will fall back to the closest available region — Germany (`DE`) for EEA countries and the United States (`US`) for all others — and set the browser location to your requested country. This fallback happens silently, so the egress IP may not match `location.country` even when the country is listed in the table above. If you need a guarantee that the request egresses from the requested country, use `proxy: "stealth"` where available, or [contact us](mailto:help@firecrawl.com).</Info>
