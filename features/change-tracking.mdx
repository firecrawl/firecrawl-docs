...
- Scoped to your team — each API key maintains its own tracking history

For endpoint details, see the [Scrape API Reference](https://docs.firecrawl.dev/api-reference/endpoint/scrape).
...
Include both `markdown` and `changeTracking` in the `formats` array. The `markdown` format is required — change tracking compares pages via their markdown content.
...
By default, change tracking compares against the most recent scrape of the same URL under your API key. Tags let you maintain **separate tracking histories** for the same URL — useful when you monitor the same page at different intervals or in different contexts.
...
|-----------|------|---------|-------------|
| `type` | `string` | — | Must be `"changeTracking"` |
| `modes` | `string[]` | `[]` | Diff modes to enable: `"git-diff"`, `"json"`, or both |
| `schema` | `object` | — | JSON Schema for field-level comparison (required for `json` mode) |
| `prompt` | `string` | — | Custom prompt to guide LLM extraction (used with `json` mode) |
...
- **Scoping** — Comparisons are scoped to your team (API key). Your first scrape of any URL returns `"new"`, even if other users have scraped it.
