...
<Note>
Once a batch scrape job is submitted, it continues processing on Firecrawl's servers even if you stop your local script. Stopping your script only prevents new jobs from being submitted — it does not cancel jobs already in progress. To cancel a running batch scrape, use the [cancel endpoint](/api-reference/endpoint/batch-scrape-delete) (`DELETE /v1/batch/scrape/{id}`).
</Note>
