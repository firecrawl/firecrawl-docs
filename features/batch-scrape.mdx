...
By default, each batch scrape job is limited to a low number of concurrent scrapes. If you have a higher-tier plan with more concurrent browser slots available (see [Rate Limits](/rate-limits)), set the `maxConcurrency` parameter to take full advantage of your plan's capacity. For example, if your plan supports 100 concurrent browsers, using `maxConcurrency: 5` (the default on some SDKs) means only 5 URLs are processed at a time — significantly slowing down large jobs.
