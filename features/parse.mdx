...
- `parsers`: File-parser controls — see [PDF parsing](#pdf-parsing) below.
...
result is assembled in reading order into clean markdown. Everything below
works identically for public PDF URLs via [`/scrape`](/features/scrape) —
same `parsers` option, same output.

All PDF behavior is controlled through the `parsers` option:
...
| `pages` | `boolean` | `false` | Also return physical per-page markdown. No additional cost. |
| `blocks` | `boolean` | `false` | Also return typed layout blocks with bounding boxes. No additional cost. |
...
- The first parse of a document runs the full pipeline; repeat parses of the same document are served from cache and return in seconds.
