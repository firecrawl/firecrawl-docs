...
- **Images too**: PNG, JPEG, JPEG 2000, TIFF, GIF, BMP, WebP and AVIF are OCR'd as one-page documents
...
Raster images are OCR'd as one-page scanned documents, on `/parse` and on
`/scrape` alike. Upload a PNG, JPEG, JPEG 2000, TIFF, GIF, BMP, WebP or AVIF
file, or scrape an image URL, and the document carries the recognized text as
markdown with `metadata.contentType` set to the image's type. An image is
billed as one page (1 credit); an image with no readable text returns an empty
markdown string.

The `image` parser is part of the default `parsers` list next to `pdf`, so
nothing needs to be set to turn it on. It takes no options: `"image"` and
`{ "type": "image" }` are equivalent. To keep image URLs failing as
unsupported files on `/scrape`, send a `parsers` list that omits it, for
example `["pdf"]`. To receive the bytes instead of OCR text, request the
`rawBase64` format.
