...
- `parsers`: File-parser controls — see [PDF Parsing](/features/pdf-parsing) for the full reference.
...
PDF behavior — parsing mode, page caps, per-page markdown, and typed layout
blocks with bounding boxes — is controlled through the `parsers` option:

```json
{
  "parsers": [
    { "type": "pdf", "mode": "ocr", "maxPages": 50, "pages": true, "blocks": true }
  ]
}
```
...
See [PDF Parsing](/features/pdf-parsing) for the full option reference, the
blocks schema, and grounding recipes.
