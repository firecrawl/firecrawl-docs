...
A long PDF can take longer than the request `timeout` to finish. Firecrawl
does not cancel that work. The request returns HTTP `408` with
`code: "SCRAPE_TIMEOUT"`, and the document keeps processing server-side.

On [`/scrape`](/features/scrape), the response includes structured retry
guidance and a `Retry-After` header:
...
`Retry-After` matches `details.retryAfterSeconds` (capped at 10 minutes).
Retrying the **same URL** after that delay attaches to the in-flight job and
returns the finished result — the document is not re-processed.

`jobStatus` is `"running"` while pages are being parsed, or `"queued"` if the
job is waiting for a worker (it keeps its place). If
`details.mayExceedProcessingWindow` is `true`, the document may be larger than
the server-side processing ceiling; keep retrying to pick up a result if it
completes, or raise `timeout` / contact support.

**What to do**

1. **Retry the same request** after `Retry-After`. Do not change the URL or
   start a different document.
2. **Raise `timeout`** (up to `300000` ms) if you want the first call to wait.
   Very large documents can still outlive the maximum request window — that is
   expected; retry.
3. **Cap the work** with [`maxPages`](#pdf-options) when you do not need every
   page.
4. **JS and Python SDKs auto-resume** on this signal: `firecrawl` ≥ 4.37 and
   `firecrawl-py` ≥ 4.40 sleep the advertised delay and re-issue `scrape()`.
   Opt out with `autoResume: false` (JS) or `auto_resume=False` (Python). Other
   SDKs and raw HTTP clients should retry only when
   `details.state === "processing_continues"` — not on every 408.

`/parse` can return the same `SCRAPE_TIMEOUT` / 408 when an uploaded PDF
outlives the request. Retry the same file. The JS/Python auto-resume loop
applies to `scrape()` only.

See [Errors](/api-reference/errors#large-pdf-timeouts) for the full error
shape and a retry snippet.
...
- `timeout`: Request timeout in milliseconds. Defaults to `30000`, max `300000`. Long PDFs that outlive this window keep processing — see [Large PDFs](#large-pdfs).
...
- Long or scanned PDFs can outlive the request `timeout`. Firecrawl keeps processing them and returns a retryable `408` — see [Large PDFs](#large-pdfs). Use `maxPages` when you want to bound the work.
