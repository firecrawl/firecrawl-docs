...
- `parsers`: File-parser controls. For PDFs, set `{ "type": "pdf", "mode": "fast" | "auto" | "ocr", "maxPages": <int>, "pages": <bool>, "blocks": <bool> }`.
...
### Per-page markdown and layout blocks

Two PDF parser options enrich the response at no additional cost:

- `pages: true` — the document also carries a `pages` array with the physical
  per-page markdown: `[{ "pageNumber": 1, "markdown": "..." }, ...]`.
- `blocks: true` — the document also carries a `blocks` array: for every page,
  typed layout blocks (`title`, `section_header`, `text`, `table`, `formula`,
  `figure`, ...) with normalized bounding boxes, reading order, and character
  spans linking each block back into the document markdown. Use this for
  citation grounding, highlight overlays, or auditing document contents.

```bash cURL
curl -X POST https://api.firecrawl.dev/v2/parse \
  -H 'Authorization: Bearer YOUR_API_KEY' \
  -F 'file=@./report.pdf' \
  -F 'options={"parsers":[{"type":"pdf","pages":true,"blocks":true}]};type=application/json'
```
