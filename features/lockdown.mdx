...
Lockdown uses the same cache-match rules as regular scrapes. For a cache hit, these parameters must match the cached entry: `url`, `mobile`, `location`, `waitFor`, `blockAds`, `screenshot` (enabled/disabled and full-page), and enhanced proxy mode. Served hits include attested cache provenance in `metadata.cache` with `source: "firecrawl-index"` and `cachedAt`; legacy `metadata.cacheState: "hit"` may also appear for compatibility.

Unlike a regular scrape, a lockdown cache miss never falls through to a live scrape. It returns `404` with `SCRAPE_LOCKDOWN_CACHE_MISS`; an absent `metadata.cache` on a successful non-lockdown response does not imply the same outcome.
