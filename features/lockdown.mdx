...
Lockdown uses the same cache-match rules as regular scrapes. For a cache hit, these parameters must match the cached entry: `url`, `mobile`, `location`, `waitFor`, `blockAds`, `screenshot` (enabled/disabled and full-page), and enhanced proxy mode. Served responses include attested cache provenance in `metadata.cache` with `source: "firecrawl-index"` and `cachedAt`; legacy `metadata.cacheState: "hit"` may also appear for compatibility.
