...
Pass `developer` in the `categories` array on `/search` when you are already calling `/search` and want developer results in that call. Developer results are returned inside the standard `web` field — there is no separate `developer` group. With `categories: ["developer"]` alone, the response contains developer-index results only. Developer results are recognizable by their `id` prefix (`doc:`, `issue:`, `pull_request:`, `readme:`).
