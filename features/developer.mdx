...
Alongside the results, `coverage` reports index health per result type and `reranked` reports whether the ranked list went through the reranking stage. Check `coverage` when a result type you expected is missing: a `degraded` or `unavailable` status means the gap came from the index rather than from the query.
...
- `language`, `topic`, `license`, `min_stars`, `max_stars`, `archived`, and `fork` filter on repository attributes
