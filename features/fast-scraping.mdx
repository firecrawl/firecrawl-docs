...
- **Request fresh retrieval**: Set `maxAge: 0` to skip Firecrawl's index cache and use the full scraping pipeline. This can take longer and be more likely to fail. Use a non-zero `maxAge` if you don't need real-time content on every request.
...
Served cache hits include attested cache provenance in `metadata.cache` with `source: "firecrawl-index"` and `cachedAt`; legacy `metadata.cacheState: "hit"` may also appear for compatibility.
