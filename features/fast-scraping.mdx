...
For an attested Firecrawl-index cache hit, responses include `metadata.cache` with `source: "firecrawl-index"` and `cachedAt`. If `metadata.cache` is absent, Firecrawl cannot attest cache provenance; absence is not proof of a cache miss, fresh scrape, bypass, or page liveness. The legacy `metadata.cacheState` field may still appear as `"hit"` for compatibility, but agents should prefer `metadata.cache`.
