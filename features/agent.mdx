...
import AgentZdrPython from "/snippets/v2/agent/zdr/python.mdx";
import AgentZdrJS from "/snippets/v2/agent/zdr/js.mdx";
import AgentZdrCURL from "/snippets/v2/agent/zdr/curl.mdx";
...
<Note>Job results are available via the API for 24 hours after completion. If you run with Zero Data Retention, activity logs and history entries are not stored.</Note>
...
| `zeroDataRetention` | boolean | No | Run with Zero Data Retention: prompts, schemas, URLs, and activity logs are not stored. |
| `forceZDR` | boolean | No | Force Zero Data Retention for approved accounts. Requests fail if ZDR is not enabled. |

## Zero Data Retention (ZDR)

Enable `zeroDataRetention` to keep agent activity out of storage and logs. Activity endpoints return empty responses for ZDR runs, and activity history in the dashboard is unavailable for those jobs.
