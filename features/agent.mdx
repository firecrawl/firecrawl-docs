...
The [Agent Playground](https://www.firecrawl.dev/app/agent) supports CSV upload for processing multiple inputs in parallel. Upload a CSV containing your input data (e.g., company names or URLs), write a prompt describing what to extract for each row, define your output fields, and run — the agent processes each row in parallel and fills in the results.
