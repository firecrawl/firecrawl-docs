...
<Warning>
**Always set `maxCredits`** to prevent runaway sessions. Without a credit limit, agent tasks can loop extensively on difficult queries and consume a large number of credits. If the limit is reached, the job will fail and no data is returned, but you won't be charged beyond the cap.
</Warning>
