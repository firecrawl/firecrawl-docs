...
| `maxCredits` | number | No | Maximum number of credits to spend on this agent task. If not set, a default cap of 38,000 credits applies. If the limit is reached, the job fails and **no data is returned**, though credits consumed for work performed are still charged. It is recommended to set this parameter to control costs. |
