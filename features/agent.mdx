...
We offer 5 daily runs for free. While in Research Preview, pricing is dynamic and based on the number of tokens `/agent` uses. That's converted to credits. Try it out [here](/app/agent).