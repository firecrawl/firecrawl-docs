...
| `maxCredits` | number | No | Maximum number of credits to spend on this request. Useful for controlling costs. |
| `strictConstrainToURLs` | boolean | No | When `true`, restricts the agent to only visit the provided URLs, preventing autonomous web searches. Significantly reduces credit usage when combined with the `urls` parameter. |
