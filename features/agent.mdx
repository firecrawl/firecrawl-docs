...
<Note>
When `maxCredits` is reached, the agent job immediately stops and fails with an error. Unlike the eventual return of partial results, **no data collected up to that point is returned**. The credits are still charged for the work performed. Plan your `maxCredits` limits accordingly to avoid losing data.
</Note>
