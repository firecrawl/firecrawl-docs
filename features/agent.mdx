...
When an agent hits the `maxCredits` limit, the job ends with `status: "failed"` and `error: "Max credits limit reached"`. The response still includes a `partial` field with the most recent results, so you can use those immediately.
...
if (status.status === "failed" && status.error === "Max credits limit reached") {
  const partial = status.partial;
  // Use partial results here
}
```
...
if status.status == "failed" and status.error == "Max credits limit reached":
