...
| `fail` | The run stops making calls as soon as it needs a gated provider, and `exchange.error.code` is `THIRD_PARTY_DATA_TERMS_REQUIRED`. Like `ask`, it also ends with `exchange.requiresAction` and a `pendingApproval` of `kind: "terms"`, so you can accept and continue the thread. This mode's behaviour may still change. |
...
- `requiresAction` (`ask` and `fail`): `type: "accept_terms"`, an `approvalId` and a `providers` list. `approvalId` is always present. It's the id of the `terms` pending approval you answer when you continue the thread. Each provider carries the exact `show` (`terms/show`) and `accept` (`terms/accept`) calls to make, plus an `id`. `accept.options.digest` can be `null`. When it is, run `terms/show` first and send the digest it returns.
- If the same turn also stopped on a paid-call approval (`requireApproval`), that approval comes first. The turn then has `skippedProviders` but no `requiresAction`, and its `pendingApproval` is the paid-call one. The terms offer, with its own `approvalId`, arrives on the next turn.
