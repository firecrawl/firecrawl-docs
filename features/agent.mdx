...
import AgentWithMaxCreditsPython from "/snippets/v2/agent/with-max-credits/python.mdx";
import AgentWithMaxCreditsJS from "/snippets/v2/agent/with-max-credits/js.mdx";
import AgentWithMaxCreditsCURL from "/snippets/v2/agent/with-max-credits/curl.mdx";
...
To prevent agents from consuming excessive credits on complex tasks, use the `maxCredits` parameter to set a spending limit. When the agent reaches this limit, it automatically stops and returns an error, preventing runaway costs:

<CodeGroup>

<AgentWithMaxCreditsPython />
<AgentWithMaxCreditsJS />
<AgentWithMaxCreditsCURL />
