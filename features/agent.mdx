...
Firecrawl `/agent` is a magic API that searches, navigates, and scrapes even the most complex websites, finding data in hard-to-reach places and discovering data anywhere on the internet. It accomplishes in a few minutes what would take a human many hours, and makes traditional web scraping obsolete.
...
<Info>
  Token usage varies based on the complexity of your prompt, the amount of data processed, and the structure of the output requested.
</Info>
...
Additional usage is billed based on token consumption and converted to credits. Each credit equals 15 credits, consistent with other Firecrawl endpoints.
...
Try Agent now at [firecrawl.dev/app/agent](/app/agent) to see how token usage scales with your specific use cases.
