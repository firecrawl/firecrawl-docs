...
The `/agent` endpoint is the next evolution of our [`/extract`](/features/extract) endpoint. It uses AI agents to intelligently gather and structure data from across the web. Unlike `/extract`, Agent requires only a prompt—URLs are optional—making it incredibly flexible for a wide variety of queries.

<Info>Agent is the successor to `/extract`: more reliable, faster, and cheaper.</Info>

## Why Agent over Extract?

Agent builds on everything great about `/extract` and takes it further:

- **Prompt-Only**: Just describe what you need—no URLs required
- **Smarter**: AI agents understand context and find relevant data automatically
- **Faster**: Optimized processing for quicker results
- **Cost-Effective**: More efficient use of resources means lower costs
- **Flexible**: Works with a wide variety of queries and use cases
