...
<Note>To retrieve your extracted data, you must use the API or SDK — the [activity logs](https://www.firecrawl.dev/app/logs) page shows job metadata (credits used, timing, model, etc.) but not the extracted results themselves. Job results are available via the API for 24 hours after completion.</Note>
...
| `maxCredits` | number | No | Maximum number of credits to spend on this agent task. Defaults to **2,500** if not set. If the limit is reached, the job fails and **no data is returned**, though credits consumed for work performed are still charged. |
