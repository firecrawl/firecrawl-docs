...
| `maxCredits` | number | No | Maximum number of credits to spend on this agent task. When the threshold is reached, the job fails and returns a best-effort `partial` payload with `errorCode: "MAX_CREDITS_EXCEEDED"`. Credits consumed for work performed are still charged. |
| `maxCreditsThreshold` | number | No | Threshold ratio `(0, 1]` used to trigger a partial emit before reaching `maxCredits` (defaults to `0.8`). |

### Partial results on max credits

If the agent hits the max-credits threshold, the status endpoint returns `status: "failed"` with `errorCode: "MAX_CREDITS_EXCEEDED"` and a `partial` payload. SDK convenience helpers throw a `MaxCreditsExceededError` so you can opt in to using `error.partial` if you want the partial data.

```js
import Firecrawl, { MaxCreditsExceededError } from "@mendable/firecrawl-js";

try {
  await firecrawl.agent({ prompt: "..." });
} catch (error) {
  if (error instanceof MaxCreditsExceededError) {
    console.log("Partial result:", error.partial);
  }
}
```
...
from firecrawl.v2.utils import MaxCreditsExceededError

try:
    app.agent(prompt="...")
except MaxCreditsExceededError as error:
    print("Partial result:", error.partial)
```
...
* **Tune `maxCreditsThreshold`**: Control how early the agent stops and emits partial results
