...
Agent pricing is dynamic and varies significantly based on your request. Key factors that impact credit consumption:

- **Output format**: Requesting structured JSON output or complex schema extraction costs more credits than simple text extraction
- **Number of sources**: Agents that search and navigate multiple websites consume more credits than single-page extractions
- **Prompt complexity**: More detailed prompts requiring deeper research and reasoning use additional credits
- **Scraping options**: If your schema requires scraping with advanced options (stealth proxy, PDF parsing), additional costs apply per page

**Recommendation**: Use your 5 daily free runs to test your specific use case and understand the credit impact before scaling up.
