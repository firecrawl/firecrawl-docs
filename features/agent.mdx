...
| `maxCredits` | number | No | Maximum number of credits to spend on this agent task. Defaults to **38,000** if not set. If the limit is reached, the job fails and **no data is returned**, though credits consumed for work performed are still charged. |
