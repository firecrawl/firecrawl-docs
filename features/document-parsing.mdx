...
- **Images** (`.png`, `.jpg`, `.jpeg`, `.jp2`, `.tif`, `.tiff`, `.gif`, `.bmp`, `.webp`, `.avif`)
  - OCR'd as a one-page scanned document (1 credit)
  - Returns an empty markdown string when the image has no readable text
  - See [Image OCR](/features/parse#image-ocr) for how to opt out per request.
