...
import ReplayPython from "/snippets/v2/interact/replay/python.mdx";
import ReplayJS from "/snippets/v2/interact/replay/js.mdx";
import ReplayCURL from "/snippets/v2/interact/replay/curl.mdx";
...
<CardGroup cols={2}>
  ...
  <Card title="Session replay" icon="clapperboard">
    Every session is recorded — replay it in the dashboard or via API as HLS video
  </Card>
</CardGroup>
...
{
  "success": true,
  "sessionId": "0198c0e3-1f34-7abc-9def-0123456789ab",
...
Every interact session is recorded automatically. Firecrawl captures what the browser actually renders — pixels, not DOM reconstructions — so the replay shows exactly what happened, including canvas, iframes, and video content. Recordings are delivered as standard HLS video and remain available for 30 days after the session ends.

Watch replays in the dashboard under [Activity Logs](https://firecrawl.dev/app/logs), or fetch them programmatically. Every interact response includes a `sessionId`; use it with the replay endpoints:
...
The metadata response lists each recorded tab (up to 10 per session) with timestamps in **milliseconds from session start** on a shared timeline:
...
```json Response
{
  ...
  "pages": [
    {
      "pageId": "0",
      "url": "/v2/interact/SESSION_ID/replay/0",
...
The playlist works with any HLS-capable player — [hls.js](https://github.com/video-dev/hls.js), Video.js, Shaka Player, or Safari's native playback:
...
<script>
  ...
  const hls = new Hls();
  // Route on your backend that proxies the replay endpoint with your API key.
...
<Warning>
Fetch playlists from your backend, never directly from the browser — the replay endpoints require your API key. Segment URLs inside the playlist are pre-signed and expire after ~6 hours; they're safe to hand to the client, and the player fetches video segments directly from storage.
</Warning>

### Disabling recording

Recording is on by default. For standalone sessions (`POST /v2/interact`), pass `recordSession: false` to skip recording entirely — the replay endpoints will return `404` for that session. Replay playback is included at no extra credit cost.
...
- [Get Session Replay](/api-reference/endpoint/browser-replay) — `GET /v2/interact/{sessionId}/replay`
- [Get Session Replay Playlist](/api-reference/endpoint/browser-replay-page) — `GET /v2/interact/{sessionId}/replay/{pageId}`
...
| `sessionId` | Browser session ID. Use it with the [session replay endpoints](/api-reference/endpoint/browser-replay) to fetch the recording |
