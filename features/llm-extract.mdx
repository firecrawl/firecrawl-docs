...
<Note>
**HTML attributes are not available in JSON mode.** JSON extraction operates on **markdown-converted content**, not raw HTML. During conversion, HTML attributes (e.g., `data-id`, custom attributes on elements) are stripped — only visible text is preserved. If you need to extract HTML attribute values, request `rawHtml` alongside JSON extraction and parse the attributes client-side, or use an [`executeJavascript` action](/advanced-scraping-guide#actions) to inject attribute values into visible text before extraction runs.
</Note>
