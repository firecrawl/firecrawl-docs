...
The On-Demand Search endpoint combines web search with Firecrawl's scraping capabilities to return full page content for any query, just like the regular `/search` endpoint, but with pay-per-use pricing.
