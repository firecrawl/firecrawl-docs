...
- **Tenant isolation** — Your workloads run on isolated infrastructure with dedicated queues, keeping your processing and data separated from other customers.
...
- **API key IP restrictions** — Lock individual API keys to specific IP ranges so keys only work from approved networks.
...
- **Extended credit rollover** — Annual Enterprise plans roll unused plan credits over 2 months. See [Billing](/billing).
