...
- **Description**: Regex patterns to include. By default, these match against the URL **pathname only** (not query parameters). To match against the full URL including query strings, use `regexOnFullURL: true`.
  ...
  **Important:** The starting `url` is also checked against `includePaths`. If the starting URL's path does not match any of the patterns, the crawl may return 0 results because it is filtered out before link discovery begins. When crawling a specific section of a site, set the starting `url` to that section (e.g., `https://example.com/blog`) rather than the domain root.
- **Example**: `["^/blog/.*$", "^/docs/.*$"]`
