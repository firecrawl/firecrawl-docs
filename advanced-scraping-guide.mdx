...
- **Type**: `array`
- **Description**: Control parsing behavior. To parse PDFs, set `parsers: ["pdf"]`. PDF parsing is billed at 1 credit per PDF page. To limit the number of pages parsed, use the object syntax: `parsers: [{"type": "pdf", "maxPages": 10}]`. To skip PDF parsing entirely and receive the raw file in base64 encoding for a flat rate of 1 credit, set `parsers: []`.
