...
- **Type**: `integer`
- **Description**: Max number of pages to crawl. Note that this controls the number of webpages crawled, not credit consumption. Total credits may exceed this value if pages contain PDFs, as PDF parsing is billed at 1 credit per PDF page. To control PDF costs, use the `parsers` option in `scrapeOptions` (see [Document Parsing](/features/document-parsing)).
- **Default**: `10000`
