...
- **Supported actions**:
    - **`wait`** - Pause execution for a specified duration
      - `milliseconds` (required, integer): Number of milliseconds to wait (minimum: 1)
      - `selector` (optional, string): CSS selector for an element to wait for before proceeding
    - **`click`** - Click on an element
      - `selector` (required, string): CSS selector to find the element (e.g., `"#button"`, `".submit-btn"`)
    - **`write`** - Type text into an input field or contenteditable element
      - `selector` (required, string): CSS selector of the input field
      - `text` (required, string): Text to type
      - Note: Must click the element first to focus it before writing
    - **`press`** - Press a keyboard key
      - `key` (required, string): Key code to press (e.g., `"Enter"`, `"Tab"`, `"Escape"`)
    - **`scroll`** - Scroll the page or a specific element
      - `direction` (required, string): Either `"up"` or `"down"`
      - `selector` (optional, string): CSS selector of element to scroll (defaults to page)
    - **`screenshot`** - Take a screenshot of the current page state
      - `fullPage` (optional, boolean): Whether to capture full page (default: false)
      - `quality` (optional, integer): Image quality 1-100 (default varies)
      - `viewport` (optional, object): Set viewport dimensions with `width` and `height` properties
    - **`scrape`** - Scrape a specific sub-element's content
      - `selector` (optional, string): CSS selector of element to scrape
      - Returns HTML content in `actions.scrapes` array
    - **`executeJavascript`** - Execute JavaScript code on the page
      - `script` (required, string): JavaScript code to execute
      - Return values appear in `actions.javascriptReturns` array
    - **`pdf`** - Generate a PDF of the current page
      - Outputs appear in `actions.pdfs` array
...
print(doc.markdown)
if hasattr(doc, 'screenshot'):
    print(f"Screenshot: {doc.screenshot}")
```
...
console.log(doc.markdown);
if (doc.screenshot) {
  console.log(`Screenshot: ${doc.screenshot}`);
}
```
...
- **Execution order**: Actions are executed sequentially in the order specified
- **Waiting for elements**: Use the optional `selector` parameter in the `wait` action to wait for specific elements to appear
- **Focus before typing**: Always click on an input field before using the `write` action
- **Screenshot placement**: Take screenshots after all interactions are complete to capture the final state
- **JavaScript execution**: The `executeJavascript` action can return values which appear in the response
