...
<Note>
The starting URL is also checked against `includePaths`. If it does not match any of the patterns, the crawl will return 0 pages because link discovery never begins. Make sure your starting URL matches at least one `includePaths` pattern, or set the starting URL directly to a path that matches.
</Note>
