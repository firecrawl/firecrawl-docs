...
These parameters control which parts of the page appear in the output. When `onlyMainContent` is `true` (the default), boilerplate elements (nav, footer, etc.) are stripped; `includeTags` and `excludeTags` are also applied to filter the content. Note that `excludeTags` may be applied before the main-content extraction step, so the selectors you provide should match elements in the original page DOM rather than in an already-filtered result. If you set `onlyMainContent: false`, the full page HTML is used as the starting point for tag filtering.
