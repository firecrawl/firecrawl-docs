...
- **Type**: `array`
- **Description**: Control parsing behavior. To parse PDFs, set `parsers: ["pdf"]`. PDFs are billed at 1 credit per page. To limit the number of pages parsed, use the object syntax: `parsers: [{ type: "pdf", maxPages: 100 }]`. Pass an empty array `parsers: []` to skip PDF parsing entirely and receive the raw file in base64 (1 credit flat).
