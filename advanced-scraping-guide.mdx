...
Firecrawl also OCRs raster images. Point `/scrape` at an image URL (PNG, JPEG, JPEG 2000, TIFF, GIF, BMP, WebP or AVIF) and the response is a one-page document: the recognized text as markdown, `metadata.contentType` set to the image's type, and a cost of 1 credit. An image with no readable text returns an empty markdown string.

Image OCR is on by default through the `image` parser, which sits next to `pdf` in the default `parsers` list. To keep the historical behavior, where image URLs fail as unsupported files, send a `parsers` list that omits it:

```json
"parsers": ["pdf"]
```

To get the raw bytes instead of OCR text, request the `rawBase64` format.
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `parsers` | `array` | `["pdf", "image"]` | Controls file processing. `"pdf"` parses PDFs (1 credit per page); `"image"` OCRs raster images (1 credit). Omit an entry to stop parsing that type: `["pdf"]` leaves image URLs failing as unsupported files, `[]` skips parsing and returns a PDF as base64 (1 credit flat). |
...
The image parser takes no options; `"image"` and `{ "type": "image" }` are equivalent. See [Scraping images](#scraping-images).
