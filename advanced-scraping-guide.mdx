...
description: "Configure scrape options, browser actions, crawling, mapping, and the agent endpoint with Firecrawl's full API surface."
og:title: "Advanced Scraping Guide | Firecrawl"
og:description: "Configure scrape options, browser actions, crawling, mapping, and the agent endpoint with Firecrawl's full API surface."
---
...
| `screenshot` | `fullPage?: boolean`, `quality?: number`, `viewport?: { width, height }` | Capture a screenshot. Max one per request. Viewport max resolution is 7680×4320. |
...
| `timeout` | `integer` (ms) | `30000` (30 seconds) | Max request duration before aborting. |

### PDF parsing

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `parsers` | `array` | `["pdf"]` | `["pdf"]` to parse (1 credit/page). `[{"type":"pdf","maxPages":10}]` to cap pages. `[]` to skip parsing and return base64 (1 credit flat). |

### Actions

Run browser actions before scraping — useful for dynamic content, navigation, or user-gated pages. You can include up to 50 actions per request, and the combined wait time across all `wait` actions and `waitFor` must not exceed 60 seconds.
...
| `pdf` | `format?: string`, `landscape?: boolean`, `scale?: number` | Generate a PDF. Supported formats: `"A0"`–`"A6"`, `"Letter"`, `"Legal"`, `"Tabloid"`, `"Ledger"`. Defaults to `"Letter"`. |
...
| `maxCredits` | `number` | `2500` | Maximum credits the agent can spend. |
