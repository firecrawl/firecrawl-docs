...
- **Type**: `array`
- **Description**: Control parsing behavior. To parse PDFs, use `parsers: ["pdf"]` for full PDF parsing or `parsers: [{"type": "pdf", "maxPages": 5}]` to limit parsing to the first 5 pages.
- **Object format**: `{"type": "pdf", "maxPages": number}` where `maxPages` is a positive integer (max 10000)
- **Billing**: 1 credit per page when parsing PDF content vs 1 credit flat rate when `parsers: []` (empty array)
...
- Parse first 5 pages of PDFs via `parsers: [{"type": "pdf", "maxPages": 5}]`.
