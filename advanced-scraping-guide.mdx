...
- **Type**: `boolean`
- **Description**: When `true`, `includePaths` and `excludePaths` regex patterns are matched against the **full URL** (including query parameters) instead of just the URL pathname. This is useful when you need to filter URLs based on query strings.
- **Default**: `false`
- **Example**: To crawl only paginated FAQ pages while excluding category and keyword filter URLs:
  ```json
  {
    "url": "https://example.com/en/faq",
    "includePaths": ["/en/faq"],
    "excludePaths": ["faq_category", "keyword="],
    "regexOnFullURL": true,
    "limit": 20
  }
  ```
