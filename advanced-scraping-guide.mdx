...
| `timeout` | `integer` (ms) | `30000` | Max request duration before aborting (default is 30 seconds). |

### PDF parsing

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `parsers` | `array` | `["pdf"]` | Controls PDF processing. Accepts objects or shorthand strings. `[]` to skip parsing and return base64 (1 credit flat). |
...
**Shorthand strings:** `"pdf"` (defaults to `auto`), `"pdf:fast"`, `"pdf:auto"`, `"pdf:ocr"`.
