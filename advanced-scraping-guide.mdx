...
Long PDFs can outlive the request `timeout`. Firecrawl keeps processing the
document and returns HTTP `408` with `code: "SCRAPE_TIMEOUT"` and
`details.state: "processing_continues"`. Retry the same URL after
`Retry-After` — the retry attaches to the in-flight job instead of starting
over. The JS and Python SDKs auto-resume this on `scrape()`. See
[Large PDFs](/features/parse#large-pdfs).
