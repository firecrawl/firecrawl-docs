...
- **Type**: `boolean`
- **Description**: When `true`, the crawler skips URLs that are structurally similar (e.g. the same path with different query parameters). Set to `false` if you need to crawl pages that differ only by query parameters.
- **Default**: `true`

#### ignoreQueryParameters

- **Type**: `boolean`
- **Description**: When `true`, the crawler treats URLs that share the same path but have different (or no) query parameters as the same page and only scrapes it once.
- **Default**: `false`
