...
The `images` format extracts all images found on a webpage and returns them as an array of resolved URLs. This includes:

- Images from `<img>` tags (including `src`, `data-src`, and `srcset` attributes)
- Images from `<picture>` elements and their `<source>` tags
- Meta tag images (Open Graph, Twitter Cards, etc.)
- Favicon and touch icons from `<link>` tags
- Background images from inline CSS styles
- Video poster images
...
The images are returned as an array of strings containing fully resolved URLs:
```json
{
  "images": [
