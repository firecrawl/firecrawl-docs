...
- **Models Available**: `spark-2` (newest, cheapest and fastest), `spark-1-mini` (default, 60% cheaper than Pro) and `spark-1-pro` (higher accuracy)
...
result = app.agent(
    ...
    schema=schema,
    model="spark-1-mini"  # or "spark-2" for the newest model, "spark-1-pro" for higher accuracy
)
```
