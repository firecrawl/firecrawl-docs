...
Firecrawl caches previously scraped pages and returns a recent copy when one is available, cutting latency and cost. `maxAge` is the maximum age, in milliseconds, of a cached copy that Firecrawl may return instead of retrieving the page again.
...
Keep caching on by default. Pay the `maxAge: 0` latency cost only for the reads where staleness would cause a wrong or costly decision. For caching mechanics, common `maxAge` values, cache-hit matching rules, and the request options that bypass caching automatically, see [Faster Scraping](/features/fast-scraping).
...
If you need a guaranteed-fresh retrieval of a page you found through `/search`, scrape that URL again with `/scrape` and `maxAge: 0`.
