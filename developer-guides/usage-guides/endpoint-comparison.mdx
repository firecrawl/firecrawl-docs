...
|---------|----------|------------|----------------------|
| **Status** | Active (Recommended) | Use `/agent` instead | Active |
...
<Tip>
`/extract` now supports extracting without URLs (in Alpha)—just provide a prompt. However, for this use case, `/agent` delivers better results.
</Tip>
