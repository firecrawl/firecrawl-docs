...
When ChatGPT opens Firecrawl in your browser, sign in, choose the team to use, and approve access. Do not put a Firecrawl API key in the server URL. See the [OAuth guide](/mcp-server/oauth) for the endpoint contract and other supported modes.
...
<Tip>
**No confirmation prompt appearing?** If ChatGPT answers your question without showing a confirmation dialog, the Firecrawl MCP connector is most likely not attached to the conversation. Go back to [Step 4](#4-access-firecrawl-tools) and make sure you click the **+** button, select **More**, and choose **Firecrawl MCP** before sending your prompt. The connector must be attached to each new conversation.
</Tip>
