---
title: "Connect MCP Clients with OAuth"
description: "Authenticate MCP clients with Firecrawl using OAuth instead of embedding your API key in the URL"
---
...
Any client that implements the MCP authorization spec can use the keyless endpoint. In each case, point the client at `https://mcp.firecrawl.dev/v2/mcp` and complete the browser sign-in when prompted.

<Note>
These snippets assume your client supports remote MCP OAuth. For clients that only accept an API key, use the API-key-in-URL form (`https://mcp.firecrawl.dev/YOUR_API_KEY/v2/mcp`) instead.
</Note>
...
<Tabs>
  <Tab title="Claude Code">
    ...
    The first time you use the server, Claude Code opens a browser window to complete the OAuth flow. Run `claude`, type `/mcp`, select **firecrawl**, and finish authorizing. Add `--scope user` to make the server available across all your projects.
  </Tab>
  <Tab title="Cursor">
    Add Firecrawl to `~/.cursor/mcp.json`:
    ...
    Cursor prompts you to sign in and authorize on first use.
  </Tab>
  <Tab title="VS Code">
    Add Firecrawl to `.vscode/mcp.json`:
  ...
  <Tab title="mcp-remote fallback">
    For clients that cannot connect to a remote HTTP server directly, wrap it with [`mcp-remote`](https://www.npmjs.com/package/mcp-remote):
    ...
    `mcp-remote` handles the OAuth flow and opens your browser to authorize.
  </Tab>
</Tabs>
