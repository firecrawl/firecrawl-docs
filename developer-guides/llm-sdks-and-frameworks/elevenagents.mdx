...
Keyless MCP exposes exactly Search, Scrape, and Parse, with shared limits. Use [OAuth](/mcp-server/oauth) or an [API key](/mcp-server/keyless-api-key#add-an-api-key) for the broader tool surface. Keep API keys in an environment variable or secret store, never in a URL.
