...
Give your [ElevenAgents](https://elevenlabs.io/agents) voice and chat agents the ability to search, scrape, and parse web content in real time using Firecrawl. This guide covers two integration paths:
...
- A Firecrawl API key from your [firecrawl.dev dashboard](https://firecrawl.dev) if you use the webhook path or need the full MCP tool set
...
| --- | --- |
| **Name** | Firecrawl |
| **Description** | Search, scrape, and parse web content. |
| **Server type** | Streamable HTTP |
| **Server URL** | `https://mcp.firecrawl.dev/v2/mcp` |

This keyless connection exposes Search, Scrape, and Parse. If the platform supports custom MCP headers, use `Authorization: Bearer fc-YOUR_API_KEY` when you need the full tool set.
...
ElevenLabs will connect to the keyless server and list exactly Search, Scrape, and Parse. Add an API key header before expecting account-gated tools.
