...
Keyless MCP exposes exactly Search, Scrape, and Parse, with shared limits. Use [OAuth](/mcp-server/human-mcp) or an [API key](/mcp-server/agent-mcp#add-an-api-key) for the broader tool surface. Keep API keys in an environment variable or secret store, never in a URL.
