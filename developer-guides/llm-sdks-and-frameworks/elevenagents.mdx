...
This URL starts the keyless Firecrawl toolset. For browser sign-in, use [OAuth](/mcp-server/human-mcp). For an unattended setup, use the [API-key guide](/mcp-server/agent-mcp#add-an-api-key). Keep API keys in an environment variable or secret store, never in a URL.
