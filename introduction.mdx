...
<Card title="Try instantly with keyless MCP" icon="bolt" href="/mcp-server/agent-mcp#try-keyless">
  No account or key. Use Search, Scrape, and Parse with shared limits. **Start keyless setup →**
</Card>

<Card title="Connect with OAuth" icon="right-to-bracket" href="/mcp-server/human-mcp">
  Sign in from an interactive MCP client and connect your Firecrawl team. **Connect with OAuth →**
</Card>

<Card title="Run with an API key" icon="key" href="/mcp-server/agent-mcp#add-an-api-key">
  Configure stable access for agents, CI, and backends without putting the secret in chat or source control. **Configure an API key →**
</Card>
