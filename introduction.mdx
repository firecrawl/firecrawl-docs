...
<Note>
  Restart your coding agent after setup so it can discover the new skills.
  `--skip-auth`, `--skip-install`, `--skip-skills`, and `--agent <name>` are
  also available. See `firecrawl init --help` and [CLI](/sdks/cli) for the
  full setup.
</Note>
