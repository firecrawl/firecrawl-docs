...
<div id="crawling-a-website">
  ### Web サイトをクロールする {#crawling-a-website}
</div>
...
### WebSockets を使って Web サイトをクロールする {#crawling-a-website-with-websockets}
...
<div id="simple-crawl-auto-pagination-default">
  ##### シンプルなクロール (自動ページ送り、デフォルト) {#simple-crawl-auto-pagination-default} 
</div>

* 既定のフローは[ウェブサイトのクロール](#crawling-a-website)を参照してください。

<div id="manual-crawl-with-pagination-control-single-page">
  ##### ページネーション制御付きの手動クロール (単一ページ) {#manual-crawl-with-pagination-control-single-page} 
</div>
...
<div id="manual-crawl-with-limits-auto-pagination-early-stop">
  ##### 制限付きの手動クロール (自動ページネーション + 早期停止) {#manual-crawl-with-limits-auto-pagination-early-stop} 
</div>
...
<div id="manual-batch-scrape-with-pagination-control-single-page">
  ##### ページネーション制御による手動バッチスクレイプ (単一ページ) {#manual-batch-scrape-with-pagination-control-single-page} 
</div>
...
### スクレイピングに紐づくインタラクティブセッション {#scrape-bound-interactive-session}
