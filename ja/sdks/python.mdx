...
Firecrawl の /crawl および batch の各エンドポイントは、追加のデータがある場合に `next` URL を返します。Python SDK はデフォルトで自動ページネーションを行い、すべてのドキュメントを集約します。この場合、`next` は `None` になります。自動ページネーションを無効化したり、上限を設定することも可能です。`next` URL は不透明な値なので、そのまま SDK に渡して次のページを取得します。
...
* ジョブを開始し、`auto_paginate=False` を指定し、`get_crawl_status_page` を使用して1ページずつ取得します。
...
if status.next:
    ...
    print("次ページのドキュメント数:", len(next_page.data), "次:", next_page.next)
```
...
* ジョブを開始し、`auto_paginate=False` を指定し、`get_batch_scrape_status_page` を使って1ページずつ取得します。
