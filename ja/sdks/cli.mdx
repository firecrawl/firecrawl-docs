...
<Note>
  **`scrape`、`search`、`interact` はログインなしで利用できます。** APIキーが設定されていない場合、これらのコマンドはAPIキー不要のFreeティアに自動的に切り替わります。無料で使えますが、IPごとにレート制限があります。その他のすべてのコマンド (crawl、map、agent など) には APIキーが必要です。1,000 クレジットとより高い上限を利用するには、[無料のキーに登録](https://firecrawl.dev)してください。設定後は、CLI が自動的にそのキーを使用します。詳しくは[Rate Limits](/ja/rate-limits#keyless-no-api-key)を参照してください。
</Note>
