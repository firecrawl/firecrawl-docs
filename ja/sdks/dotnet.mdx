...
公式の .NET SDK は、Firecrawl のモノレポ内の [apps/.net-sdk](https://github.com/firecrawl/firecrawl/tree/main/apps/.net-sdk) で管理されています。

Firecrawl .NET SDK をインストールするには、NuGet パッケージを追加します。

<Tabs>
  <Tab title=".NET CLI">`bash dotnet add package firecrawl-sdk `</Tab>
  <Tab title="パッケージ マネージャー">`powershell Install-Package firecrawl-sdk `</Tab>
