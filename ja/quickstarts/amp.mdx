...
キーレスのアクセスには、同じパブリックIPを使用するユーザー間で共有される制限があります。対話型のアカウントアクセスには[OAuth](/ja/mcp-server/oauth)を、ユーザー操作なしでの利用には[APIキーガイド](/ja/mcp-server/keyless-api-key#add-an-api-key)を使用してください。
