...
この構成では、キー不要のFirecrawlツールセットを使用します。アカウント接続のセットアップについては[Human MCP](/ja/mcp-server/human-mcp)を、無人運用のセットアップについては[Agent MCP](/ja/mcp-server/agent-mcp)を参照してください。APIキーはURLには含めず、Authorizationヘッダーまたは安全なシークレットストアに保管してください。
