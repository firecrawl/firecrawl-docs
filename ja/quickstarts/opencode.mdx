...
OpenCode は `~/.config/opencode/opencode.json` (グローバル) またはプロジェクト内の `./opencode.json` から設定を読み込みます。以下を追加します:
...
この構成では、キー不要のFirecrawlツールセットを使用します。アカウント接続型セットアップについては[人間MCP](/ja/mcp-server/human-mcp)を、無人セットアップについては[Agent MCP](/ja/mcp-server/agent-mcp)を参照してください。APIキーはURLではなく、Authorizationヘッダーまたは安全なシークレットストアに保管してください。
