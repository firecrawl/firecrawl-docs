...
キーなしアクセスには、同じパブリックIPを使用するユーザー間で共有される制限があります。対話型でアカウントにアクセスする場合は[OAuth](/ja/mcp-server/oauth)、ユーザー操作なしで利用する場合は[APIキーガイド](/ja/mcp-server/keyless-api-key#add-an-api-key)を使用してください。
