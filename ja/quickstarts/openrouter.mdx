...
* 主要な OpenRouter アプリ (Cline、Roo Code、Kilo、Cursor、Continue) の多くは、Firecrawl MCP を利用できるエージェント実行基盤でもあります。Firecrawl をそれらに直接接続する方法については、[MCP Server](/ja/mcp-server) を参照してください。
* 大規模なジョブでは、LLM のコンテキスト上限内に収めるために [バッチスクレイプ](/ja/features/batch-scrape) を使用してください。