...
この構成では、キーなしで利用できるFirecrawlツールセットを使用します。アカウント接続型のセットアップについては[Human MCP](/ja/mcp-server/human-mcp)、ユーザー操作なしのセットアップについては[Agent MCP](/ja/mcp-server/agent-mcp)を参照してください。APIキーはURLには絶対に含めず、Authorizationヘッダーまたは安全なシークレットストアに保管してください。
