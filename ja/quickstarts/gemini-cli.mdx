...
この構成では、キーなしで利用できるFirecrawlツールセットを使用します。アカウント接続型または無人セットアップの場合は、[ホスト型MCP接続モード](/ja/developer-guides/mcp-setup-guides/oauth)に従ってください。APIキーはURLには絶対に含めず、Authorizationヘッダーまたは安全なシークレットストアに保管してください。
