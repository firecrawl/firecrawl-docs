...
キーレスアクセスには、同じパブリック IP を共有するユーザー間で共通の制限が適用されます。対話型のアカウントアクセスには [OAuth](/ja/mcp-server/oauth) を、ユーザー操作なしで利用する場合は [API キーガイド](/ja/mcp-server/keyless-api-key#add-an-api-key) を使用してください。
