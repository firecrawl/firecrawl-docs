...
この構成では、キーなしで利用できるFirecrawlツールセットを使用します。アカウントに接続するセットアップや無人セットアップについては、[ホスト型MCP接続モード](/ja/developer-guides/mcp-setup-guides/oauth)に従ってください。APIキーはURLには含めず、Authorizationヘッダーまたは安全なシークレットストアに保管してください。
