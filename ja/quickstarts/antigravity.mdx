...
この構成では、キーなしで利用できるFirecrawlツールセットを使用します。アカウントに接続するセットアップについては[Human MCP](/ja/mcp-server/human-mcp)、無人セットアップについては[Agent MCP](/ja/mcp-server/agent-mcp)を参照してください。APIキーはURLには含めず、Authorizationヘッダーまたは安全なシークレットストアに保管してください。
