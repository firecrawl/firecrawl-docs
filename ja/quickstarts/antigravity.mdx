...
キーレスアクセスには、同じパブリック IP を使用するユーザー間で共有される制限があります。対話型でアカウントにアクセスするには [OAuth](/ja/mcp-server/oauth) を、ユーザー操作なしで利用するには [API キーガイド](/ja/mcp-server/keyless-api-key#add-an-api-key) を使用してください。
