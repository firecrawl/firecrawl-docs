...
* **Gateway**: インテグレーションで作成したアカウントはGatewayに登録され、ユーザー自身のcreditsを使い切った後の対象利用分は自社の組織に課金されます。GatewayのアカウントはAPI専用で、独自のdashboardログインはありません。

セットアップはインテグレーション作成時に選択します。後からGatewayを追加・削除する場合は、Firecrawlへのお問い合わせが必要です。概要については[Partner APIページ](https://www.firecrawl.dev/partner-program)をご覧ください。
...
[Firecrawl ダッシュボード](https://www.firecrawl.dev/app/partner-api)でインテグレーションを作成します。サインインし、Partner API または Gateway を選択して規約に同意し、パートナーキーをコピーしてください。あとは、上記のエンドポイントをサーバーから呼び出すだけです。ご質問がある場合や、より大規模な連携をご検討中の場合は、[partnerships@firecrawl.dev](mailto:partnerships@firecrawl.dev) までご連絡ください。