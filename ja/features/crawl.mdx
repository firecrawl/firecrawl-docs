---
title: 'クロール'
description: 'ウェブサイトを再帰的にクロールし、各ページからコンテンツを抽出します'
og:title: 'クロール | Firecrawl'
og:description: 'ウェブサイトを再帰的にクロールし、各ページからコンテンツを抽出します'
icon: 'spider'
---
...
[Scrape エンドポイント](/ja/api-reference/endpoint/scrape) のすべてのオプションは、`scrapeOptions` (JS) / `scrape_options` (Python) 経由で crawl でも利用できます。これらはクローラーがスクレイプするすべてのページに適用されます (フォーマット、プロキシ、キャッシュ、アクション、ロケーション、タグを含む) 。
