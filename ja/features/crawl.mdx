...
<div id="start-and-check-later">
  ### 開始して後で確認 {#start-and-check-later}
</div>
...
<div id="real-time-results-with-websocket">
  ## WebSocket によるリアルタイム結果 {#real-time-results-with-websocket}
</div>
...
<div id="verifying-webhook-signatures">
  ### Webhook シグネチャの検証 {#verifying-webhook-signatures}
</div>
...
## 実行と結果の集計 {#execution-and-result-accounting}
...
### ステータスカウンターの読み方 {#reading-the-status-counters}
...
### 結果のページング {#paging-through-results}
...
### 失敗したページとブロックされたページ {#failed-and-blocked-pages}
...
### クローラーが到達できる範囲 {#what-the-crawler-is-scoped-to-reach}
...
### 実行ごとに結果が異なる場合 {#when-results-vary-between-runs}
...
### クロールの完了を知る方法 {#knowing-when-a-crawl-is-done}
