...
| `maxResults`     | `number`                                         | 各チェックで評価する結果の合計数です。`1`～`50` を指定できます。デフォルトは `10` です。これは各 `queries` 全体での合計上限であり (結果は先にマージ・重複排除されます) 、クエリごとの上限ではありません。そのため、他のクエリで先に上限に達すると、個々のクエリの結果数は少なくなるか、0 件になることがあります。 |
...
<Note>
  **クレジットはクエリ数に応じて増加します。** 各クエリは最大 `maxResults` 件の結果を取得し、検索呼び出しのクレジットは、マージ・重複排除*前*の全クエリの生の結果数に基づいて課金されます。つまり、クエリを追加すると、事前見積もりだけでなく実際の検索コストも増加します。マージ・重複排除後に Firecrawl が評価する候補は最大 `maxResults` 件までのため、judge のクレジットは選択・判定された結果数の範囲内に収まります。
</Note>
...
Webスケールのモニター の品質は、役割の異なる2つの要素で決まります。

* **`queries` は再現率を左右します** — 各検索で何を拾うかです。狭すぎると本当に重要な出来事が上がってこなくなり、広すぎると judge がノイズの絞り込みに credits を使うことになります。
* **`goal` は適合率を左右します** — 取得した結果のうち、実際にどれを alert にするかです。judge は新しい結果を1件ずつゴールに照らして評価するため、本当に一致しているものと、話題には沿っているが無関係なものを分けるのはゴールです。

両方を調整してください。どれだけ完璧なゴールでも、クエリが取得できなかった結果に対して alert を出すことはできません。逆に、広いクエリに曖昧なゴールを組み合わせると、価値の低い alert が絶えず発生します。

**良いクエリは、文章ではなく検索語のように読めるものです。**
...
* 各クエリは簡潔に保ちます (およそ2〜6語) 。通常は、狭いクエリをいくつも作るより、少し広めのクエリを1つ使うほうが効果的です — クエリを増やしてもカバレッジは増えず、`maxResults` の枠が分散されるだけです。
* *明確に異なる*主題ごとに1つのクエリを使います。1つの主題に複数の側面 (「launches、benchmarks、docs」) があっても、それは1つのクエリです。ゴールが本当に別々の対象を挙げている場合にだけ分けてください (たとえば「OpenAI、Anthropic、Google」) 。
* `site:` / `-site:` 演算子はクエリに入れず、代わりに `includeDomains` / `excludeDomains` を使ってください。

**良いゴールは、一致条件を平易な言葉で述べ、除外条件は意図に含まれる場合にだけ追加します。**

* 対象と、一致と見なす条件を明示します: *「OpenAI がまったく新しいモデルを発表したら alert する。」*
* 多義的な用語は明確にします: *「Firecrawl (web scraping API) 」* としておくと、judge が消防機器の話題に引っ張られません。
* `Ignore ...` は、意図に特有の非一致だけに追加します: *「意見記事、チュートリアル、無関係な AI ニュースは無視する。」* 一般的なノイズまで書き直して繰り返す必要はありません — judge は書式、追跡パラメータ、再インデックスによる揺れをすでに処理します。
* 意図が広いなら、そのまま広く保ちます。狭めすぎたゴールは、本来一致すべきものまで抑えてしまいます。

**健全な モニター はどのような状態か。** 適切に調整されたWebスケールのモニター は、ほとんどの場合何も報告しません — ほとんどの check は `new: 0` を返します — そして、本当に新しく、かつゴールに合うものが現れたときだけ alert します。調整がうまくできているかどうかは、check の summary と各結果の `searchStatus` ([ステータスと重複排除](#statuses-and-dedup) を参照) を見れば判断できます。

* `ignored` の結果が安定して出続ける場合、クエリがノイズを拾い、それをゴールが後から弾いていることを意味します — alert されることのない結果を judge するために credits を払い続けないよう、クエリを絞るか `searchWindow` を狭めてください。
* `watching` が頻繁に出る場合、ゴールが曖昧です — 一致条件をより明確にして、judge が判断を確定できるようにしてください。
* 話題が活発なのに結果ゼロの状態が長く続く場合、クエリが狭すぎるか `searchWindow` が短すぎます — 語を広げるか、ウィンドウを広げてください。
* ユーザーが dismiss する alert が出る場合、ゴールが広すぎます — 意図に特化した `Ignore ...` を追加してください。

目指すべき状態は、十分な再現率を保ちながら高い適合率を実現することです。すべての alert が対応する価値を持ち、本当に重要なものを見逃さない状態です。

<div id="judging">
  ## 判定
</div>

各チェックが結果ごとにどこまで処理を行うかは、モニター の `judgeEnabled` で制御されます。これは、[ゴールと判定](/ja/features/monitoring#goals-and-judging) で説明されているものと同じフラグです。判定を有効にすると、Firecrawl は一致した各結果をスクレイピングし、その内容をゴールに照らして評価します。この場合、search 呼び出しに加えて、判定対象の結果 1 件ごとに 1 credit が課金されます。`judgeEnabled: false` の場合、Webスケールのモニター は AI による判定を行わず、重複を除いた 検索結果 を返します。つまり、新しい SERP ヒットのみを返し、課金されるのは search-call credits のみです (10 件ごとに 2 credits) 。
...
| -------------- | ------------- | --------------------------------------------------------- |
| `alert`        | `new`         | judgeが意味のあると判断した新しい結果 — 通知を送信します。                         |
