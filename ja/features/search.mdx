...
検索結果をスクレイプしない場合（スクレイプオプションを指定しない場合）、コストは検索結果10件ごとに2クレジットです。スクレイピングを有効にした場合、標準のスクレイピングコスト以外に、各検索結果の基本的なスクレイプについて追加料金は発生しません。

ただし、次のコスト要因に注意してください:

- **PDF解析**: PDFページ1枚あたり1クレジット（複数ページのPDFではコストが大幅に増加する可能性があります）
- **ステルスプロキシモード**: 検索結果1件あたり+4クレジットの追加
- ***JSONモード***: 検索結果1件あたり+4クレジットの追加

コストを抑えるには:

- PDFコンテンツが不要な場合は `parsers: []` を設定する
- 可能であれば `"stealth"` の代わりに `proxy: "basic"` を使用する
