...
* `github`: GitHub リポジトリ、コード、Issue、ドキュメントを検索
...
<Note>
  **`research` は論文インデックスではなく、Web サイトのフィルターです。** 通常の Web 検索を固定の学術ドメイン一覧に限定し、それらのページのスニペットを返します。
  ...
  PubMed、bioRxiv、medRxiv、arXiv の論文インデックスで、完全な要旨、論文内のパッセージ、citation グラフの拡張を含む科学文献を検索するには、[Research Index](/ja/features/research) を使用してください。
</Note>
...
開発者向けの結果は標準の `web` グループ内に返され、各結果には `category: "developer"` が含まれます。`developer` カテゴリは他のカテゴリと併用できません。ランク付け済み結果や一致した本文箇所、リポジトリおよびドキュメントソースでの絞り込みには、[開発者検索エンドポイント](/ja/features/developer#search-the-developer-index)を使用してください。
