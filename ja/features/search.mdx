...
* `pdf`: PDF を検索
* `developer`: [Developer Index](/ja/features/developer) を検索 — GitHub Issue、マージ済みプルリクエスト、リポジトリの README、厳選されたドキュメントサイト
...
開発者向けの結果は、`web` とは別の `developer` グループで返され、各結果には `category: "developer"` が含まれます。ランク付け済み結果や一致した本文箇所、リポジトリおよびドキュメントソースでの絞り込みには、[開発者検索エンドポイント](/ja/features/developer#developer-search-endpoint)を使用してください。
