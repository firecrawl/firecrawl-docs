...
`/search` の `categories` 配列に `developer` のみを指定します。開発者向けの結果は標準の `web` グループ内に `category: "developer"` 付きで返されます。`developer` カテゴリは他のカテゴリと併用できません。
...
開発者向けの結果には、web結果と同じ形式の `url`、`title`、`description`、`position` に加え、`category: "developer"` が含まれます。SDK ユーザーは `result.web` から取得します。
