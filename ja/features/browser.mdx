...
デフォルトでは、各ブラウザセッションは常にまっさらな状態から始まります。`persistentSession` を使うと、cookie、localStorage、その他のブラウザ状態を保存し、今後のセッションに引き継ぐことができます。ログイン状態の維持、設定の保持、複数ステップのワークフローの再開などに役立ちます。
...
| `writeMode` | `"readwrite"` | `"readwrite"` は変更内容を書き戻します。`"readonly"` は既存データを読み込み専用で使用し、書き込みません — 複数の読み取り専用セッションを同時に扱いたい場合に便利です。 |

<Note>
  一度に 1 つのセッションだけがプロファイルへ書き込みできます。ほかのセッションがすでに書き込み中の場合、`409` エラーが返されます。同じプロファイルを `"readonly"` モードで開くか、時間をおいて再試行してください。
</Note>
