...
<Info>
  ブラウザサンドボックス は現在 [Interact](/ja/features/interact) となっており、同じ機能に加えて、プロンプトで制御することもできます。
</Info>

Firecrawl ブラウザサンドボックス は、エージェントが Web とやり取りできる安全なブラウザ環境を提供します。フォームへの入力、ボタンのクリック、認証などを実行できます。
...
同時に利用できるブラウザセッションの数は、チームのプランに基づく同時実行制限によって決まります。
