...
料金はセッションの操作方法によって異なります。セッションで `prompt` を使用する場合はブラウザ 1 分あたり 7 credits、使用しない場合 (Playwright の `code` のみ) はブラウザ 1 分あたり 2 credits です。課金はブラウザ利用時間 1 分単位で、最低利用時間は 1 分です。Free ユーザーは 5 時間分を無料で利用できます。
