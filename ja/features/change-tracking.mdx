...
if 'diff' in result.change_tracking:
    print(result.change_tracking.diff.text)  # Git 形式の差分テキスト
    print(result.change_tracking.diff.json)  # 構造化された差分データ

# JSON 比較の差分にアクセス
if 'json' in result.change_tracking:
    print(result.change_tracking.json.title.previous)  # 変更前のタイトル
    print(result.change_tracking.json.title.current)  # 変更後のタイトル
```
...
`json`モードは、コンテンツから抽出した特定フィールドの差分を構造化して比較します。コンテンツ全体ではなく、特定のデータポイントの変更を追跡するのに役立ちます。
