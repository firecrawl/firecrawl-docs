...
Firecrawl Agent には 3 つのモデルがあります。**Spark 1 Pro** がデフォルトです。**Spark 2** は Spark 1 のいずれのモデルよりも低コストかつ高速で、精度も同等です。使用するには、`model` を `spark-2` に設定します。

| モデル            | コスト                 | 精度     | 最適な用途                 |
| -------------- | ------------------- | ------ | --------------------- |
| `spark-2`      | 最低                  | Pro 相当 | 低コストで短い実行時間           |
| `spark-1-mini` | Pro より **60% 低コスト** | 標準     | コスト重視の Spark 1 ワークロード |
| `spark-1-pro`  | 標準                  | 高い     | 複雑な調査、重要な抽出 (デフォルト)   |

<Tip>
  **Spark 1 Pro がデフォルト**のため、`model` パラメータを指定しないリクエストでは `spark-1-pro` が使用されます。同等の精度で低コストかつ短い実行時間にするには、`model` を `spark-2` に設定します。単純な Spark 1 タスクのコストを 60% 削減するには、`model` を `spark-1-mini` に設定します。
</Tip>
...
`spark-2` は最新のエージェントモデルです。従来は Mini と Pro の選択が必要だった幅広いタスクに対応するため、精度とコストのトレードオフを考える必要はありません。使用するには `model` を `spark-2` に設定してください。

**次の場合は Spark 2 を使用してください。**

* 実行あたりのコストを最小限に抑えたい
* 実行時間が重要
* Spark 1 Pro の精度が必要になる場合
...
* 単純なデータポイント (連絡先情報、価格情報など) を抽出するとき
...
`spark-1-pro` は、複雑な抽出タスクで最大限の精度を発揮するよう設計された、当社のフラッグシップモデルです。デフォルトモデルのため、`model` を設定しないリクエストでは `spark-1-pro` が使用されます。
...
`spark-1-pro` がデフォルトのため、`model` パラメータは任意です。別のモデルを使用する場合は、`model` パラメータで指定します。
...
| `model`      | string | No       | 使用するモデル: `spark-1-pro` (デフォルト)、`spark-1-mini`、または `spark-2`                                                                                                                                                                                                                                                                                            |
