...
`/agent` エンドポイントは、[`/extract`](/ja/features/extract) エンドポイントの次世代版です。AI エージェントを用いて、ウェブ全体からデータをインテリジェントに収集・構造化します。`/extract` と異なり、Agent ではプロンプトだけを指定すればよく、URL の指定は任意のため、非常に柔軟にあらゆる種類のクエリに対応できます。

<Info>Agent は `/extract` の後継であり、より信頼性が高く、高速で、コストも低く抑えられています。</Info>`

<div id="why-agent-over-extract">
  ## なぜ Extract ではなく Agent なのか？
</div>

Agent は `/extract` の優れた点をすべて引き継ぎつつ、さらに拡張した機能を提供します：

* **プロンプトだけで利用可能**: 必要な内容を説明するだけで OK — URL の指定は不要です
* **よりスマート**: AI エージェントがコンテキストを理解し、自動で関連データを見つけます
* **より高速**: 最適化された処理により、結果を素早く返します
* **より低コスト**: リソースを効率的に使うことでコストを削減します
* **より柔軟**: 幅広いクエリやユースケースに対応します

<div id="using-agent">
  ## `/agent` の使い方
</div>

必須パラメータは `prompt` のみです。どのようなデータを抽出したいのかを記述してください。
...
構造化された出力を得るには、JSON スキーマを指定してください。各 SDK は、型安全なスキーマ定義のために Pydantic（Python）と Zod（Node）をサポートしています：
