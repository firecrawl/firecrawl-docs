...
## URL を指定する場合 (任意) {#providing-urls-optional} 
...
## ジョブのステータスと完了 {#job-status-and-completion}
...
## エージェント実行の一覧取得 {#listing-agent-runs}
...
## 実行中の 実行 を追跡する {#following-a-run-in-progress}
...
## 実行トレースとスナップショット {#execution-traces-and-snapshots}
...
## agent のソースデータを取得する {#getting-the-agents-source-data}
...
## エージェントの実行を共有する {#share-agent-runs}
...
## Agent と Extract：何が改善されたか {#agent-vs-extract-whats-improved}
...
## Agent Playground での CSV アップロード {#csv-upload-in-agent-playground}
...
## Ask を使ったトラブルシューティング {#troubleshooting-with-ask}
...
### Agentの料金の仕組み {#how-agent-pricing-works}
...
### Parallel Agents の料金 {#parallel-agents-pricing}
