...
Firecrawl の `/agent` は、検索・ナビゲーション・スクレイピングを自動で行い、非常に複雑な Web サイトからでも、通常はアクセスしづらい場所のデータやインターネット上のあらゆる場所にあるデータを見つけ出す魔法のような API です。人間なら何時間もかかる作業を数分でこなし、従来の Web スクレイピングを過去のものにします。
...
1日に5回まで無料で実行できます。Research Preview期間中の料金は動的に変動し、`/agent` が使用するトークン数に基づいてクレジットへ換算されます。ぜひ[こちら](/ja/app/agent)からお試しください。