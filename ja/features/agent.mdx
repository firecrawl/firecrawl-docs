...
Firecrawl Agent には 3 つのモデルがあります。**Spark 2** はデフォルトのモデルで、Spark 1 のどちらのモデルよりも低コストかつ高速でありながら、精度は同等です。Spark 1 の両モデルも変更なく引き続き利用できます。

| モデル            | コスト                 | 精度     | 最適な用途                 |
| -------------- | ------------------- | ------ | --------------------- |
| `spark-2`      | 最低                  | Pro 相当 | すべてのタスク (デフォルト)       |
| `spark-1-mini` | Pro より **60% 低コスト** | 標準     | コスト重視の Spark 1 ワークロード |
| `spark-1-pro`  | 標準                  | 高い     | 複雑な調査、重要な抽出           |

<Tip>
  **Spark 2 がデフォルト**のため、`model` パラメータは省略できます。ほとんどのタスクで Spark 1 の両モデルよりも低コストかつ高速です。Spark 1 モデルも引き続き利用できます。これらを使用する場合は、まず Mini を試し、複雑な複数ドメインにわたる調査が必要な場合や精度が重要な場合にのみ Pro に切り替えてください。
</Tip>
...
`spark-2` は最新のエージェントモデルで、すべてのリクエストでデフォルトとして使用されます。従来は Mini と Pro の選択が必要だった幅広いタスクに対応するため、精度とコストのトレードオフを考える必要はありません。
...
`spark-2` がデフォルトのため、`model` パラメータは任意です。別のモデルを使用する場合は、`model` パラメータで指定します。
...
| `model`      | string | No       | 使用するモデル: `spark-2` (デフォルト)、`spark-1-mini`、または `spark-1-pro`                                                                                                                                                                                                                                                                                            |
