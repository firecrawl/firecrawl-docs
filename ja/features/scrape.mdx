...
<div id="scraping-a-url-with-firecrawl">
  ## FirecrawlでURLをスクレイピングする {#scraping-a-url-with-firecrawl}
</div>

<div id="scrape-endpoint">
  ### /scrape エンドポイント {#scrape-endpoint}
</div>
...
### レスポンスのメタデータとステータスコード {#response-metadata-and-status-codes}
...
<div id="extract-structured-data">
  ## 構造化データの抽出 {#extract-structured-data}
</div>

### /scrape (json あり) エンドポイント {#scrape-with-json-endpoint}
...
<div id="extracting-without-schema">
  ### スキーマなしでの抽出 {#extracting-without-schema}
</div>
...
<div id="json-format-options">
  ### JSON フォーマットのオプション {#json-format-options}
</div>
...
<div id="extract-brand-identity">
  ## ブランドアイデンティティの抽出 {#extract-brand-identity}
</div>

<div id="scrape-with-branding-endpoint">
  ### /scrape (ブランディング付き) エンドポイント {#scrape-with-branding-endpoint}
</div>
...
<div id="branding-profile-structure">
  ### ブランディングプロファイルの構造 {#branding-profile-structure}
</div>
...
<div id="combining-with-other-formats">
  ### 他のフォーマットとの併用 {#combining-with-other-formats}
</div>
...
### /scrape (product付き) エンドポイント {#scrape-with-product-endpoint}
...
### Product オブジェクトの構造 {#product-object-structure}
...
### 商品抽出の仕組み {#how-product-extraction-works}
...
### 他のフォーマットと組み合わせる {#combining-with-other-formats-2}
...
<div id="highlights-format">
  ## Highlights フォーマット {#highlights-format}
</div>
...
<div id="interacting-with-the-page-with-actions">
  ## アクションを使ってページとやり取りする {#interacting-with-the-page-with-actions}
</div>
...
<div id="location-and-language">
  ## ロケーションと言語 {#location-and-language}
</div>
...
## キャッシュと maxAge {#caching-and-maxage}
...
### 鮮度と項目のステータス {#freshness-and-item-status}
...
<div id="batch-scraping-multiple-urls">
  ## 複数のURLのバッチスクレイピング {#batch-scraping-multiple-urls}
</div>
