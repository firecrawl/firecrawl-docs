...
import ScrapeQueryPython from "/snippets/ja/v2/scrape/query/python.mdx";
import ScrapeQueryNode from "/snippets/ja/v2/scrape/query/js.mdx";
import ScrapeQueryCURL from "/snippets/ja/v2/scrape/query/curl.mdx";
...
<Info>
  各スクレイプで1クレジットを消費します。次のオプションには追加クレジットが必要です。JSONモードは1ページあたり追加で4クレジット、queryフォーマットは1ページあたり追加で4クレジット、強化プロキシは1ページあたり追加で4クレジット、PDF解析はPDFの1ページあたり1クレジットを消費し、音声抽出は1ページあたり追加で4クレジットかかります。
</Info>
...
`query` フォーマットを使うと、ページについて自然言語で質問できます。Firecrawl はページをスクレイピングし、その内容に対して LLM を実行して、レスポンスの `answer` フィールドに回答を返します。JSONスキーマを書かずに、URL に対して単発の Q&amp;A を行いたい場合に便利です。

<Info>
  `query` フォーマットのコストは 1 ページあたり 5 クレジットです (基本 1 + LLM 呼び出しの追加 4) 。
</Info>

format object 内の options:

* `prompt` (必須) : 回答する質問です。最大 10,000 文字。
* `mode` (任意) : `"freeform"` はページの内容を使って質問に回答します。`"directQuote"` はページ内の関連する元テキストを返します。デフォルトは `"freeform"` です。

`query` は他のフォーマットと組み合わせることもできます。たとえば、`markdown` と `query` を一緒にリクエストすると、ページ内容と回答の両方を 1 回の呼び出しで取得できます。
...
`query` フォーマットは `/search` でも `scrapeOptions` を通じて利用でき、各検索結果に対して同じ Q&amp;A を実行します。
