...
## スクレイピングのオプション {#scrape-options}

`/scrape` エンドポイントを使用する場合、以下のオプションでリクエストをカスタマイズできます。

### フォーマット (`formats`) {#formats-formats}
...
<div id="action-execution-notes">
  #### アクション実行時の注意点 {#action-execution-notes}
</div>
...
<div id="advanced-action-examples">
  #### 高度なアクション例 {#advanced-action-examples}
</div>
...
## `formats` を使った JSON 抽出 {#json-extraction-via-formats}
...
<div id="check-agent-status">
  ### エージェントのステータスを確認する {#check-agent-status}
</div>
...
### クロール用`prompt`とパラメータのプレビュー {#crawl-prompt-and-params-preview}
...
<div id="sitemap-and-deduplication">
  #### サイトマップと重複排除 {#sitemap-and-deduplication}
</div>
...
<div id="scrape-options-for-crawl">
  #### クローリング時のスクレイプオプション {#scrape-options-for-crawl}
</div>
...
## Web サイトリンクのマッピング {#mapping-website-links}
...
<div id="whitelisting-firecrawl">
  ## Firecrawl をホワイトリストに追加する {#whitelisting-firecrawl}
</div>

### Firecrawl に Web サイトのスクレイピングを許可する {#allowing-firecrawl-to-scrape-your-website}
...
<div id="allowing-your-application-to-call-the-firecrawl-api">
  ### アプリケーションから Firecrawl API への呼び出しを許可する {#allowing-your-application-to-call-the-firecrawl-api}
</div>
