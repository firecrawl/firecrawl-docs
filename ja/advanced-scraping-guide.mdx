...
- **コスト**: PDF の解析には、PDF 1 ページあたり 1 クレジットがかかります。PDF 解析をスキップして、ファイルを base64 形式で受け取る場合（固定 1 クレジット）は、`parsers: []` を設定します。
- **ページ数の制限**: PDF 解析を特定のページ数までに制限するには、`parsers: [{"type": "pdf", "maxPages": 10}]` を使用します。
...
- **PDF ではアクションはサポートされません**: アクションにはブラウザエンジンが必要なため、PDF コンテンツを返す URL（PDF へリダイレクトされる URL を含む）では使用できません。リクエストに `actions` を含めていて、その URL が PDF を返した場合、そのリクエストは失敗します。PDF をスクレイピングする際は、リクエストから `actions` を削除してください。
...
- **型**: `array`
- **説明**: インクルード対象とする正規表現パターン。デフォルトでは、これらは URL の**パス名のみに**マッチします（クエリパラメータにはマッチしません）。クエリ文字列を含む完全な URL 全体に対してマッチさせたい場合は、`regexOnFullURL: true` を使用します。
- **例**: `["^/blog/.*$", "^/docs/.*$"]`
...
- **型**: `array`
- **説明**: 除外対象を指定する正規表現パターン。デフォルトでは、これらは URL の**パス名のみ**（クエリパラメータは含まれません）に対してマッチします。クエリ文字列を含む URL 全体に対してマッチさせたい場合は、`regexOnFullURL: true` を使用してください。
- **例**: `["^/admin/.*$", "^/private/.*$"]`
...
- **Type**: `boolean`
- **Description**: `true` の場合、`includePaths` と `excludePaths` のパターンは、パスだけでなくクエリパラメータを含む URL 全体にマッチします。クエリ文字列を使って URL をフィルタリングする際に便利です。
- **Default**: `false`
...
- **Description**: クローラーがサイトマップをどのように利用するかを制御します。`"include"`（デフォルト）はリンク探索と併用してサイトマップを使用します。`"skip"` はサイトマップを完全に無視します。`"only"` は、サイトマップ内で見つかったURL（開始URLを含む）のみにクロールを制限します。
- **Default**: `"include"`

<div id="deduplicatesimilarurls">
  #### deduplicateSimilarURLs
</div>

- **Type**: `boolean`
- **Description**: `true` の場合、クローラーは同じページを指す URL のバリエーションを重複として扱います。たとえば、`www.` の有無、`http` と `https`、末尾のスラッシュ、`index.html` / `index.php` のサフィックスなどは、重複判定の前に正規化されます。これによってクエリパラメータが削除されることはありません（クエリパラメータについては `ignoreQueryParameters` を参照してください）。
- **Default**: `true`

<div id="ignorequeryparameters">
  #### ignoreQueryParameters
</div>

- **Type**: `boolean`
- **Description**: `true` の場合、重複排除の前に URL からクエリ文字列が取り除かれるため、`/page?a=1` と `/page?a=2` は同一の URL とみなされ、1 回だけスクレイプされます。
- **Default**: `false`
...
自分のウェブサイトのスクレイピングを Firecrawl に許可するにあたり、クローラーをホワイトリストに登録する必要がある場合は、次を設定してください。

- **User Agent**: Firecrawl は `FirecrawlAgent` というユーザーエージェント文字列でアクセスします。ファイアウォールやセキュリティルールで、このユーザーエージェント文字列を許可してください。
- **IP Addresses**: Firecrawl は、外向きのスクレイピングリクエストに固定の送信元 IP アドレスを使用していません。

<div id="allowing-your-application-to-call-the-firecrawl-api">
  ### アプリケーションから Firecrawl API への呼び出しを許可する
</div>

ファイアウォールがアプリケーションから外部サービスへのアウトバウンドリクエストをブロックしている場合は、Firecrawl の API への呼び出しを許可するために `35.245.250.27` をホワイトリストに追加してください。