...
import ExtractWithoutURLsCURL from "/snippets/ja/v1/extract/without-urls/curl.mdx";

`/extract` エンドポイントは、任意の数のURLやドメイン全体から構造化データを収集する作業を簡素化します。URLのリストを、必要に応じてワイルドカード（例: `example.com/*`）付きで指定し、取得したい情報を記述するプロンプトまたはスキーマを用意してください。Firecrawl がクロール、解析、統合の処理を引き受け、大小さまざまなデータセットに対応します。

<Info>Extract は他のエンドポイントとは異なる課金体系です。詳細は [Extract の料金](https://www.firecrawl.dev/extract#pricing) を参照してください。</Info>
