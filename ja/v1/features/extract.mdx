...
単一または複数の URL から、ワイルドカードを含めて構造化データを抽出できます：
...
`/*` を使用すると、Firecrawl はそのドメイン内で検出したすべての URL を自動的にクロールおよび解析し、その後リクエストされたデータを抽出します。この機能は実験的な機能です。問題がある場合は [help@firecrawl.com](mailto:help@firecrawl.com) までメールでお問い合わせください。
