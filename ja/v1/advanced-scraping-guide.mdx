...
`/scrape` エンドポイントを使用する際、`extract` パラメータを使ってページのコンテンツから**構造化された情報を抽出**するためのオプションを指定できます。利用可能なオプションは次のとおりです。
...
`/scrape` エンドポイントを使用する場合、Firecrawl を使うとコンテンツをスクレイピングする前に Web ページ上でさまざまなアクションを実行できます。これは、動的コンテンツを操作したり、ページを遷移したり、ユーザー操作が必要なコンテンツにアクセスしたりする際に特に有用です。
...
コンテンツが 10MB を超える場合、またはクロールジョブがまだ実行中の場合、レスポンスには `next` パラメータが含まれます。  
このパラメータは、次の結果ページへのURLです。  
このパラメータを使って、次の結果ページを取得できます。
...
`/map` エンドポイントは、指定したウェブサイトとコンテキスト的に関連する URL を特定するのに優れています。この機能は、サイトのコンテキストに基づくリンク環境を把握するうえで重要であり、戦略的なサイト分析やナビゲーション設計に大いに役立ちます。
