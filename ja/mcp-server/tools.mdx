...
これらのツールは、ホスト型のキーなし環境には含まれません。GitHub Issue、マージ済みのプルリクエスト、READMEについては、[`firecrawl_developer_search`](#12b-developer-search-tool-firecrawl_developer_search)を使用してください。

<div id="12b-developer-search-tool-firecrawl_developer_search">
  ### 12b. Developer Search Tool (`firecrawl_developer_search`)
</div>

[Developer Index](/ja/features/developer) を単独で検索します。GitHub issue、マージ済みのプルリクエスト、リポジトリの README、厳選されたドキュメントサイトを対象に検索できます。読み取り専用で、ランク付け済み結果と一致する本文箇所を返します。
