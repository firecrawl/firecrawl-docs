...
クライアントがローカルプロセスを起動する必要がある場合、ローカル HTTP トランスポートを使用する場合、または MCP サーバーをセルフホスト型 Firecrawl API に接続する必要がある場合は、Firecrawl MCP をローカルで実行します。マネージドサービスを使用する場合は、[MCP の設定](/ja/mcp-server)から始めてください。
