...
キーなしの利用枠は、ネットワークごとに適用されるローリング制限です。利用可能な場合、MCP エラーには次回の再試行可能時刻が含まれます。その時刻まで待てば、アカウントなしで Search、Scrape、Parse を引き続き使用できます。

すぐに続行するには、次のいずれかを選択してください。

* **対話型 OAuth:** アカウントエンドポイント `https://mcp.firecrawl.dev/v2/mcp-oauth` に接続します。Claude Code の場合:

  ```bash
  claude mcp add --transport http firecrawl https://mcp.firecrawl.dev/v2/mcp-oauth
  ```

  次に `/mcp` を実行し、ブラウザで認証します。
* **ユーザー操作なしの API キー:** [Firecrawl dashboard](https://www.firecrawl.dev/app/api-keys) でキーを作成し、`Authorization: Bearer <FIRECRAWL_API_KEY>` を設定します。
