...
<a href="cursor://anysphere.cursor-deeplink/mcp/install?name=firecrawl&config=eyJ1cmwiOiJodHRwczovL21jcC5maXJlY3Jhd2wuZGV2L3YyL21jcCJ9">
  <img src="https://cursor.com/deeplink/mcp-install-dark.png" alt="Firecrawl MCP server を Cursor に追加" style={{ maxHeight: 32 }} />

  <span className="fc-sr-only">Cursor に Firecrawl MCP をインストール</span>
</a>
...
最新の構成手順については、MCP servers の構成に関する Cursor 公式ドキュメントを参照してください。
...
4. 次の内容を入力します:
   ...
   * 種類: &quot;command&quot;
   * コマンド: `env FIRECRAWL_API_KEY=your-api-key npx -y firecrawl-mcp@3.23.0`

> Windowsで問題が発生する場合は、`cmd /c "set FIRECRAWL_API_KEY=your-api-key && npx -y firecrawl-mcp@3.23.0"` を試してください。

`your-api-key` を Firecrawl APIキーに置き換えてください。まだお持ちでない場合は、アカウントを作成して https://www.firecrawl.dev/app/api-keys から取得できます。

追加後、MCP serverのリストを更新すると、新しいツールが表示されます。Composer Agent は必要に応じて Firecrawl MCP を自動的に使用しますが、必要なWebデータを説明して明示的にリクエストすることもできます。Command+L (Mac) で Composer を開き、送信ボタンの横にある「Agent」を選択して、query を入力します。
...
ローカルサーバーには `http://localhost:3000/mcp` を使用します。`/v2/mcp` ルートは、`https://mcp.firecrawl.dev/v2/mcp` でホストされているサービスのものです。

クライアントを接続する前に、ローカルプロセスの準備ができていることを確認します：

```bash
curl http://localhost:3000/health
```

ヘルスチェックは `ok` を返します。前提条件と構成については、[Firecrawl MCP をローカルで実行する](/ja/mcp-server/local#run-with-streamable-http)を参照してください。
...
# ユーザー操作なしでAPIキーを構成する場合は、Authorizationヘッダーを使用します
...
7. 認証を選択します：
   * 完全な MCP ツールセットを利用するには、**Authentication** を **Bearer** に設定して Firecrawl APIキーを貼り付けます。
   * キーなしのFreeティアを使用する場合は、**Authentication** を **None** のままにします。この場合、利用できるのは Search、Scrape、Parse のみです。
8. **Tools to include** では、認証モードで提供されるツールから **All**、**Selected**、または **All Except** を選択します。
...
これにより、サーバーが `http://localhost:3000/mcp` で起動します。この URL は、n8n のエンドポイントとして使用できます。n8n では HTTP トランスポートが必要なため、環境変数 `HTTP_STREAMABLE_SERVER=true` の設定が必要です。