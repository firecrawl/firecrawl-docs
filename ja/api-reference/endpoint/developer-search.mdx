...
* `repos` は GitHub 側、つまり `issue`、`pull_request`、`readme` タイプの範囲を制限します
...
* `types` に GitHub タイプが含まれない状態で `repos` を指定すると、`400` が返され、`repos cannot match any requested type; add github types or drop repos` となります
...
7 つのリポジトリフィルター (`language` (`Rust` など) 、`topic` (`async` など) 、`license` (`MIT` など) 、`min_stars`、`max_stars`、`archived`、`fork`) は、GitHub リポジトリの属性を表します。インデックス内のドキュメントページの大半は、リポジトリに紐付かないクロール済み Web サイトから取得されています。そのため、リポジトリの属性でそのようなページを含めたり除外したりすることはできません。

したがって、これらのフィルターのいずれかを指定し、`sources` で範囲を指定しないリクエストでは、`doc` の結果は返されません。レスポンスに含まれるのは GitHub の根拠のみ、すなわち `issue`、`pull_request`、`readme` タイプです。インデックスのドキュメント側は実行されないため、`coverage` マップでは `doc` が `unavailable` と表示されます。これは仕様であり、インデックスの不具合ではありません。
