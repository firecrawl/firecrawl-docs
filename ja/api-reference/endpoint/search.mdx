...
`categories` パラメータを使用して、特定のカテゴリで検索結果を絞り込みます。

* **`github`**: GitHub リポジトリ、コード、Issue、ドキュメントを検索
* **`research`**: Web 検索を学術・研究 **Web サイト** (arxiv.org、nature.com、ieee.org、pubmed.ncbi.nlm.nih.gov、biorxiv.org、medrxiv.org など) に限定します。論文レコードではなく、スニペット付きの通常の Web ページ結果を返します
* **`pdf`**: PDF を検索
* **`developer`**: [Developer Index](/ja/features/developer) を検索 — 公開コードリポジトリの Issue、マージ済みのプルリクエスト、README、および厳選されたドキュメントサイトを対象に検索します

<Note>
  **`research` は論文インデックスではなく、Web サイト用のフィルターです。** このエンドポイントの Web 検索結果を、あらかじめ定められた学術ドメインのリストに絞り込みます。

  科学文献を直接検索するには、PubMed、bioRxiv、medRxiv、arXiv の論文要旨に加え、論文内のパッセージ読み取りや引用グラフの展開を利用できる [Research Index](/ja/features/research) の [`GET /search/research/papers`](/ja/api-reference/endpoint/research-search-papers) を使用してください。
</Note>
