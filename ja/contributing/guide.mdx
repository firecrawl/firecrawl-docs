...
まず、`pnpm` を使って依存関係をインストールします。

```bash
# cd apps/api # 正しいフォルダにいることを確認
pnpm install # pnpm バージョン9以上が必要です
```
...
次に、apps/api/ ディレクトリに移動し、次を実行します：

```bash
pnpm start
# [llm-extract機能](https://github.com/firecrawl/firecrawl/pull/586/)を使用する場合は、OPENAI_API_KEY=sk-______もエクスポートしてください
```

これで、クロールジョブの処理を行うワーカーが起動します。


<div id="terminal-3-sending-our-first-request">
  ### ターミナル 3 - 最初のリクエストを送信する
</div>

では、最初のリクエストを送信してみましょう。
...
<div id="alternative-using-docker-compose">
  ### 代替案: Docker Compose を使用する
</div>

よりシンプルなセットアップのために、Docker Compose を使用してすべてのサービスを実行できます:

1. 前提条件: Docker と Docker Compose がインストールされていることを確認してください
2. `/apps/api/` ディレクトリで `.env.example` ファイルを `.env` にコピーし、必要に応じて設定してください
3. ルートディレクトリから次を実行してください:

```bash
docker compose up
```

これにより、Redis、API サーバー、ワーカーが正しい構成で自動的に起動します。
...
これを行うには、`npm run test:snips` を実行するのが最もよい方法です。