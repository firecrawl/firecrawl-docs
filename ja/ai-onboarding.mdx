...
<Card title="MCP Server" icon="plug" href="/ja/mcp-server">
  ...
  [Windsurf quickstart](/ja/quickstarts/windsurf) を使用してください。
</Card>
...
<Steps>
  ...
  <Step title="MCP ドキュメントサーバー">
    MCP ツールを使って構造化された形で利用する場合は、Codex、Cursor、Claude Code、Claude Desktop、またはその他の互換クライアントで Firecrawl MCP Server に接続してください。ホスト型のセットアップについては、[MCP を始める](/ja/mcp-server) を参照してください。Windsurf ユーザーは [Windsurf クイックスタート](/ja/quickstarts/windsurf) に従ってください。
  </Step>
