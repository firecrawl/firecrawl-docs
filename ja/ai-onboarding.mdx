...
AIを活用して開発しているなら、Firecrawlは開発体験を向上させるためのさまざまなリソースを提供します。Firecrawlには**スキル**が用意されています。これは、AIコーディングエージェントが自動的に見つけて使う自己完結型のナレッジパックです。インストールコマンドを1つ実行するだけで、エージェントは3つの完全なスキル群を利用できるようになります。リアルタイムのWeb作業向けCLIスキル、Firecrawlをアプリケーションコードに統合するためのBuild スキル、そして反復可能な成果物を生み出すためのワークフロースキルです。ベアラートークンの取得方法はセットアップによって異なります。下の[認証情報を取得する](#get-credentials)を参照してください。
...
このページは**判断レイヤー**です。適切な認証情報の取得パスへ案内します。実際の手順はここではなく、以下でリンクしている正規ドキュメントにあります。

Firecrawl は 2 つの認証情報取得パスをサポートしています。人間がサインインしたり、ダッシュボードで API キーを作成したりする必要がある場合は、人間参加型の CLI/ブラウザ オンボーディング (Path A、デフォルト) を使います。WorkOS ID-JAG 登録 (Path B) は、エージェントプラットフォームが ID-JAG を発行できる場合にのみ使用します。

**判断ルール:** ID-JAG がない、または判断に迷う → **Path A (デフォルト)**。ID-JAG に対応している → **Path B**。

**このページに来る主なケース:**

* **API `401`** — `https://api.firecrawl.dev` への未認証リクエストでは、`WWW-Authenticate: Bearer resource_metadata="https://www.firecrawl.dev/.well-known/oauth-protected-resource"` が返されます。[`auth.md`](https://www.firecrawl.dev/auth.md) の Step 1 に従って、その discovery chain を進めてください。プラットフォームで WorkOS ID-JAG 登録を完了できる場合 → **Path B**。それ以外 → **Path A (デフォルト)**。このページを使う必要はありません。
* **Docs** — このページにたどり着いた場合です。上の判断ルールを使ってください。
* **Direct URL** — すでに `auth.md` または `SKILL.md` がある場合は、そのドキュメントを取得して従ってください。

<CardGroup cols={2}>
  <Card title="Path A — 人間 + エージェント支援 (デフォルト)" icon="user" href="https://www.firecrawl.dev/agent-onboarding/SKILL.md">
    **Path A (デフォルト)。** WorkOS ID-JAG を発行できない場合、または判断に迷う場合に使います。ブラウザでのサインイン、CLI `--browser`、Skills と MCP のインストール、または人間がダッシュボードで作成した API キーが該当します。
  </Card>

  <Card title="Path B — エージェント登録 (WorkOS ID-JAG)" icon="robot" href="/ja/ai-onboarding/agent-auth">
    **ID-JAG に対応している場合のみ。** エージェントプラットフォームは `aud=https://api.firecrawl.dev/` 向けの WorkOS ID-JAG を発行できる必要があります。[Agent Auth](/ja/ai-onboarding/agent-auth) を参照し、その後で正規仕様の [`auth.md`](https://www.firecrawl.dev/auth.md) に従ってください。
  </Card>
</CardGroup>

<Card title="API キーを取得 (Path A)" icon="key" href="https://www.firecrawl.dev/app/api-keys">
  サインアップしてダッシュボードから API キーを取得してください。WorkOS ID-JAG 登録を使わない場合のデフォルト パスです。
</Card>

Bearer トークンを取得したら、以下の [Skills + CLI](#skills-cli) に進んでください。
...
<Steps>
  ...
  <Step title="REST API（インストール不要）">
    ...
    * `POST https://api.firecrawl.dev/v2/interact` — スクレイピング済みページをInteractする
...
* パス A — 人間 + エージェント支援 (デフォルト) : [`firecrawl.dev/agent-onboarding/SKILL.md`](https://www.firecrawl.dev/agent-onboarding/SKILL.md)
* パス B — マシン登録 (ID-JAG) : [`firecrawl.dev/auth.md`](https://www.firecrawl.dev/auth.md)
...
<Steps>
  <Step title="認証情報のルーティング">
    まずは [認証情報を取得](/ja/ai-onboarding#get-credentials) から始めて、Path A (デフォルト、人間 + エージェント補助) または Path B (WorkOS ID-JAG) を選択してください。ID-JAG の登録については、[Agent Auth](/ja/ai-onboarding/agent-auth) と正式な [`auth.md`](https://www.firecrawl.dev/auth.md) 仕様を参照してください。
  </Step>
