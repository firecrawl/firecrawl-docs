...
* **Stripe Projects:** すでに Stripe CLI を使用しているプロジェクトでは、`stripe projects add firecrawl/api --name firecrawl` を実行するとアカウントがプロビジョニングされ、`FIRECRAWL_API_KEY` が `.env` に書き込まれます。[Stripe Projects](/ja/integrations/stripe-projects) を参照してください。
...
### インストールで利用できるもの {#what-the-install-gives-you}
...
### エージェントがツールを連携して使う方法 {#how-agents-chain-tools-together}
...
## エージェント向け Firecrawl ドキュメント {#firecrawl-docs-for-agents}
