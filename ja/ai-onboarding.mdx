...
APIキーを取得したら、以下の [CLI](#cli) に進んでください。
...
<Card title="MCP Server" icon="plug" href="/ja/mcp-server/human-mcp">
  Codex、Claude Desktop、Claude Code、Cursor、その他の互換クライアント向けのホスト型セットアップを確認。Windsurf については、専用の
  [Windsurf quickstart](/ja/quickstarts/windsurf) を使用してください。ユーザー操作なしのエージェントまたは CI
  セットアップについては、[Agent MCP](/ja/mcp-server/agent-mcp) を参照してください。
</Card>
...
<Steps>
  ...
  <Step title="MCP ドキュメントサーバー">
    MCP ツールを使って構造化された形で利用する場合は、Codex、Cursor、Claude Code、Claude Desktop、またはその他の互換クライアントで Firecrawl MCP Server に接続してください。対話型のホスト型セットアップについては [Human MCP](/ja/mcp-server/human-mcp) を、ユーザー操作なしで実行するエージェントについては [Agent MCP](/ja/mcp-server/agent-mcp) を参照してください。Windsurf ユーザーは [Windsurf クイックスタート](/ja/quickstarts/windsurf) に従ってください。
  </Step>
