---
title: "認証付きスクレイピング"
description: "Cookie を使って認証の裏側にあるコンテンツをスクレイピングする方法"
---

<Info>
このガイドでは、ブラウザの DevTools から取得したセッション Cookie を使って、認証が必要なコンテンツをスクレイピングする方法を説明します。社内ツール、管理ダッシュボード、ログイン資格情報を持つユーザー向けの認可済みシステムに最適です。
</Info>
...
認証が必要なスクレイピングには、**Cookie ベースの認証**が推奨です。手順は次のとおりです：

1. アプリケーションに手動でログインする
2. DevTools からセッション Cookie を取得する
3. Firecrawl にその Cookie を渡して保護されたページへアクセスする

<Note>
**Cookie の有効期限:**
- **社内ツール**: 多くは 7～30 日、またはそれ以上
- **一般公開サイト**: 通常 1～24 時間
- **銀行/機密性の高いサイト**: 15～30 分

社内ツールは一般的に Cookie の有効期間が長いため、この方法は定期的なスクレイピングに適しています。
</Note>
