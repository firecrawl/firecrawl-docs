...
## 1. `/agent` エンドポイント {#1-agent-endpoint}
...
### 最適なユースケース: 自律的なリサーチと発見 {#best-use-case-autonomous-research-discovery}
...
## 2. `/extract` エンドポイント {#2-extract-endpoint}
...
### 最適なユースケース: ターゲットを絞ったマルチページ抽出 {#best-use-case-targeted-multi-page-extraction}
...
### 最適なユースケース：単一ページの高精度抽出 {#best-use-case-single-page-precision-extraction}
...
### シナリオ別の推奨エンドポイント {#recommendations-by-scenario}
...
### 例: 「Find the founders of Firecrawl」 {#example-find-the-founders-of-firecrawl}
...
## 移行: `/extract` → `/agent` {#migration-extract-agent}
