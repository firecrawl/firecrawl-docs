...
## 鮮度とレイテンシのトレードオフ (`maxAge`) {#the-freshness-tradeoff-maxage}
...
### `maxAge` が適用される箇所 {#where-maxage-applies}
...
## 鮮度は稼働性を意味しない {#freshness-is-not-liveness}
...
## 鮮度が重要なアクションのチェックリスト {#freshness-sensitive-action-checklist}
...
## 実践例: 現在のページの情報を収集する {#worked-example-collect-current-page-evidence}
...
## シナリオ別の推奨事項 {#recommendations-by-scenario}
