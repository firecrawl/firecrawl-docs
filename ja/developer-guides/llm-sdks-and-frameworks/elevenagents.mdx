...
[ElevenAgents](https://elevenlabs.io/agents) の音声エージェントとチャットエージェントに、Firecrawl を使って Web をリアルタイムでスクレイピング、検索、クロールできる機能を追加します。このガイドでは、2 つの統合方法を紹介します。
...
| --------------- | ----------------------------------------- |
| **Name**        | Firecrawl                                 |
| **Description** | あらゆる Web サイトのコンテンツを検索、スクレイピング、クロール、抽出します。 |
| **Server type** | Streamable HTTP                           |
| **Server URL**  | `https://mcp.firecrawl.dev/v2/mcp`        |

この URL で、キー不要の Firecrawl ツールセットを利用できます。アカウント接続のセットアップについては[Human MCP](/ja/mcp-server/human-mcp)を、ユーザー操作なしのセットアップについては[Agent MCP](/ja/mcp-server/agent-mcp)を参照してください。API キーは URL には含めず、Authorization ヘッダーまたは安全なシークレットストアに保管してください。
...
* [ElevenAgents MCP インテグレーション識別子](https://elevenlabs.io/docs/eleven-agents/customization/tools/mcp)
* [Firecrawl API リファレンス](https://docs.firecrawl.dev/api-reference/v2-introduction)
* [Firecrawl MCP サーバー](https://docs.firecrawl.dev/mcp-server/agent-mcp)