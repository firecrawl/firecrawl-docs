...
この URL で、キー不要の Firecrawl ツールセットを利用できます。アカウント接続または無人セットアップについては、[ホスト型 MCP 接続モード](/ja/developer-guides/mcp-setup-guides/oauth)に従ってください。API キーは URL には含めず、Authorization ヘッダーまたは安全なシークレットストアに保管してください。
