...
キー不要の MCP では、共有制限のもとで Search、Scrape、Parse のみを利用できます。より広範なツールセットを使用するには、[OAuth](/ja/mcp-server/oauth) または [API キー](/ja/mcp-server/keyless-api-key#add-an-api-key) を使用してください。API キーは URL には含めず、環境変数またはシークレットストアに保管してください。
