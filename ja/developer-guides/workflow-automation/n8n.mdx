...
<Tip>
  **n8n Cloudユーザー:** APIキーを手動で入力する代わりに、Firecrawlノードを追加する際にワンクリックの**「Connect to Firecrawl」**OAuthボタンを使用できます。これにより、n8nアカウントに連携された新しいFirecrawlチームが自動的に作成され、**100,000無料クレジット**が付与されます。これらのクレジットを[Firecrawlダッシュボード](https://www.firecrawl.dev/app/usage)で確認するには、左上のチームセレクターを使って、n8nに連携されたチームに切り替えていることを確認してください。
</Tip>
