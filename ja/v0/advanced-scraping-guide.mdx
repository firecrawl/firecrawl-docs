...
`/scrape` エンドポイントを使用する際、`extractorOptions` パラメータを使って、ページのコンテンツから**構造化情報を抽出**するためのオプションを指定できます。指定可能なオプションは次のとおりです。
...
スクレイピング処理のタイムアウト時間は、`timeout` パラメータでミリ秒単位に指定して調整できます。
...
複数のページをクロールするには、`/crawl` エンドポイントを使用します。このエンドポイントでは、クロールしたいベース URL を指定でき、その配下のアクセス可能なすべてのサブページがクロールされます。
...
クロールジョブのステータスを確認し、その結果を取得するために使用します。
...
`/crawl` エンドポイントを使用する際、`crawlerOptions` パラメーターでクロールの挙動をカスタマイズできます。利用可能なオプションは次のとおりです。
...
`pageOptions` と `crawlerOptions` パラメーターを組み合わせて、クロール全体の動作をカスタマイズできます。
