...
# APIキーを使用して FirecrawlApp を初期化します
...
const schema = z.object({
  top: z
    ...
    .length(5)
    .describe("Hacker News のトップストーリー上位5件"),
});
...
func main() {
  ...
  if err != nil {
      log.Fatalf("FirecrawlAppの初期化に失敗しました: %v", err)
  }
  ...
  if err != nil {
    log.Fatalf("LLM抽出の実行に失敗しました: %v", err)
  }
...
async fn main() {
    ...
    let app = FirecrawlApp::new(api_key, api_url).expect("FirecrawlApp の初期化に失敗")
    ...
    match llm_extraction_result {
        Ok(data) => println!("LLM 抽出結果:\n{}", data["llm_extraction"]),
        Err(e) => eprintln!("LLM 抽出に失敗: {}", e),
    }
}
```
