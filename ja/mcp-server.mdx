...
<Note>
  **APIキーがありませんか？スクレイピング、検索、Interact は引き続き利用できます。** `FIRECRAWL_API_KEY` を設定せずにサーバーをローカルで実行すると、`firecrawl_scrape`、`firecrawl_search`、`firecrawl_interact` ツールはキー不要のFreeティアにフォールバックします。無料ですが、IPごとにレート制限があり、他のツール (crawl、extract、map など) では利用できません。`FIRECRAWL_API_KEY` を設定すると、すべてのツールが使えるようになり、より高い上限も利用できます。[Rate Limits](/ja/rate-limits#keyless-no-api-key) を参照してください。
</Note>
