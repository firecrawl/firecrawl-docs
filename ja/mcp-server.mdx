...
リモートホストのURLを使用するか、サーバーをローカルで実行します。APIキーは [https://firecrawl.dev/app/api-keys](https://www.firecrawl.dev/app/api-keys) から取得してください。

<Note>
  **APIキーがありませんか？スクレイピング、検索、Interact は引き続き利用できます。** キー不要のリモートURL `https://mcp.firecrawl.dev/v2/mcp` に接続するか (`FIRECRAWL_API_KEY` を設定せずにサーバーをローカルで実行しても同様です)、`firecrawl_scrape`、`firecrawl_search`、`firecrawl_interact` ツールはキー不要のFreeティアにフォールバックします。無料ですが、IPごとにレート制限があり、他のツール (crawl、extract、map など) では利用できません。`FIRECRAWL_API_KEY` を設定すると、すべてのツールが使えるようになり、より高い上限も利用できます。[Rate Limits](/ja/rate-limits#keyless-no-api-key) を参照してください。
</Note>
...
または、APIキーなしで接続して始めることもできます — APIキー不要のFreeティアでは、スクレイピング、検索、Interact を利用できます (IPごとにレート制限あり) :
