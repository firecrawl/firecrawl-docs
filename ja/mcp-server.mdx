...
<Note>
  **Firecrawl APIキーがありませんか？スクレイピング、検索、Interact、解析はキー不要のリモートURLで利用できます。** キー不要のFreeティアを利用するには `https://mcp.firecrawl.dev/v2/mcp` に接続してください。無料ですが、IPごとにレート制限があり、他のツール (crawl、extract、map など) では利用できません。`FIRECRAWL_API_KEY` を設定すると、すべてのツールが使えるようになり、より高い上限も利用できます。[Rate Limits](/ja/rate-limits#keyless-no-api-key) を参照してください。
</Note>
...
API key を使用する場合 (すべてのツールと、より高い上限が利用可能) :

```bash
https://mcp.firecrawl.dev/{FIRECRAWL_API_KEY}/v2/mcp
```

または、APIキーなしで接続して始めることもできます — リモートのAPIキー不要のFreeティアでは、スクレイピング、検索、Interact、解析 を利用できます (IPごとにレート制限あり) :
...
<div id="10-create-browser-session-firecrawl_browser_create">
  ### 10. ブラウザセッションの作成 (`firecrawl_browser_create`)
</div>

CDP (Chrome DevTools Protocol) 経由でコードを実行するための永続的なブラウザセッションを作成します。
...
<div id="11-execute-code-in-browser-firecrawl_browser_execute">
  ### 11. ブラウザでコードを実行する (`firecrawl_browser_execute`)
</div>
...
<div id="12-delete-browser-session-firecrawl_browser_delete">
  ### 12. ブラウザセッションの削除 (`firecrawl_browser_delete`)
</div>
...
<div id="13-list-browser-sessions-firecrawl_browser_list">
  ### 13. ブラウザセッションの一覧 (`firecrawl_browser_list`)
</div>

ブラウザセッションを一覧表示し、必要に応じてステータスでフィルタリングできます。
...
<div id="14-interact-with-scraped-page-firecrawl_interact">
  ### 14. スクレイピングしたページを Interact (`firecrawl_interact`)
</div>
...
<div id="15-stop-interact-session-firecrawl_interact_stop">
  ### 15. Interact セッションの停止 (`firecrawl_interact_stop`)
</div>
