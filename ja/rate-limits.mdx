...
## 同時ブラウザ数の上限 {#concurrent-browser-limits}
...
### キーレス (APIキー不要) {#keyless-no-api-key}
...
### Extract エンドポイント {#extract-endpoints}

Extract エンドポイントには、対応する `/agent` エンドポイントと同じレート制限が適用されます。

### バッチスクレイプエンドポイント {#batch-scrape-endpoints}
