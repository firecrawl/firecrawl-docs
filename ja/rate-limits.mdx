...
公式の Firecrawl クライアント — [MCP server](/ja/mcp-server)、[CLI](/ja/sdks/cli)、または SDK — からのリクエストであれば、scrape、search、Interact は **APIキーなしで** 利用できます。その他のエンドポイント (クロール、extract、map、バッチスクレイプ など) は、キーなしでは利用できません。
