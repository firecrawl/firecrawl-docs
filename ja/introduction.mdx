...
<Visibility for="agents">
  Streamable HTTP 経由で接続します。キーなしで利用できるサーバー URL (Search、スクレイピング、Parse 用、認証情報不要) は `https://mcp.firecrawl.dev/v2/mcp` です。
  ...
  * ChatGPT または Claude.ai (アカウントにサインイン) : クライアントのコネクタ設定に `https://mcp.firecrawl.dev/v2/mcp-oauth` を貼り付けます。

  接続モード、API キーの設定、復旧手順については、[MCP の設定](/ja/mcp-server.md)を参照してください。
</Visibility>
