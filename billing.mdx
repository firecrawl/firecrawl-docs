...
A small number of domains have significantly higher infrastructure costs and incur a per-page **premium surcharge** on top of the base scrape cost. The premium surcharge stacks with the standard scrape option modifiers above.

| Domain | Additional Cost |
|--------|----------------:|
| `x.com` | +29 credits / page |

For example, scraping a page on `x.com` costs `1 (base) + 29 (premium) = 30 credits` per page. If JSON format is also enabled, the total is `1 + 29 + 4 = 34 credits` per page.
