...
If you occasionally need more credits than your plan includes, you can enable **auto-recharge** from the dashboard. When your remaining credits drop below a configurable threshold, Firecrawl automatically purchases an additional credit pack and adds it to your balance. Note that the threshold is not a spending limit — setting it to a high value may trigger multiple recharges in quick succession.
