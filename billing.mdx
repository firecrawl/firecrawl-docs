...
If you need an invoice reissued with updated billing details (for example, after adding a VAT number, company name, or new billing address), team admins can regenerate any paid invoice directly from the dashboard — no support ticket required.
...
- For a normal paid invoice, Firecrawl issues a **credit note** for the original invoice and creates a new paid invoice with the updated details. Your card is **not** charged again, and your credit balance is unchanged.
- For a zero-amount invoice (for example, a 100% coupon), the original invoice is voided and a new replacement invoice is created and backdated to the original date. No credit note is needed because there is nothing to credit.
- The new invoice replaces the old one in your Billing History. The original PDF remains accessible from Stripe via the credit note record for accounting purposes.
