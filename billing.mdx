...
| **Search** | 2 credits / 10 results | Search the web and optionally scrape the results. Additional per-page scrape costs apply to each result that is scraped. Enterprise ZDR search costs 10 credits / 10 results (see [ZDR Search](/features/search#zero-data-retention-zdr-search)). |
