...
Firecrawl uses a **credit-based billing system**. Every API call that you make consumes credits. The number of credits depends on the endpoint and the options that you use. Your plan gives you a monthly credit allotment. Auto-reload can buy more credits when the allotment runs out.
...
| Prompt injection check | +4 credits / page | Opt-in `checkPromptInjection` guard for JSON format (see [Prompt injection detection](/features/llm-extract#prompt-injection-detection)). If the scrape fails after the check has run — including when an injection is detected and the request is blocked with a 403 — 5 credits are billed instead of the usual 0 for a failed scrape. |
...
Firecrawl offers subscription plans with monthly or yearly billing. Self-serve plans use pay-as-you-go billing. Auto-reload adds credits when your plan allotment runs out. See [Auto-reload](#auto-reload).
...
Auto-reload buys credits in batches of &#36;5. Firecrawl buys a batch when your credit balance reaches zero. Firecrawl charges the card on file.

Auto-reload needs a paid self-serve plan. You cannot use auto-reload on the free plan.

You can also buy credits yourself at any time. Use **Load more credits** in your [billing settings](https://www.firecrawl.dev/app/settings?tab=billing). Enter a multiple of 5 USD. One purchase can be 1,000 USD at most. Firecrawl charges the card on file. This works whether auto-reload is on or off.
...
The control is a field named **Monthly auto-reload limit**. The field is in two places:

- Your [billing settings](https://www.firecrawl.dev/app/settings?tab=billing), on the **Billing** tab.
- The plan-confirm dialog on the [pricing page](https://www.firecrawl.dev/pricing), when you select a plan.

Enter a value in the field, then save it. The field accepts these values:

| Value | Result |
|-------|--------|
| Blank | No monthly limit. Auto-reload continues for as long as you use credits. |
| A multiple of 5 USD | Auto-reload spends no more than this amount each month. |
| `0` | Auto-reload is off. Firecrawl does not buy credits for you. |

To turn auto-reload on again, enter a multiple of 5 USD in the same field.

### How the limit caps your monthly spend

The limit is the largest amount that auto-reload can spend in one month. Firecrawl rounds the limit down to whole &#36;5 batches. The limit has no maximum.

For example, a limit of &#36;25 allows five batches each month. A limit of &#36;22 allows four batches, because Firecrawl rounds &#36;22 down to &#36;20.
...
With auto-reload on, Firecrawl buys a new &#36;5 batch of credits when your balance reaches zero. Your requests continue.
...
<AccordionGroup>
  ...
  <Accordion title="How does the monthly auto-reload limit work?">
    The limit caps what auto-reload spends each month. Firecrawl rounds the limit down to whole &#36;5 batches. A limit of &#36;25 allows five batches each month. Leave the field blank for no limit. Enter `0` to turn auto-reload off.
  </Accordion>
