...
<Note>
**A spend limit does not cap search.** The limit applies to credit usage. The Search endpoint bills a separate search credit meter, so search usage does not count toward a credit limit.
</Note>
