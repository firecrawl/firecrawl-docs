...
- **Faster by default**
  - Requests are cached with `maxAge` defaulting to 2 days, and sensible defaults like `blockAds`, `skipTlsVerification`, and `removeBase64Images` are enabled.

- **Scrape summary format**
  - **New**: You can now specify `"summary"` as a format to directly receive a concise summary of the page content.

- **Scrape json format and extract**
  - JSON extraction and change tracking are now an object format: `{ type: "json", prompt, schema }`.
  - The old `"extract"` format no longer exists (renamed to `"json"`).

- **Scrape screenshot options**
  - Use the object form: `{ type: "screenshot", fullPage, quality, viewport }`.

- **Search can return news and images**
  - When source is `"news"` or `"images"`

- **Crawl prompt + preview**
  - **New**: pass a natural-language `prompt` to crawl and the system derives paths/limits automatically.
  - **New**: crawl-params-preview endpoint lets you inspect the derived options before starting a job.
