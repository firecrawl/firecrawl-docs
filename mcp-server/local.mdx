...
Run Firecrawl MCP locally when a client needs to launch a local process, when you need a local HTTP transport, or when the MCP server must connect to a self-hosted Firecrawl API. For the managed service, use [Agent MCP](/mcp-server/agent-mcp) or [Human MCP](/mcp-server/human-mcp).
...
<Tabs>
  <Tab title="stdio">
    ...
    Configure the client to run `npx -y firecrawl-mcp@3.23.4` and provide `FIRECRAWL_API_KEY` through the client's protected environment or secret mechanism.
  </Tab>
