...
<McpClientSelector variant="agent" showSeeAll={false} includeManaged={false} />

Use keyless access for a quick trial. Use an API key when the connection must run reliably without a person present. If you want to sign in through a browser instead — including from ChatGPT or Claude.ai — use [OAuth](/mcp-server/oauth).
