...
sidebarTitle: Firecrawl MCP Tools
---
...
Start with [MCP setup](/mcp-server) to choose keyless, OAuth, or API-key access. Some optional tools can be disabled by environment or team policy.
...
| Extract structured data from a known URL | `firecrawl_scrape` with JSON format | You know which page contains the data and want output that follows a prompt or schema. |
| Find and extract data when the sources are unknown | `firecrawl_agent` and `firecrawl_agent_status` | The task requires discovery across one or more sources before producing structured results. |
...
<AccordionGroup>
  ...
  <Accordion title="Check a running crawl">
    `firecrawl_crawl` and `firecrawl_agent` return job identifiers. Use `firecrawl_check_crawl_status` or `firecrawl_agent_status` until the job completes or fails.
  </Accordion>
...
- **A tool is missing:** confirm the connection mode on [MCP setup](/mcp-server), reconnect or restart the client, and check whether team policy disables optional tools.
- **The client returns `401`:** repeat the OAuth sign-in or verify that the configured bearer-token environment variable contains a current Firecrawl API key.
