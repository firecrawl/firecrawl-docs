...
- `categories`: Array of categories to search (`github`, `research`, `pdf`, `developer`). Use `developer` for programming questions; those results come back in a `developer` group
...
Search the [Developer Index](/features/developer) for a programming question, covering GitHub issues, merged pull requests, repository READMEs, and curated documentation sites. The tool only reads.
...
- `query`: Natural language developer question or search phrase (required)
- `k`: Number of ranked results to return, 1 to 100 (default 20)

**Returns:** Ranked results as markdown blocks carrying the result id, source type, URL, title, and the matched passages.

**Best for:** Code behavior, a library or framework, an API contract, an error message, or a known bug. Pass `categories: ["developer"]` to `firecrawl_search` instead when you want developer results alongside ordinary web results.
