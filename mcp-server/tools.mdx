...
Start with [Firecrawl MCP setup](/mcp-server) to choose OAuth, API-key, or keyless access. Some optional tools can be disabled by environment or team policy.
...
| Extract many pages | `firecrawl_crawl` and `firecrawl_check_crawl_status` | You need to traverse a site or section and poll an asynchronous job. |
...
<AccordionGroup>
  ...
  <Accordion title="Poll asynchronous jobs">
...
- **A tool is missing:** confirm the [connection mode](/mcp-server#setup-firecrawl-mcp-server), reconnect or restart the client, and check whether team policy disables optional tools.
