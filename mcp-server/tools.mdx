...
Start with [Agent MCP](/mcp-server/agent-mcp) or [Human MCP](/mcp-server/human-mcp) to choose OAuth, API-key, or keyless access. Some optional tools can be disabled by environment or team policy.
...
- **A tool is missing:** confirm the connection mode on [Agent MCP](/mcp-server/agent-mcp) or [Human MCP](/mcp-server/human-mcp), reconnect or restart the client, and check whether team policy disables optional tools.
