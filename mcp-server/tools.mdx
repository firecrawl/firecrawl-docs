---
title: Firecrawl MCP tools and operations
description: Available tools, operational behavior, and error handling for Firecrawl MCP.
...
| Hosted keyless | `firecrawl_search`, `firecrawl_scrape`, and `firecrawl_parse` only. `firecrawl_search` accepts `categories: ["developer"]` on this surface |
...
Start a crawl with advanced options. This MCP call blocks while the server polls the crawl to a terminal state, then returns the final status and collected data.
...
Example MCP response (`content[0].text` contains serialized JSON):
...
Parse `content[0].text` as JSON to access the decoded extracted payload.
...
Use the read-only research tools for literature review, paper inspection, and citation discovery.
...
- `query`: The search query string (required)
- `k`: Number of ranked results to return, from 1 to 100. Defaults to 10
- `skills`: Set to `"only"` to limit the search to indexed agent-skill files

**Best for:** A programming question — code behavior, a library or framework, an API contract, an error message, or a known bug — answered from primary sources.
**Returns:** Each hit as a markdown block carrying its id, type, url, title, and matched passages.

This tool is not part of the hosted keyless surface. For developer results without an account connection, call `firecrawl_search` with `categories: ["developer"]`.
