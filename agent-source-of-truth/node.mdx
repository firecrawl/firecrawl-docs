...
Canonical Firecrawl Node.js source of truth for agents. Aligned with `firecrawl` **v4.40.0** (`firecrawl/apps/js-sdk/firecrawl`) and the v2 OpenAPI spec. Method names, parameters, and types match the SDK public API.
...
- `options.sources`
  ...
  - Confirmed values:
    ...
    - `{ type: "web" | "news" | "images" | "alexandria" }`: typed source object form
...
- `options.categories`
  ...
  - Confirmed values:
    ...
    - `{ type: "developer" | "research" | "pdf" | "github" }`: typed category object form
...
- `options.domainTools`
  - Type: boolean
  - Use when: you want domain-matched Alexandria contracts in `tools` alongside semantic matches.

- `options.country`
  - Type: string
  - Use when: you need ISO 3166-1 alpha-2 geo-targeting (e.g. "US").

- `options.highlights`
  - Type: boolean
  - Use when: you want query-relevant highlights for search results. Default: true.
...
- `options.excludeDomains`
  - Type: string[]
  - Use when: you want to exclude specific domains. Cannot be combined with includeDomains.

- `options.enterprise`
  - Type: array of "default" | "anon" | "zdr"
  - Use when: you need enterprise search options (Zero Data Retention).

- `options.threatProtection`
  - Type: ThreatProtectionOptions
  - Use when: you need enterprise per-request threat protection override.
...
Alexandria scrape overload: `client.scrape({ alexandria: [...], requestId?, timeout?, integration? })` — executes Alexandria domain-tool calls without a URL.
...
const doc = await client.scrape("https://example.com/pricing", {
  ...
  onlyMainContent: true,
  waitFor: 1000,
  parsers: [{ type: "pdf", mode: "auto", maxPages: 5, pages: true, blocks: true }],
  actions: [
...
- `options.formats`
  ...
  - Object-only format types (at minimum `type` as shown):
    ...
    - `{ type: "query", prompt: string, mode?: "freeform" | "directQuote" }`: deprecated — use `question` or `highlights` instead.
...
- `options.redactPII`
  ...
  - RedactPIIOptions fields:
    ...
    - `entities`: array of `"PERSON"` | `"EMAIL"` | `"PHONE"` | `"LOCATION"` | `"FINANCIAL"` | `"SECRET"`
...
- `options.autoResume`
  - Type: boolean
  - SDK-only parameter (never sent to API). When not explicitly set to false, automatically retries if the server signals processing_continues. Bounded to 5 resumes / 20 min total wait.
...
- `cdpUrl`: string (CDP WebSocket URL when available)
- `error`: string
...
- Deprecated client aliases: `scrapeExecute` → `interact`; `stopInteractiveBrowser` and `deleteScrapeBrowser` → `stopInteraction`; `scrapeUrl` → `scrape`.
...
- `client.findTools(options?)` discovers Alexandria tool catalogue entries.
- `client.developerSearch(query, options?)` searches the dedicated developer index.
