...
Canonical Firecrawl Node.js source of truth for agents. Aligned with `firecrawl` **v4.41.0** (`firecrawl/apps/js-sdk/firecrawl`) and the v2 OpenAPI spec. Method names, parameters, and types match the SDK public API.
...
const client = new Firecrawl({
  ...
  // timeoutMs: 60000    // optional; request timeout in milliseconds
  // maxRetries: 3       // optional; max retry attempts on transient errors
  // backoffFactor: 2    // optional; exponential backoff multiplier between retries
});
```
...
- `news`: news hits
- `images`: image hits
- `tools`: discovered tools (array of `DiscoveredTool`), present when `domainTools` is enabled
- `agent_hints`: agent hint strings, present when `domainTools` is enabled
...
- `options.domainTools`
  - Type: boolean
  - Use when: you want Alexandria domain-tool discovery in results.

- `options.toolDetail`
  - Type: `"compact"` | `"summary"` | `"full"`
  - Use when: you want to control the detail level of domain-tool discovery responses.
...
- `options.country`
  - Type: string
  - Use when: you want geo-targeted results.
  - Notes: ISO 3166-1 alpha-2 country code (for example `"US"`, `"GB"`, `"DE"`).

- `options.highlights`
  - Type: boolean
  - Use when: you want query-relevant highlights generated for results (defaults to true).
...
- `options.threatProtection`
  ...
  - Use when: you need threat-protection configuration for search results.

- `options.integration`
  - Type: string
  - Use when: you want to pass an integration identifier for telemetry.

- `options.origin`
  - Type: string
  - Use when: you want to pass an origin label for telemetry.
...
- `options.lockdown`
  - Type: boolean
  - Use when: you want to serve only cached results with no outbound requests.

- `options.redactPII`
  - Type: boolean | `RedactPIIOptions`
  - Use when: you want to redact personally identifiable information from content.

- `options.threatProtection`
  - Type: `ThreatProtectionOptions`
  - Use when: you need threat-protection configuration for the scrape.
...
- `options.domainTools`
  - Type: boolean
  - Use when: you want Alexandria domain-tool discovery alongside the scrape.
...
- `options.autoResume`
  - Type: boolean
  - Use when: you want to control SDK-side automatic resume for large documents (defaults to true). SDK-only option, not sent to the API.
...
- `args.origin`
  - Type: string
  - Use when: you want to pass an origin label for execution telemetry.
...
- `cdpUrl`: string (CDP URL for the browser session)
...
- Response fields in typings include `success`, `status`, `sessionDurationMs`, `creditsBilled`, `error`.
