...
# For self-hosted instances, set a custom base URL:
config :firecrawl, base_url: "https://firecrawl.your-company.com"

# or pass api_key / base_url per call
{:ok, res} = Firecrawl.search_and_scrape([query: "site:docs.firecrawl.dev webhook retries"], api_key: "fc-your-api-key")
```

The keyless free tier is available on the hosted API — omit `api_key` to use it with default rate limits.

Every request automatically includes an `"origin": "elixir-sdk@1.11.0"` header for telemetry attribution.
...
- `audit_metadata`
  - Type: keyword list with `username:`
...
- `code`
  ...
  - Notes: this parameter is required in the Elixir SDK.
...
- The Elixir client is auto-generated from the OpenAPI spec via `generate.exs`; function names and parameter keys mirror the spec.
- Each public function has a bang (`!`) variant that raises on error instead of returning `{:error, _}`.
- Each request body automatically includes `"origin": "elixir-sdk@1.11.0"` for telemetry attribution.
