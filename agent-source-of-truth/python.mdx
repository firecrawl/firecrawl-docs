...
Canonical Firecrawl Python source of truth for agents. Generated from SDK source (`firecrawl-py` / `firecrawl` **4.34.0**) and the v2 OpenAPI spec. Method names, parameters, and return types match the v2 client in `firecrawl/v2/client.py` unless noted.
...
- `developer` — developer hits (`SearchResultWeb` or `Document`)
...
- `threat_protection`
  ...
  - Fields: `mode` (`"off"` or `"normal"`), `risk_score_threshold` (0-100), `blacklist`, `whitelist`, `blocked_tlds`, `failure_policy` (`"open"` or `"closed"`)
...
doc = client.scrape(
    ...
    audit_metadata={"username": "bot-user"},
...
- `audit_metadata`
  - Type: `AuditMetadata` or dict with `username`
  - Use when: you want to attach audit metadata (such as an end-user identifier) to the scrape.
  - Fields: `username` (str)
...
- Deprecated aliases: `scrape_execute` → `interact`; `stop_interactive_browser` and `delete_scrape_browser` → `stop_interaction`; `scrape_url` → `scrape`; `crawl_url` → `crawl`; `map_url` → `map`.
...
- `RedactPIIOptions`: `mode` (`"accurate"`, `"aggressive"`, `"fast"`), `entities` (list of `"PERSON"`, `"EMAIL"`, `"PHONE"`, `"LOCATION"`, `"FINANCIAL"`, `"SECRET"`), `replace_style` (`"tag"`, `"mask"`, `"remove"`).
