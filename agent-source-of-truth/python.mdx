...
Canonical Firecrawl Python source of truth for agents. Generated from SDK source (`firecrawl-py` / `firecrawl` **4.43.0**) and the v2 OpenAPI spec. Method names, parameters, and return types match the v2 client in `firecrawl/v2/client.py` unless noted.
...
- `tools` — discovered Alexandria tools (present when `domain_tools=True`)

Omitted buckets are `None` when the API did not return that key.

**Wrong turn to avoid:** `search()` does not return `{ data: [...] }`. Do not access `result.data` — it raises `AttributeError` with a helpful message directing you to the correct attributes. Web results are in `result.web`, news in `result.news`, images in `result.images`.
...
- `sources`
  ...
  - Confirmed values:
    ...
    - `"alexandria"`: Alexandria knowledge-base results
    - `Source(type="web" | "news" | "images" | "alexandria")`: typed source object form
...
- `categories`
  ...
  - Confirmed values:
    ...
    - `"github"`: GitHub code results
    - `Category(type="developer" | "research" | "pdf" | "github")`: typed category object form
...
- `domain_tools`
  - Type: bool
  - Use when: you want Alexandria domain-tool discovery.
...
- `exclude_domains`
  - Type: list of str
  - Use when: you want to exclude specific domains. Cannot combine with `include_domains`.

- `country`
  - Type: str
  - Use when: you need ISO 3166-1 alpha-2 geo-targeting.
...
- `integration`
  - Type: str
  - Use when: you need an integration identifier.
...
- `threat_protection`
  ...
  - Use when: enterprise per-request threat protection override.
...
Execute Alexandria domain-tool calls without a URL:

```py
from firecrawl.v2.types import AlexandriaCall

doc = client.scrape(alexandria=AlexandriaCall(...))
# or with multiple calls:
doc = client.scrape(alexandria=[...], request_id="...", timeout=...)
```
...
- `auto_resume`
  - Type: bool
  - Use when: you want automatic retry on timeout (attaches to same in-flight job).

- `lockdown`
  - Type: bool
  - Use when: you want to serve only previously cached results (returns 404 on cache miss).

- `audit_metadata`
  - Type: AuditMetadata (has `username: str`)
...
- `domain_tools`
  - Type: bool
  - Use when: you want Alexandria domain-tool discovery for this URL.
...
- `origin`
  - Type: str
  - Use when: optional origin label for execution telemetry.

### Return value

Returns `BrowserExecuteResponse`: `success`, optional `live_view_url`, `interactive_live_view_url`, `output`, `stdout`, `result`, `stderr`, `exit_code`, `killed`, `error`, `cdp_url` (API camelCase is normalized to snake_case on the model).
...
- Alexandria tool calling is supported via `client.scrape(alexandria=...)`.
- `client.scrape_alexandria(calls, ...)` is also available as a convenience method for Alexandria calls.
- `client.developer_search(query, **kwargs)` searches the dedicated developer index.
- `client.search_papers(query, **kwargs)` searches the research paper index.
- `client.search_github(query, **kwargs)` searches the GitHub code index.
- `client.find_tools(**kwargs)` discovers the Alexandria tool catalogue.
- `client.parse(file, **options)` parses uploaded files.
- Async client available via `AsyncFirecrawl` / `AsyncFirecrawlApp`.
