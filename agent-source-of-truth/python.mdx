...
`client.scrape(url, *, auto_resume=None, **options)` → `Document` (`firecrawl.v2.types`)
...
- `timeout`
  - Type: int
  - Use when: you need a timeout in milliseconds. Large PDFs that outlive this window keep processing server-side and return HTTP 408 with `code: "SCRAPE_TIMEOUT"` and `details.state: "processing_continues"` — retry the same URL after `Retry-After`.

- `auto_resume`
  - Type: bool (SDK-only keyword on `scrape()`, never sent on the wire; `firecrawl-py` ≥ 4.40)
  - Use when: you want to disable automatic resume of large-PDF scrapes. Default is on: the SDK sleeps `Retry-After` and re-issues `scrape()` so a long document completes as one call. Set `False` to surface the timeout immediately.
