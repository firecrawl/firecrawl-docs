...
- `tools` — discovered tools (`Optional[List[DiscoveredTool]]`, present when `domain_tools=True`)
...
- `sources`
  ...
  - Confirmed values:
    ...
    - `"alexandria"`: Alexandria knowledge source
...
- `tool_detail`
  ...
  - Use when: you want to control the detail level of discovery responses.
...
- `threat_protection`
  ...
  - Use when: you need threat protection configuration.

- `integration`
  - Type: str
  - Use when: you need to pass an integration identifier.
...
- `auto_resume`
  - Type: bool
  - Use when: you want the SDK to automatically resume large documents.
  - Notes: SDK-only feature for auto-resuming large document scrapes.
...
- `use_mock`
  - Type: str
  - Use when: you want to enable mock mode.
...
- `origin`
  - Type: str
  - Use when: you want to attach an origin label for execution telemetry.

### Return value

Returns `BrowserExecuteResponse`: `success`, optional `live_view_url`, `interactive_live_view_url`, `output`, `stdout`, `result`, `stderr`, `exit_code`, `killed`, `error`, `cdp_url`, `truncated` (API camelCase is normalized to snake_case on the model).
