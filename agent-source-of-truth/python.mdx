...
Canonical Firecrawl Python source of truth for agents. Generated from SDK source (`firecrawl-py` **4.41.0**) and the v2 OpenAPI spec. Method names, parameters, and return types match the v2 client in `firecrawl/v2/client.py` unless noted.
...
- `categories`
  ...
  - Confirmed values:
    ...
    - `"github"`: site:github.com filter
    - `"research"`: research and academic results
    - `"pdf"`: PDF-focused results
    - `Category(type="developer" | "github" | "research" | "pdf")`: typed category object form

- `include_domains`
  - Type: list of str
  - Use when: you want to restrict results to specific domains. Cannot be combined with `exclude_domains`.

- `exclude_domains`
  - Type: list of str
  - Use when: you want to exclude results from specific domains. Cannot be combined with `include_domains`.
...
- `enterprise`
  - Type: list of str
  - Use when: you need enterprise search controls. `"zdr"` for end-to-end Zero Data Retention, `"anon"` for anonymized search. Must be enabled for your team.

- `threat_protection`
  - Type: `ThreatProtectionOptions`
  - Use when: you need enterprise per-request threat protection overrides.
...
- `formats`
  ...
  - Format object fields:
    ...
    - `check_prompt_injection`: boolean for `type: "json"` to enable prompt-injection safety check
...
- `lockdown`
  - Type: bool
  - Use when: you want to serve only previously cached results with no outbound requests. Returns 404 on cache miss.
...
- `audit_metadata`
  - Type: `AuditMetadata` with field `username: str`
...
- Deprecated aliases: `scrape_execute` → `interact`; `stop_interactive_browser` and `delete_scrape_browser` → `stop_interaction`; `scrape_url` → `scrape` (V1 compatibility).
