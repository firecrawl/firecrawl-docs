...
Canonical Firecrawl Java source of truth for agents. Generated from SDK source (`com.firecrawl:firecrawl-java` **v1.17.0**) and the v2 OpenAPI spec.
...
- `options.formats`
  ...
  - Format object fields:
    ...
    - `prompt`, `schema`: JSON extraction options for `type: "json"` (or use `JsonFormat.builder()`)
    - `question`: for `type: "question"` (or use `QuestionFormat.builder()`)
    - `query`: for `type: "highlights"` (or use `HighlightsFormat.builder()`)
...
- `options.auditMetadata`
  - Type: `AuditMetadata` with field `username: String`
