...
You can also create a client directly from the `FIRECRAWL_API_KEY` environment variable:
...
- `options.country`
  - Type: String
  - Use when: you need geo-targeted results (ISO country code, e.g. `"US"`).

- `options.highlights`
  - Type: Boolean
  - Use when: you want the API to generate highlights for each result. Defaults to true.
...
- `options.auditMetadata`
  - Type: `AuditMetadata`
  - Use when: you need user attribution for SIEM or audit logging.
  - Fields:
    - `username`: String identifying the acting user.
...
Async variants are also available: `client.interactAsync(...)` with the same overload signatures.
...
- Deprecated aliases: `scrapeExecute` → `interact`, `scrapeExecuteAsync` → `interactAsync`, `deleteScrapeBrowser` → `stopInteractiveBrowser`, `deleteScrapeBrowserAsync` → `stopInteractiveBrowserAsync` (and any other corresponding `*Async` helpers).
...
- The SDK automatically sets `origin` to `"java-sdk@{version}"` on outgoing requests when the caller does not provide one.
