...
- `options.country`
  - Type: String
  - Use when: you need ISO 3166-1 alpha-2 geo-targeting (e.g. `"US"`, `"GB"`).
...
- `options.formats`
  ...
  - Named format objects:
    - `QuestionFormat`: `type` is `"question"`, has `question` field. Use for question-based extraction.
    - `HighlightsFormat`: `type` is `"highlights"`, has `query` field. Use for query-relevant highlights.
    - `QueryFormat`: deprecated — use `QuestionFormat` or `HighlightsFormat` instead.
...
- `options.parsers`
  ...
  - PdfParser additional fields:
    - `mode`: String (`"fast"`, `"auto"`, or `"ocr"`)
    - `pages`: Boolean — include page-level content
    - `blocks`: Boolean — include block-level content
    - `pageMarkers`: Boolean — include page marker annotations
...
- `options.redactPII`
  - Type: Boolean
  - Use when: you want to redact PII from returned content. (JSON property: `redactPII`)

- `options.auditMetadata`
  - Type: `AuditMetadata`
  - Use when: you need SIEM logging attribution.
  - Fields:
    - `username`: String — the user identity for audit logs.
...
- Async variants are available for all methods (e.g. `scrapeAsync`, `searchAsync`, `interactAsync`).
- `client.fromEnv()` factory creates a client from environment variables.
