...
Canonical Firecrawl Java source of truth for agents. Generated from SDK source (`firecrawl-java` **v1.12.1**) and the v2 OpenAPI spec.
...
`scrape` returns `Document`. Typical getters include `getMarkdown()`, `getHtml()`, `getRawHtml()`, `getJson()`, `getMetadata()`, `getLinks()`, `getAudio()`, `getVideo()`, `getProduct()`, `getMenu()`, `getBranding()`, `getHighlights()`, `getAnswer()`, and additional fields when the corresponding formats are requested.
...
ScrapeOptions options = ScrapeOptions.builder()
    .formats(List.of(
        ...
        HighlightsFormat.builder().query("enterprise pricing").build(),
...
- `options.formats`
  ...
  - Format object types:
    - `JsonFormat.builder().prompt("...").schema(map).build()`: JSON extraction
    - `QuestionFormat.builder().question("...").build()`: question-answer extraction. Answer returned in `document.getAnswer()`.
    - `HighlightsFormat.builder().query("...").build()`: relevant source-text extraction. Result returned in `document.getHighlights()`.
  - Format map fields:
