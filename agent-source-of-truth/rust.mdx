    ...
    .scrape("https://example.com/pricing", ScrapeOptions {
        formats: Some(vec![
            ...
            Format::Highlights(HighlightsFormat {
                query: "enterprise plan".to_string(),
            }),
        ]),
        json_options: Some(JsonOptions {
...
- `options.formats`
  ...
  - Confirmed simple values: `Markdown`, `Html`, `RawHtml`, `Links`, `Images`, `Screenshot`, `Summary`, `ChangeTracking`, `Json`, `Attributes`, `Branding`, `Product`, `Menu`, `Audio`, `Video`
  - Confirmed object variants:
    - `Question(QuestionFormat)` — serialized as `{ "type": "question", "question": "..." }`
    - `Highlights(HighlightsFormat)` — serialized as `{ "type": "highlights", "query": "..." }`
  - Deprecated: `Query(QueryFormat)` — use `Question` or `Highlights` instead.
...
- Deprecated format: `Format::Query(QueryFormat)` — use `Format::Question(QuestionFormat)` or `Format::Highlights(HighlightsFormat)` instead.
