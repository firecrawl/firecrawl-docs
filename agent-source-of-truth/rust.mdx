...
Crate: **`firecrawl`** on crates.io. The current SDK version is **2.20.0** (verify the latest release on crates.io before pinning).
...
let options = SearchOptions {
    sources: Some(vec![SearchSource::Web, SearchSource::News, SearchSource::Alexandria]),
...
- `SearchData`
  ...
  - `tools: Option<Vec<DiscoveredTool>>` — populated when `domain_tools` is enabled.
...
- `options.sources`
  ...
  - Confirmed values: `Web`, `News`, `Images`, `Alexandria`

- `options.categories`
  - Type: `Vec<SearchCategory>`
  - Use when: you want to filter results by category.
  - Confirmed values: `Research`, `Pdf`, `Github`
...
- `options.formats`
  ...
  - Notes: `Question(QuestionFormat)`, `Highlights(HighlightsFormat)`, `Query(QueryFormat)` are parametric variants.
...
- `options.parsers`
  ...
  - Confirmed values:
    ...
    - `ParserConfig::Pdf { parser_type: "pdf", max_pages: Some(n), mode: Option<String>, pages: Option<bool>, blocks: Option<bool>, page_markers: Option<bool> }`
...
- `options.redact_pii`
  - Type: bool
  - Use when: you want to redact PII from returned content.
...
- `options.origin`
  - Type: String
  - Use when: you need an origin label for request telemetry.
  - Notes: auto-set to `"rust-sdk@{version}"` if not provided.
...
- `interact` → `Result<ScrapeExecuteResponse, FirecrawlError>`
  - `success: bool` is always present; `live_view_url`, `interactive_live_view_url`, `cdp_url`, `output`, `stdout`, `result`, `stderr`, `exit_code`, `killed`, and `error` are `Option` fields on the struct.
...
- `client.scrape_alexandria(calls, options)` executes Alexandria domain-tool calls.
- `client.find_tools(options)` discovers Alexandria tool catalogue.
- `client.search_papers(query, options)` and `client.search_github(query, options)` search specialized indexes.
- `client.scrape_with_schema(url, schema, prompt)` is a convenience method for JSON extraction with a schema.
