...
Crate: **`firecrawl`** on crates.io. The current SDK version is **2.21.0** (verify the latest release on crates.io before pinning).
...
- `SearchData`
  ...
  - `tools: Option<Vec<DiscoveredTool>>` — present when `domain_tools` is enabled; each entry describes a tool discovered via Alexandria.
...
- `options.domain_tools`
  - Type: bool
  - Use when: you want Alexandria domain-tool discovery included in results.

- `options.tool_detail`
  - Type: `ToolDetail`
  - Use when: you want to control the detail level of discovered tools.
  - Confirmed values: `Compact`, `Summary`, `Full`
...
- `options.country`
  - Type: String
  - Use when: you want to filter results by country code.

- `options.highlights`
  - Type: bool
  - Use when: you want to control whether search highlights are generated.
...
- `options.formats`
  ...
  - Confirmed values: `Markdown`, `Html`, `RawHtml`, `Links`, `Images`, `Screenshot`, `Summary`, `ChangeTracking`, `Json`, `Attributes`, `Branding`, `Audio`, `Video`, `Product`, `Menu`, `Question(...)`, `Highlights(...)`, `Query(...)`
...
- `options.redact_pii`
  - Type: bool
  - Use when: you want to redact personally identifiable information from results.
  - Notes: serialized as `redactPII` in the API request.

- `options.audit_metadata`
  - Type: `AuditMetadata`
  - Use when: you need SIEM logging metadata attached to the scrape.

- `options.domain_tools`
  - Type: bool
  - Use when: you want Alexandria domain-tool discovery included in scrape results.
...
- `options.origin`
  - Type: `Option<String>`
  - Use when: you need an origin tag for attribution.
  - Notes: auto-set to `"rust-sdk@{version}"` if None.
...
- `scrape_with_schema` is a convenience helper that wraps `scrape` with a typed JSON schema for structured extraction.
- `scrape_alexandria` is a method for querying the Alexandria tool-discovery index directly.
