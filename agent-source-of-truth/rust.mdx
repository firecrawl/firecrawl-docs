...
Canonical Firecrawl Rust source of truth for agents. Generated from SDK source (`firecrawl` crate **v2.18.0**) and the v2 OpenAPI spec.
...
Crate: **`firecrawl`** on crates.io. The current SDK version is **2.18.0**.
...
- `options.formats`
  ...
  - Confirmed values: `Markdown`, `Html`, `RawHtml`, `Links`, `Images`, `Screenshot`, `Summary`, `ChangeTracking`, `Json`, `Attributes`, `Branding`, `Product`, `Menu`, `Audio`, `Video`
  - Object-style format variants: `Question(QuestionFormat)`, `Highlights(HighlightsFormat)`
...
- `options.parsers`
  ...
  - Confirmed values:
    ...
    - `ParserConfig::Pdf { parser_type: "pdf", mode, max_pages, pages, blocks, page_markers }`
...
- `options.lockdown`
  - Type: bool
  - Use when: you want to serve only previously cached results with no outbound requests.
...
- Error types: `FirecrawlError::Misuse` for SDK-side validation failures.
