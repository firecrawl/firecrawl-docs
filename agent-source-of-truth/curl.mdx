...
- `timeout`
  - Type: number
  - Use when: you need a timeout in milliseconds. Large PDFs that outlive this window keep processing server-side and return HTTP 408 with `code: "SCRAPE_TIMEOUT"` and `details.state: "processing_continues"`. Honor `Retry-After` and retry the **same URL** — the retry attaches to the in-flight job and returns the finished result without re-processing.
