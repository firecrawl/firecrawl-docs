...
The hosted Firecrawl MCP keyless endpoint exposes exactly **Search, Scrape, and Parse** without an API key. Other hosted MCP tools require an account connection or an API key. Research endpoints can also be used without an API key on Firecrawl Cloud where the research index is enabled. No other endpoints (crawl, extract, map, batch scrape, etc.) are available without a key.
...
- A maximum number of **credits** per day. Operations cost different amounts of credits (for example, JSON extraction costs more than a basic scrape), so heavier usage reaches the credit cap sooner.
