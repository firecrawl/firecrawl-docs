...
For official Firecrawl clients, the CLI, SDKs, and REST API, keyless access also includes **Interact**. Research and developer search endpoints can be used without an API key on Firecrawl Cloud where those indexes are enabled. No other endpoints (crawl, extract, map, batch scrape, etc.) are available without a key.
