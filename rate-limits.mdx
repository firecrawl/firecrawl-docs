...
The `/browser` endpoints use a separate concurrency pool from scrape/crawl. Each team can have up to 20 active browser sessions at a time. If you exceed this limit, new session requests will return a `429` status code until existing sessions are destroyed. See [Browser](/features/browser#rate-limits) for more details.
