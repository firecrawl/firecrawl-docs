...
Each team has a maximum number of jobs that can be waiting in the concurrency queue at any time. If you exceed this limit, new jobs will be rejected with a `429` status code until existing jobs complete. This prevents unbounded queue growth.
...
For larger plans with custom concurrency limits, the max queued jobs is 20 times your concurrency limit, capped at 2,000,000.
