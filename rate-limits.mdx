...
The `/browser` endpoints have their own session limit. Each team can have up to 20 active browser sessions at a time. If you exceed this limit, new session requests will return a `429` status code until existing sessions are destroyed.

<Note>
Despite the naming, the “concurrent browsers” limit does not actually apply to the `/browser` endpoint itself, but it applies to internal scrapes performed as part of browser session execution. See [Browser](/features/browser#rate-limits) for more details.
</Note>
