...
Keyless usage is free. Each IP address gets two daily caps, one on **requests** and one on **credits**, and exceeding either returns a `429`. Operations cost different amounts of credits, so heavier usage reaches the credit cap sooner. Both caps are shared across every keyless endpoint.

Your caps reset 24 hours after your first keyless request, not at midnight. When a `429` arrives, read `retry_after_seconds` in the body to see how long is left. Keyless access allows 2 concurrent browsers, the same as the Free plan.
...
Firecrawl sends no `Retry-After` header and no `x-ratelimit-*` headers on any response. Read the `429` body instead. Its `reason` field names the cap you hit, `requests` or `credits`, and `retry_after_seconds` counts the seconds until the reset.
