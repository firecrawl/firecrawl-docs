...
# Remove Firecrawl from this project; a paid plan is cancelled with it
...
Removing the resource cancels its paid plan immediately, so nothing keeps billing after the resource is gone. The Firecrawl account, its team, and the API key stay; add the resource again to keep using them, and pick a plan again if you want one.
...
- **Removing the resource cancels the plan.** `stripe projects remove firecrawl` cancels the subscription and the plan right away; there is no billing period to run out. Downgrading to Free with `stripe projects upgrade firecrawl` does the same without removing the resource.
