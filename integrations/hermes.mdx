---
title: "Hermes Agent"
description: "Use Firecrawl as the default web search, extract, and browser backend in Hermes Agent"
---
...
Hermes Agent is an open-source terminal and desktop agent from Nous Research. It ships with model-callable web tools:
...
Firecrawl is the **default** provider for both (search + extract). You can also run Hermes browser automation through Firecrawl cloud mode.
...
With a paid [Nous Portal](https://portal.nousresearch.com/) subscription, web search and extract can run through Hermes **Tool Gateway** with managed Firecrawl — no separate Firecrawl key required:
...
When `FIRECRAWL_API_URL` is set, the API key is optional if your instance has auth disabled.
