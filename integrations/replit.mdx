...
<Steps>
  ...
  <Step title="Choose billing">
    Keep Replit's default configuration (no Firecrawl account needed), or open **Manage → Custom configurations** and either paste your Firecrawl API key or claim the 10,000-credit offer, which provisions a new key.
  </Step>

  <Step title="Verify the connection">
    In an app in the connected workspace, ask Agent:

    ```text
    Use the Firecrawl connector to scrape https://example.com.
    Return only the page title and first paragraph.
    Do not install an SDK or ask me for an API key.
    ```

    The connection is working when Agent uses Firecrawl, returns **Example Domain** with the page's first paragraph, and does not ask you to supply an API key in chat or application code.
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Claim partner credits" icon="gift">
    ...
    - **Firecrawl account:** A partner-associated account is provisioned.
...
<Tip>
**Launch offer:** Under **Manage → Custom configurations**, claim **10,000 free Firecrawl credits** to provision a **new** key. The offer applies once per user per partner, expires 3 months after it is applied, and uses Firecrawl's Free-plan limits. Pasting an existing key does not add the offer to that key. Default Replit billing does not include it. See [Partner credits](/partner-credits) for full terms.
</Tip>
...
- Ask Agent to use the **Firecrawl connector** explicitly rather than installing an SDK.
- If you intend to use your own key, add it under **Manage → Custom configurations** instead of pasting it into the prompt.

### The credit-claim option is unavailable

- Confirm you are viewing Firecrawl under **Manage → Custom configurations**.
