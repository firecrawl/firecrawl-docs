...
This configuration uses the keyless Firecrawl toolset. For an account-connected setup see [Human MCP](/mcp-server/human-mcp), or for an unattended setup see [Agent MCP](/mcp-server/agent-mcp); keep API keys in an Authorization header or secure-secret store, never in a URL.
