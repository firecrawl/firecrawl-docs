...
OpenCode reads config from `~/.config/opencode/opencode.json` (global) or `./opencode.json` in your project. Add:
...
```json
{
  ...
  "mcp": {
    "firecrawl": {
      "type": "local",
      "command": ["npx", "-y", "firecrawl-mcp@3.23.6"],
      "environment": {
...
This configuration uses the keyless Firecrawl toolset. For browser sign-in, use [OAuth](/mcp-server/human-mcp). For unattended use, follow the [API-key guide](/mcp-server/agent-mcp#add-an-api-key). Keep API keys in an environment variable or secret store, never in a URL.
