...
- [Try keyless MCP](/mcp-server/agent-mcp#try-keyless) for Search, Scrape, and Parse without an account or key.
- [Configure an API key](/mcp-server/agent-mcp#add-an-api-key) with `bearer_token_env_var` for unattended use. Keep the secret outside agent chat and source control.
