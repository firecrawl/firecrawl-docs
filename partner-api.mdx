...
There are two integration types. Both provision accounts the same way through the endpoints below, with the same partner key; they differ in who pays for the user's usage:
...
You choose the integration type when you create the integration. It is not easy to change afterwards, so we recommend reviewing both options carefully before you accept the agreement. See the [Partner API page](https://www.firecrawl.dev/partner-api) for an overview.
...
Gateway is an integration type, not a separate API. The same partner key and the same endpoints apply. What changes for a Gateway integration:
