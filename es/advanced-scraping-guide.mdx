...
description: "Configura opciones de scraping, acciones del navegador, crawling, mapping y el endpoint del agente con todas las capacidades de la API de Firecrawl."
og:title: "Guía avanzada de scraping | Firecrawl"
og:description: "Configura opciones de scraping, acciones del navegador, crawling, mapping y el endpoint del agente con todas las capacidades de la API de Firecrawl."
---
...
| `screenshot` | `fullPage?: boolean`, `quality?: number`, `viewport?: { width, height }` | Toma una captura de pantalla. Máximo una por solicitud. La resolución máxima del viewport es 7680×4320. |
...
|-----------|------|---------|-------------|
| `parsers` | `array` | `["pdf"]` | `["pdf"]` para procesar (1 crédito/página). `[{"type":"pdf","maxPages":10}]` para limitar las páginas. `[]` para omitir el procesamiento y devolver base64 (1 crédito fijo). |
...
| `scrape` | _(ninguno)_ | Captura el HTML de la página actual en este punto de la secuencia de acciones. |
...
| Parámetro | Tipo | Predeterminado | Descripción |
|-----------|------|----------------|-------------|
| `prompt` | `string` | _(required)_ | Instrucciones en lenguaje natural que describen qué datos extraer (máx. 10.000 caracteres). |
| `urls` | `array` | — | URLs a las que se restringirá el agente. |
| `schema` | `object` | — | Esquema JSON para estructurar los datos extraídos. |
| `maxCredits` | `number` | `2500` | Máximo de créditos que el agente puede consumir. |
