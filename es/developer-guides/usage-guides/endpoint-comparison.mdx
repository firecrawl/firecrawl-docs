...
<Info>
  **Vista previa de investigación**: Agent está en acceso anticipado. Seguirá mejorando de forma significativa con el tiempo. [Envía tus comentarios →](mailto:product@firecrawl.com)
</Info>
...
<Tip>
  `/extract` ahora admite extraer sin incluir URLs: basta con proporcionar un prompt. Sin embargo, para este caso de uso, `/agent` ofrece mejores resultados.
</Tip>
...
Usa este diagrama de flujo para elegir el endpoint más adecuado:
...
└─ SÍ
    ...
    │
    └─ ¿Múltiples páginas? → Usa /agent con URLs
                              (o /scrape con procesamiento por lotes)
```
