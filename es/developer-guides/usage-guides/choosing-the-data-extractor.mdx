...
* **Modelos disponibles**: `spark-1-pro` (predeterminado, mayor precisión), `spark-1-mini` (60% más económico que Pro) y `spark-2` (el más económico y rápido)
...
result = app.agent(
    ...
    schema=schema,
    model="spark-2"  # "spark-1-pro" es el predeterminado; "spark-1-mini" también está disponible
)
```
