...
Puedes ajustar el tiempo de espera del proceso de scraping usando el parámetro `timeout` en milisegundos.
...
Para rastrear varias páginas, puedes usar el endpoint `/crawl`. Este endpoint te permite especificar una URL base que quieres rastrear y todas las subpáginas accesibles se rastrearán automáticamente.
...
Al usar el endpoint `/crawl`, puedes personalizar el comportamiento del rastreador con el parámetro `crawlerOptions`. Estas son las opciones disponibles:
...
Puedes combinar los parámetros `pageOptions` y `crawlerOptions` para personalizar por completo el comportamiento del rastreador.
...
<div id="extractor-options-crawler-options">
  ## Opciones del extractor + Opciones del crawler
</div>
