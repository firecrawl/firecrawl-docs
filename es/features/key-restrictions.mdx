...
Algunas reglas hacen que la lista de permitidos sea práctica:
...
* **Los endpoints de cuenta y metadatos siempre son accesibles** (por ejemplo, `/v2/team/*`, `/v2/concurrency-check`), por lo que la gestión interna del SDK sigue funcionando independientemente de la lista de permitidos.
* **Los endpoints que hacen scraping internamente están controlados por su propio grupo.** `extract` y `agent` obtienen páginas como parte de su trabajo, por lo que requieren `extract` / `agent` en la lista de permitidos; permitir solo `scrape` no les da acceso.
...
* Las restricciones de clave son independientes de las restricciones basadas en IP; una clave puede tener ambas.