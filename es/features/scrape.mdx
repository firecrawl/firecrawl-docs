...
import ScrapeQueryPython from "/snippets/es/v2/scrape/query/python.mdx";
import ScrapeQueryNode from "/snippets/es/v2/scrape/query/js.mdx";
import ScrapeQueryCURL from "/snippets/es/v2/scrape/query/curl.mdx";
...
<Info>
  Cada scrape consume 1 crédito. Se aplican créditos adicionales para ciertas opciones: el modo JSON cuesta 4 créditos adicionales por página, el formato de consulta cuesta 4 créditos adicionales por página, el proxy mejorado cuesta 4 créditos adicionales por página, el procesamiento de PDF cuesta 1 crédito por página de PDF y la extracción de audio cuesta 4 créditos adicionales por página.
</Info>
...
El formato `query` te permite hacer una pregunta en lenguaje natural sobre la página. Firecrawl hace scraping de la página y ejecuta un LLM sobre el contenido, devolviendo la respuesta en el campo `answer` de la respuesta. Úsalo para hacer una consulta puntual sobre una URL sin tener que escribir un JSON schema.

<Info>
  El formato `query` cuesta 5 créditos por página (1 base + 4 adicionales por la llamada al LLM).
</Info>

Opciones dentro del objeto de formato:

* `prompt` (obligatorio): la pregunta que se debe responder. Máximo 10.000 caracteres.
* `mode` (opcional): `"freeform"` responde la pregunta usando el contenido de la página. `"directQuote"` devuelve el texto fuente relevante de la página. El valor predeterminado es `"freeform"`.

Puedes combinar `query` con otros formatos; por ejemplo, solicita `markdown` y `query` juntos para obtener tanto el contenido de la página como la respuesta en una sola llamada.
...
El formato `query` también está disponible en `/search` mediante `scrapeOptions`, lo que ejecuta la misma consulta de preguntas y respuestas en cada resultado de búsqueda.
