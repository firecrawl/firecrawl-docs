...
De forma predeterminada, cada sesión de navegador comienza desde cero. Con `persistentSession`, puedes guardar cookies, localStorage y otros estados del navegador para que se conserven en sesiones futuras. Esto es útil para seguir conectado, mantener tus preferencias o reanudar flujos de trabajo de varios pasos.
...
| `writeMode` | `"readwrite"` | `"readwrite"` guarda los cambios. `"readonly"` carga datos existentes sin escribir, útil cuando necesitas varios lectores concurrentes. |

<Note>
  Solo una sesión puede escribir en un perfil a la vez. Si otra sesión ya está escribiendo, recibirás un error `409`. Aun así puedes abrir el mismo perfil en modo `"readonly"` o intentarlo de nuevo más tarde.
</Note>
