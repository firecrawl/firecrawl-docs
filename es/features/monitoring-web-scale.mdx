...
| `maxResults`     | `number`                                         | Total de resultados que se evalúan por comprobación, `1`–`50`. El valor predeterminado es `10`. Este es un límite combinado para todas las `queries` (los resultados primero se combinan y se deduplican), no un límite por consulta; por eso, una consulta individual puede aportar menos resultados, o ninguno, si otras consultas llenan antes el límite. |
...
<div id="writing-good-goals-and-queries">
  ## Cómo escribir buenos goals y queries
</div>

La calidad de un monitor a escala web depende de dos factores que cumplen funciones distintas:

* **`queries` controlan el recall** — lo que recupera cada búsqueda. Si son demasiado específicas, los eventos reales nunca afloran; si son demasiado amplias, el evaluador gasta credits filtrando ruido.
* **`goal` controla la precisión** — qué resultados recuperados realmente generan alertas. El evaluador evalúa cada resultado nuevo frente al goal, así que el goal es lo que separa una coincidencia real de un resultado relacionado con el tema, pero irrelevante en la práctica.

Ajusta ambos. Un goal perfecto no puede generar una alerta sobre un resultado que las queries nunca recuperaron, y unas queries amplias combinadas con un goal vago producen alertas constantes de poco valor.
...
* Mantén cada query acotada (aproximadamente de 2 a 6 términos). Una query amplia suele funcionar mejor que varias específicas — las queries adicionales dividen el presupuesto de `maxResults` sin ampliar la cobertura.
* Usa una query por cada tema *distinto*. Un mismo tema con varias facetas (&quot;launches, benchmarks, docs&quot;) sigue siendo una sola query; divídela solo cuando el goal mencione entidades realmente distintas (por ejemplo, &quot;OpenAI, Anthropic, and Google&quot;).
...
* Desambigua términos polisémicos: *&quot;Firecrawl (the web scraping API)&quot;* evita que el evaluador se desvíe hacia equipos contra incendios.
* Añade `Ignore ...` solo para no coincidencias específicas de la intención: *&quot;Ignore opinion pieces, tutorials, and unrelated AI news.&quot;* No repitas ruido genérico — el evaluador ya gestiona el formato, los parámetros de seguimiento y los cambios por reindexación.
* Si la intención es amplia, mantenla amplia. Los goals demasiado restrictivos suprimen coincidencias reales.

**Cómo es un monitor saludable.** Un monitor a escala web bien ajustado no suele informar nada — la mayoría de las comprobaciones devuelven `new: 0` — y solo genera alertas cuando aparece algo realmente nuevo y alineado con el goal. Lee el resumen de la comprobación y el `searchStatus` de cada resultado (consulta [Statuses and dedup](#statuses-and-dedup)) para saber si está bien ajustado:

* Un flujo constante de resultados `ignored` significa que las queries recuperan ruido que luego el goal rechaza — ajusta más las queries (o reduce `searchWindow`) para dejar de pagar por evaluar resultados que nunca generarán alertas.
* `watching` frecuente significa que el goal es ambiguo — afina los criterios de coincidencia para que el evaluador pueda decidir.
* Largos periodos sin resultados sobre un tema activo significan que las queries son demasiado específicas o que `searchWindow` es demasiado estrecho — amplía los términos o ensancha la ventana.
* Las alertas que el usuario descarta significan que el goal es demasiado amplio — añade un `Ignore ...` específico para la intención.

El resultado ideal es una alta precisión con suficiente recall: cada alerta merece atención y no se pierde nada importante.
