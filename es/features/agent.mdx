...
| `maxCredits` | number | No | Número máximo de créditos que se pueden usar en esta tarea del agente. De forma predeterminada es **2.500** si no se especifica. Si se alcanza el límite, la tarea falla y **no se devuelve ningún dato**, aunque los créditos consumidos por el trabajo realizado se siguen cobrando. |
...
El [Agent Playground](https://www.firecrawl.dev/app/agent) admite la carga de archivos CSV para el procesamiento por lotes. Sube un CSV que contenga tus datos de entrada (por ejemplo, nombres de empresas, URL o cualquier entidad), escribe un prompt que describa qué datos quieres que el agente obtenga para cada fila, define tus campos de salida y ejecútalo: el agente procesa cada fila en paralelo y completa los resultados.
