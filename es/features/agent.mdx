...
Firecrawl Agent ofrece tres modelos. **Spark 2** es el modelo predeterminado: más económico y rápido que cualquiera de los modelos Spark 1, con una precisión comparable. Ambos modelos Spark 1 siguen disponibles y sin cambios.
...
| `spark-1-pro`  | Estándar                          | Mayor            | Investigación compleja, extracciones críticas   |

<Tip>
  **Spark 2 es el modelo predeterminado**, por lo que puedes omitir el parámetro `model`: cuesta menos y finaliza más rápido que ambos modelos Spark 1 en la mayoría de las tareas. Los modelos Spark 1 siguen disponibles: entre ellos, empieza con Mini y cambia a Pro solo para investigación compleja en múltiples dominios o cuando la precisión sea crítica.
</Tip>
...
`spark-2` es nuestro modelo de agente más reciente y el predeterminado para todas las solicitudes. Gestiona toda la gama de tareas que antes requerían elegir entre Mini y Pro, por lo que no es necesario sacrificar precisión por costo.
...
`spark-2` es el modelo predeterminado, por lo que el parámetro `model` es opcional. Pásalo para seleccionar un modelo diferente:
...
| `model`      | string | No          | Modelo a utilizar: `spark-2` (predeterminado), `spark-1-mini` o `spark-1-pro`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
