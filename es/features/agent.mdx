...
Firecrawl Agent ofrece tres modelos. **Spark 1 Pro** es el modelo predeterminado. **Spark 2** es más económico y rápido que cualquiera de los modelos Spark 1, con una precisión comparable; configura `model` como `spark-2` para usarlo.

| Modelo         | Costo                             | Precisión        | Ideal para                                                     |
| -------------- | --------------------------------- | ---------------- | -------------------------------------------------------------- |
| `spark-2`      | El más bajo                       | Comparable a Pro | Bajo costo y tiempo de ejecución corto                         |
| `spark-1-mini` | **Un 60 % más económico** que Pro | Estándar         | Cargas de trabajo de Spark 1 sensibles al costo                |
| `spark-1-pro`  | Estándar                          | Mayor            | Investigación compleja, extracciones críticas (predeterminado) |

<Tip>
  **Spark 1 Pro es el modelo predeterminado**, por lo que una solicitud sin el parámetro `model` usa `spark-1-pro`. Configura `model` como `spark-2` para obtener un menor costo y un tiempo de ejecución más corto con una precisión comparable. Configura `model` como `spark-1-mini` para obtener un costo un 60 % menor en tareas simples de Spark 1.
</Tip>
...
`spark-2` es nuestro modelo de agente más reciente. Gestiona toda la gama de tareas que antes requerían elegir entre Mini y Pro, por lo que no es necesario sacrificar precisión por costo. Establece `model` en `spark-2` para usarlo.
...
`spark-1-pro` es el modelo predeterminado, por lo que el parámetro `model` es opcional. Pásalo para seleccionar un modelo diferente:
...
| `model`      | string | No          | Modelo a utilizar: `spark-1-pro` (predeterminado), `spark-1-mini` o `spark-2`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
