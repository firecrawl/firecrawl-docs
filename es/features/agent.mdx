---
title: "Agent"
description: "La siguiente evolución de /extract: extracción de datos web más rápida, confiable y rentable"
og:title: "Agent | Firecrawl"
og:description: "La siguiente evolución de /extract: extracción de datos web más rápida, confiable y rentable"
icon: "robot"
sidebarTitle: "Agent"
---
...
El endpoint `/agent` es la siguiente evolución de nuestro endpoint [`/extract`](/es/features/extract). Utiliza agentes de IA para recopilar y estructurar datos de forma inteligente en toda la web. A diferencia de `/extract`, Agent solo necesita un prompt; las URL son opcionales, lo que lo hace extremadamente flexible para una amplia variedad de consultas.

<Info>Agent es el sucesor de `/extract`: más fiable, más rápido y más económico.</Info>

<div id="why-agent-over-extract">
  ## ¿Por qué Agent en lugar de Extract?
</div>

Agent se basa en todo lo bueno de `/extract` y lo lleva aún más lejos:

* **Solo con prompt**: Simplemente describe lo que necesitas, no se requieren URLs
* **Más inteligente**: Los agentes de IA entienden el contexto y encuentran datos relevantes automáticamente
* **Más rápido**: Procesamiento optimizado para obtener resultados más rápidos
* **Más rentable**: Un uso más eficiente de los recursos se traduce en menores costes
* **Flexible**: Funciona con una amplia variedad de consultas y casos de uso

<div id="using-agent">
  ## Uso de `/agent`
</div>

El único parámetro obligatorio es `prompt`. Solo tienes que describir qué datos quieres extraer.

<div id="basic-usage-prompt-only">
  ### Uso básico (solo con prompt)
</div>
...
<div id="with-schema-optional">
  ## Con esquema (opcional)
</div>

Para obtener salida estructurada, proporciona un esquema JSON. Los SDK son compatibles con Pydantic (Python) y Zod (Node) para definiciones de esquemas con tipado seguro:
