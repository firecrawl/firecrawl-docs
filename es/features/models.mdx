...
| -------------- | ------------------------------ | ---------------- | --------------------------------------------------------- |
| `spark-2`      | El más bajo                    | Comparable a Pro | Todas las tareas (predeterminado)                         |
...
<Tip>
  **Spark 2 es el modelo predeterminado**, por lo que puedes omitir el parámetro `model`: cuesta menos y finaliza más rápido que ambos modelos de Spark 1 en la mayoría de las tareas. Los modelos de Spark 1 siguen disponibles: empieza con Mini y cambia a Pro solo para investigaciones complejas en varios dominios o cuando la precisión sea crítica.
</Tip>

<div id="spark-2-default">
  ## Spark 2 (Predeterminado)
</div>

`spark-2` es nuestro modelo de agente más reciente y el predeterminado para todas las solicitudes. Elimina la disyuntiva entre precisión y costo: es más económico y rápido que ambos modelos Spark 1, iguala la precisión de Spark 1 Pro y completa una mayor proporción de ejecuciones.
...
* No has configurado `model`; esto es lo que obtienes
...
`spark-2` es el modelo predeterminado, por lo que el parámetro `model` es opcional. Pásalo para seleccionar otro modelo:
