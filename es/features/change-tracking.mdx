...
Incluye tanto `markdown` como `changeTracking` en el array `formats`. El formato `markdown` es obligatorio porque changeTracking compara las páginas según su contenido en markdown.
...
Para operaciones asíncronas como `crawl` y *batch scrape*, usa [webhooks](/es/webhooks/overview) para recibir resultados de changeTracking a medida que llegan en lugar de hacer *polling*.
