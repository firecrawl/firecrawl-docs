...
Una vez que tengas una clave de API, continúa con [CLI](#cli) a continuación.
...
<Card title="MCP Server" icon="plug" href="/es/mcp-server/human-mcp">
  Consulta la configuración alojada para Codex, Claude Desktop, Claude Code, Cursor y otros
  clientes compatibles. Para Windsurf, usa el
  [inicio rápido de Windsurf](/es/quickstarts/windsurf). Para una configuración de agente sin supervisión o de CI,
  consulta [Agent MCP](/es/mcp-server/agent-mcp).
</Card>
...
<Steps>
  ...
  <Step title="Servidor de documentación MCP">
    Para un enfoque estructurado con herramientas MCP, conecta el Firecrawl MCP Server en Codex, Cursor, Claude Code, Claude Desktop u otro cliente compatible. Consulta [MCP para personas](/es/mcp-server/human-mcp) para una configuración alojada interactiva o [MCP para agentes](/es/mcp-server/agent-mcp) para un agente sin supervisión. Los usuarios de Windsurf deben seguir el [inicio rápido de Windsurf](/es/quickstarts/windsurf).
  </Step>
