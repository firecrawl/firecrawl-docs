...
Esta página es una **capa de decisión**: te dirige a la ruta de credenciales correcta. La ejecución está en la documentación canónica enlazada más abajo, no aquí.

Firecrawl admite dos rutas de credenciales: incorporación con CLI/navegador con una persona supervisando cuando alguien necesita iniciar sesión o crear una clave API en el dashboard (Ruta A, predeterminada), y registro de WorkOS ID-JAG solo cuando tu plataforma de agentes puede emitir un ID-JAG (Ruta B).

**Regla de decisión:** Si no tienes ID-JAG o no estás seguro → **Ruta A (predeterminada)**. Si puedes usar ID-JAG → **Ruta B**.

**Cómo podrías llegar aquí:**

* **API `401`**: una llamada no autenticada a `https://api.firecrawl.dev` devuelve `WWW-Authenticate: Bearer resource_metadata="https://www.firecrawl.dev/.well-known/oauth-protected-resource"`. Sigue esa cadena de descubrimiento en el Paso 1 de [`auth.md`](https://www.firecrawl.dev/auth.md). Si tu plataforma puede completar el registro de WorkOS ID-JAG → **Ruta B**. En caso contrario → **Ruta A (predeterminada)**. No necesitas esta página.
* **Docs**: llegaste aquí. Usa la regla de decisión anterior.
* **URL directa**: si ya tienes `auth.md` o `SKILL.md`, recupera y sigue ese documento.

<CardGroup cols={2}>
  <Card title="Ruta A — Persona + asistencia del agente (predeterminada)" icon="user" href="https://www.firecrawl.dev/agent-onboarding/SKILL.md">
    **Ruta A (predeterminada).** Úsala cuando no puedas emitir un WorkOS ID-JAG o no estés seguro. Inicio de sesión en navegador, CLI `--browser`, instalación de skills y MCP, o una clave API creada por una persona desde el dashboard.
  </Card>

  <Card title="Ruta B — Registro agéntico (WorkOS ID-JAG)" icon="robot" href="/es/ai-onboarding/agent-auth">
    **Solo si puedes usar ID-JAG.** Tu plataforma de agentes debe poder emitir un WorkOS ID-JAG para `aud=https://api.firecrawl.dev/`. Consulta [Agent Auth](/es/ai-onboarding/agent-auth) y luego sigue la especificación canónica de [`auth.md`](https://www.firecrawl.dev/auth.md).
  </Card>
</CardGroup>

<Card title="Obtén tu clave API (Ruta A)" icon="key" href="https://www.firecrawl.dev/app/api-keys">
  Regístrate y obtén una clave API desde el dashboard: es la ruta predeterminada cuando no usas el registro de WorkOS ID-JAG.
</Card>

Una vez que tengas un token bearer, continúa con [Skills + CLI](#skills-cli) más abajo.
...
* Ruta A — Persona + asistencia del agente (predeterminada): [`firecrawl.dev/agent-onboarding/SKILL.md`](https://www.firecrawl.dev/agent-onboarding/SKILL.md)
* Ruta B — Registro de máquina (ID-JAG): [`firecrawl.dev/auth.md`](https://www.firecrawl.dev/auth.md)
...
<Steps>
  <Step title="Enrutamiento de credenciales">
    Empieza en [Obtener credenciales](/es/ai-onboarding#get-credentials) para elegir la Ruta A (predeterminada, persona + asistencia de agente) o la Ruta B (WorkOS ID-JAG). Para el registro de ID-JAG, consulta [Agent Auth](/es/ai-onboarding/agent-auth) y la especificación canónica de [`auth.md`](https://www.firecrawl.dev/auth.md).
  </Step>
