...
* `--all` instala las skills de Firecrawl en todos los agentes de código con IA detectados en el equipo
...
<Steps>
  ...
  <Step title="Integración en aplicaciones (skills de Build)">
    ...
    El agente responde una pregunta clave: *¿qué debe hacer Firecrawl dentro del producto?*; y las skills de Build enrutan a `/search`, `/scrape`, `/interact`, `/crawl`, `/map` o `/agent` según corresponda.
  </Step>
...
La definición completa de onboarding está disponible en [`firecrawl.dev/agent-onboarding/SKILL.md`](https://www.firecrawl.dev/agent-onboarding/SKILL.md); los agentes pueden obtenerla directamente para incorporarse por su cuenta.

<Card title="Skills + CLI" icon="terminal" href="/es/sdks/cli">
  Instala la CLI y las skills, autentícate y ejecuta comandos de scrape, search, crawl,
  interact, map, extract y agent desde la terminal.
</Card>
