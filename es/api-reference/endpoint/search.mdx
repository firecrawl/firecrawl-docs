...
El endpoint de búsqueda combina la búsqueda web (SERP) con las capacidades de scraping de Firecrawl para devolver el contenido completo de la página para cualquier consulta.

Incluye `scrapeOptions` con `formats: [{"type": "markdown"}]` para obtener el contenido completo en markdown de cada resultado de búsqueda; de lo contrario, por defecto solo obtendrás los resultados del SERP (url, title, description). También puedes usar otros formatos como `{"type": "summary"}` para obtener contenido condensado.
