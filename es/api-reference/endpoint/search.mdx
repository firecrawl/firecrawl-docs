...
Incluye `scrapeOptions` con `formats: [{"type": "markdown"}]` para obtener el contenido completo en markdown de cada resultado de búsqueda; de lo contrario, por defecto solo obtendrás los resultados del search (url, title, description). También puedes usar otros formatos como `{"type": "summary"}` para obtener contenido condensado.
