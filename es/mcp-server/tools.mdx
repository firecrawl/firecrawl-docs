...
Inicia un rastreo con opciones avanzadas. Esta llamada MCP permanece bloqueada mientras el servidor consulta el estado del rastreo hasta que finaliza y, después, devuelve el estado final y los datos recopilados.
...
Respuesta de ejemplo de MCP (`content[0].text` contiene JSON serializado):
...
Procesa `content[0].text` como JSON para acceder a la carga útil extraída y decodificada.
...
Estas herramientas no forman parte de la oferta alojada sin clave. Para incidencias del repositorio, pull requests fusionadas y archivos README, usa [`firecrawl_developer_search`](#12b-developer-search-tool-firecrawl_developer_search).
...
Busca de forma independiente en el [índice para desarrolladores](/es/features/developer): incidencias, pull requests fusionadas y archivos README de repositorios de código públicos, junto con sitios de documentación seleccionados. Es de solo lectura y devuelve resultados ordenados por relevancia con los pasajes coincidentes.
