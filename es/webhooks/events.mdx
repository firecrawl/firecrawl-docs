...
| Field         | Description                                                |
| ------------- | ---------------------------------------------------------- |
| `creditsUsed` | Créditos consumidos estimados hasta el momento             |
| `action`      | La herramienta que se ejecutó (p. ej., `scrape`, `search`) |
| `input`       | Los parámetros proporcionados a la herramienta             |

<Note>
  El valor de `creditsUsed` en los eventos de `action` es una **estimación**. El recuento final y preciso de créditos solo está disponible en los eventos `completed`, `failed` o `cancelled`.
</Note>
