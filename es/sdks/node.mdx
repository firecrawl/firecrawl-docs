...
Extrae una sola URL y obtén datos estructurados de la página con el método `scrapeUrl`.
...
Rastrea un sitio web completo a partir de una sola URL con el método `crawlUrl`. Puedes establecer un límite de páginas, restringir el rastreo a dominios específicos y seleccionar formatos de salida. Consulta [Pagination](#pagination) para la paginación automática y manual.
...
Descubre todas las URL de un sitio web con el método `mapUrl`. Pasa una URL inicial y obtén una lista de las páginas detectadas.
