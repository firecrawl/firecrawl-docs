...
Busca, haz scraping, interactúa, rastrea, mapea y ejecuta trabajos de agentes directamente desde la terminal. La CLI de Firecrawl funciona por sí sola o con skills que los agentes de programación con IA, como Claude Code, Antigravity y OpenCode, pueden descubrir y usar automáticamente.
...
<Note>
  **`scrape`, `search` e `interact` funcionan sin iniciar sesión.** Si no tienes ninguna clave de API configurada, estos comandos usan el nivel gratuito sin clave: gratis, pero con límites por IP. Todos los demás comandos (crawl, map, agent, etc.) requieren una clave de API. [Regístrate para obtener una clave gratuita](https://firecrawl.dev) y consigue 1.000 créditos y límites más altos; la CLI la usará automáticamente una vez configurada. Consulta [Límites de tasa](/es/rate-limits#keyless-no-api-key).
</Note>
