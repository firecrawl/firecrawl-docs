...
<Note>
  **¿No tienes clave de API? El scraping, la búsqueda y la interacción siguen funcionando.** Cuando ejecutas el servidor localmente sin `FIRECRAWL_API_KEY`, las herramientas `firecrawl_scrape`, `firecrawl_search` y `firecrawl_interact` recurren al plan Free sin clave: gratis, pero con límite de tasa por IP, y no disponible para otras herramientas (crawl, extracción, mapeo, etc.). Configura `FIRECRAWL_API_KEY` para desbloquear todas las herramientas y límites más altos. Consulta [Límites de tasa](/es/rate-limits#keyless-no-api-key).
</Note>
