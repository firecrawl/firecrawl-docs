...
<Note>
  **¿No tienes clave de API? El scraping, la búsqueda, la interacción y el procesamiento funcionan en la URL remota sin clave.** Conéctate a `https://mcp.firecrawl.dev/v2/mcp` para usar el plan Free sin clave: gratis, pero con límite de tasa por IP, y no disponible para otras herramientas (crawl, extracción, mapeo, etc.). Configura `FIRECRAWL_API_KEY` para desbloquear todas las herramientas y límites más altos. Consulta [Límites de tasa](/es/rate-limits#keyless-no-api-key).
</Note>
...
O conéctate sin una clave de API para empezar: scrape, search e interact y procesar funcionan en el plan Free remoto sin clave (limitado por IP):
...
<div id="10-create-browser-session-firecrawl_browser_create">
  ### 10. Crear sesión de navegador (`firecrawl_browser_create`)
</div>
...
<div id="11-execute-code-in-browser-firecrawl_browser_execute">
  ### 11. Ejecutar código en el navegador (`firecrawl_browser_execute`)
</div>
...
<div id="12-delete-browser-session-firecrawl_browser_delete">
  ### 12. Eliminar sesión del navegador (`firecrawl_browser_delete`)
</div>
...
<div id="13-list-browser-sessions-firecrawl_browser_list">
  ### 13. Listar sesiones de navegador (`firecrawl_browser_list`)
</div>
...
<div id="14-interact-with-scraped-page-firecrawl_interact">
  ### 14. Interact con una página extraída (`firecrawl_interact`)
</div>
...
<div id="15-stop-interact-session-firecrawl_interact_stop">
  ### 15. Detener una sesión de Interact (`firecrawl_interact_stop`)
</div>
