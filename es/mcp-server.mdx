...
<Note>
  **¿No tienes clave de API? El scraping, la búsqueda y la interacción siguen funcionando.** Conéctate a la URL remota sin clave `https://mcp.firecrawl.dev/v2/mcp` (o ejecuta el servidor localmente sin `FIRECRAWL_API_KEY`), y las herramientas `firecrawl_scrape`, `firecrawl_search` y `firecrawl_interact` recurren al plan Free sin clave: gratis, pero con límite de tasa por IP, y no disponible para otras herramientas (crawl, extracción, mapeo, etc.). Configura `FIRECRAWL_API_KEY` para desbloquear todas las herramientas y límites más altos. Consulta [Límites de tasa](/es/rate-limits#keyless-no-api-key).
</Note>
...
Con una API key (desbloquea todas las herramientas y ofrece límites superiores):

```bash
https://mcp.firecrawl.dev/{FIRECRAWL_API_KEY}/v2/mcp
```

O conéctate sin una API key para empezar: scrape, search e interact funcionan en el plan Free sin clave (limitado por IP):
