...
Realiza scraping de varias URL de forma eficiente gracias a la limitación de velocidad integrada y al procesamiento en paralelo.
...
La respuesta incluye el ID de la operación para consultar su estado:
...
```json
{
  "content": [
    {
      "type": "text",
      "text": "Operación por lotes en cola con ID: batch_1. Usa firecrawl_check_batch_status para verificar el progreso."
    }
  ],
  "isError": false
}
...
<div id="3-check-batch-status-firecrawl_check_batch_status">
  ### 3. Comprobar el estado de un lote (`firecrawl_check_batch_status`)
</div>

Consulta el estado de una operación por lotes.
...
<div id="7-check-crawl-status-firecrawl_check_crawl_status">
  ### 7. Comprobar el estado del rastreo (`firecrawl_check_crawl_status`)
</div>
