...
- Investigación avanzada y scraping por lotes
...
Para ejecutar el servidor localmente usando transporte HTTP en streaming en lugar del transporte predeterminado `stdio`:

```bash
env HTTP_STREAMABLE_SERVER=true FIRECRAWL_API_KEY=fc-TU_CLAVE_API npx -y firecrawl-mcp
```

Usa la URL: http://localhost:3000/v2/mcp o https://mcp.firecrawl.dev/{FIRECRAWL_API_KEY}/v2/mcp
...
Para la instalación manual, añade el siguiente bloque JSON a tu archivo User Settings (JSON) en VS Code. Puedes hacerlo presionando Ctrl+Shift+P y escribiendo Preferences: Open User Settings (JSON).
...
**Nota:** Algunos usuarios han informado de problemas al agregar el servidor MCP a VS Code debido a que valida JSON con un formato de esquema obsoleto ([microsoft/vscode#155379](https://github.com/microsoft/vscode/issues/155379)).
Esto afecta a varias herramientas MCP, incluido Firecrawl.

**Solución temporal:** Desactiva la validación de JSON en VS Code para permitir que el servidor MCP se cargue correctamente.\
...
El servidor MCP sigue funcionando correctamente cuando se invoca mediante otras extensiones, pero el problema ocurre específicamente al registrarlo directamente en la lista de servidores MCP. Planeamos agregar recomendaciones una vez que VS Code actualice su validación de esquemas.
...
5. Introduce el endpoint del servidor MCP (sustituye `{YOUR_FIRECRAWL_API_KEY}` por tu clave de API real):

```
  https://mcp.firecrawl.dev/{TU_CLAVE_API_DE_FIRECRAWL}/v2/mcp
```

6. Configura **Server Transport** en **HTTP Streamable**
7. Configura **Authentication** en **None**
8. En **Tools to include**, puedes seleccionar **All**, **Selected** o **All Except**; esto expondrá las herramientas de Firecrawl (scrape, crawl, map, search, extract, etc.).
...
env HTTP_STREAMABLE_SERVER=true \
    FIRECRAWL_API_KEY=fc-TU_CLAVE_API \
    FIRECRAWL_API_URL=TU_INSTANCIA_FIRECRAWL \
    npx -y firecrawl-mcp
```

Esto iniciará el servidor en `http://localhost:3000/v2/mcp`, que puedes usar en tu flujo de trabajo de n8n como endpoint. La variable de entorno `HTTP_STREAMABLE_SERVER=true` es necesaria porque n8n requiere transporte HTTP.
...
- `sitemap`: Control del uso del mapa del sitio: "include", "skip" o "only"
- `includeSubdomains`: Si se deben incluir subdominios en el mapeo
- `limit`: Número máximo de URL a devolver
- `ignoreQueryParameters`: Si se deben ignorar los parámetros de consulta al mapear

**Ideal para:** Descubrir URL en un sitio web antes de decidir qué extraer; encontrar secciones específicas de un sitio.
**Devuelve:** Lista de URL encontradas en el sitio.
...
Consulta el estado de un job de rastreo.
...
**Devuelve:** Estado y progreso del trabajo de rastreo, incluidos los resultados si están disponibles.
...
Extrae información estructurada de páginas web usando capacidades de LLM. Admite extracción con IA en la nube y con LLM autoalojados.
...
```json
{
  ...
  "arguments": {
    ...
    "prompt": "Extrae información del producto incluyendo nombre, precio y descripción",
  ...
  "content": [
    {
      ...
      "text": {
        ...
        "price": 99.99,
        "description": "Esta es una descripción de producto de ejemplo"
      }
    }
  ],
  "isError": false
