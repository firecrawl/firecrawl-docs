---
title: "Búsqueda y scraping web con MCP en Windsurf"
description: "Añade scraping y búsqueda web a Windsurf en 2 minutos"
---

Añade capacidades de scraping y búsqueda web a Windsurf con Firecrawl MCP.

<div id="quick-setup">
  ## Configuración rápida
</div>

<div id="1-get-your-api-key">
  ### 1. Obtén tu clave de API
</div>

Regístrate en [firecrawl.dev/app](https://firecrawl.dev/app) y copia tu clave de API.

<div id="2-add-to-windsurf">
  ### 2. Añadir a Windsurf
</div>

Añade esto a `~/.codeium/mcp_config.json` (o usa **Configuración → Herramientas → Configuración de Windsurf → Añadir servidor** / **Ver configuración sin procesar**):

```json
{
  "mcpServers": {
    "firecrawl": {
      "command": "npx",
      "args": ["-y", "firecrawl-mcp"],
      "env": {
        "FIRECRAWL_API_KEY": "TU_API_KEY"
      }
    }
  }
}
```

Reemplaza `YOUR_API_KEY` por tu clave real de la API de Firecrawl.

<div id="3-restart-windsurf">
  ### 3. Reinicia Windsurf
</div>

¡Listo! Windsurf ya puede buscar y hacer scraping en la web.

<div id="quick-demo">
  ## Demostración rápida
</div>

Pruébalos en Windsurf:

**Búsqueda:**

```
Busca las últimas funcionalidades de Tailwind CSS
```

**Extracción:**

```
Extrae datos de firecrawl.dev y explica qué hace
```

**Obtener docs:**

```
Busca y extrae la documentación de autenticación de Supabase
```

Los agentes de IA de Windsurf usarán automáticamente las herramientas de Firecrawl.
