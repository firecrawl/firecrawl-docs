---
title: "Búsqueda y scraping web con MCP en Claude Code"
description: "Añade scraping y búsqueda web a Claude Code en 2 minutos"
---

Añade funciones de scraping y búsqueda web a Claude Code con Firecrawl MCP.

<div id="quick-setup">
  ## Configuración rápida
</div>

<div id="1-get-your-api-key">
  ### 1. Obtén tu clave de API
</div>

Crea una cuenta en [firecrawl.dev/app](https://firecrawl.dev/app) y copia tu clave de API.

<div id="2-add-firecrawl-mcp-server">
  ### 2. Añade el servidor MCP de Firecrawl
</div>

**Opción A: URL alojada en remoto (recomendada)**

```bash
claude mcp add --transport http firecrawl https://mcp.firecrawl.dev/v2/mcp-oauth
```

Después de añadir el servidor, usa `/mcp` en Claude Code para completar el inicio de sesión en Firecrawl.

**Opción B: Local (npx)**

```bash
claude mcp add firecrawl -e FIRECRAWL_API_KEY=your-api-key -- npx -y firecrawl-mcp@3.22.4
```

Reemplaza `your-api-key` por tu clave real de API de Firecrawl.

¡Listo! Ahora puedes buscar y hacer scraping de la web desde Claude Code.

<div id="quick-demo">
  ## Demostración rápida
</div>

Prueba lo siguiente en Claude Code:

**Buscar en la web:**

```
Search for the latest Next.js 15 features
```

**Extrae datos de una página:**

```
Scrape firecrawl.dev and tell me what it does
```

**Consulta la documentación:**

```
Find and scrape the Stripe API docs for payment intents
```

Claude usará automáticamente las herramientas de búsqueda y scraping de Firecrawl para obtener la información.
