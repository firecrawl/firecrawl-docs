...
Al usar el endpoint `/scrape`, puedes especificar opciones para **extraer información estructurada** del contenido de la página utilizando el parámetro `extract`. Estas son las opciones disponibles:
...
Cuando usas el endpoint `/scrape`, Firecrawl te permite realizar distintas acciones en una página web antes de extraer su contenido. Esto es especialmente útil para interactuar con contenido dinámico, navegar entre páginas o acceder a contenido que requiere interacción por parte del usuario.
...
Se usa para consultar el estado de un trabajo de rastreo y obtener su resultado.
...
Si el contenido es superior a 10 MB o si la tarea de rastreo aún se está ejecutando, la respuesta incluirá un parámetro `next`. Este parámetro es una URL a la siguiente página de resultados. Puedes usar este parámetro para obtener la siguiente página de resultados.
...
El endpoint `/map` es muy eficaz para identificar URLs que están contextualmente relacionadas con un sitio web determinado. Esta funcionalidad es crucial para comprender el entorno de enlaces contextuales de un sitio, lo que puede ayudar en gran medida en el análisis estratégico del sitio y la planificación de la navegación.
