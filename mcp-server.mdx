...
<Warning>
The dedicated account endpoint, deterministic keyless recovery contract, and account-connection management described below are arriving with the hosted MCP account-endpoint rollout. Today, use `/v2/mcp` keyless or with an API key header. The docs update must deploy after the account endpoint and recovery behavior are live.
</Warning>
...
The rollout contract makes anonymous initialization succeed deterministically. Keyless tool denials are returned as structured tool results rather than transport authentication challenges. Quota exhaustion returns a structured tool result or `429` with retry guidance.

On `/v2/mcp`, an invalid API key or unrecognized Bearer credential receives a correction-only session with no Firecrawl tools and a model-visible `CREDENTIAL_INVALID` result. A `401` with `error="invalid_token"` is reserved for recognizable expired or revoked OAuth tokens that can safely re-authenticate against the same resource. Anonymous/keyless capability and quota failures remain non-auth-shaped: they use structured tool results, or `429` for hard rate limiting.
...
<Warning>
The account endpoint and `https://firecrawl.dev/connect/mcp` currently return `404` in production. Until the coordinated rollout completes, use the keyless endpoint or configure an API key header; do not place a credential in the URL.
</Warning>
...
With the recovery rollout, when a keyless caller requests a tool that needs an account, the tool result includes two machine-readable recovery actions:

```json
{
  "code": "KEYLESS_TOOL_NOT_AVAILABLE",
  "message": "This tool is not available in keyless mode. If a person is present, connect a free Firecrawl account in the browser; no credit card is required and free accounts include monthly credits. For CI, servers, or unattended agents, configure an API key in the Authorization header. Then retry the same tool call.",
  "auth_mode": "keyless",
  ...
  "additional_unavailable_tool_count": 17,
  "next_actions": [
    {
...
The connect link is a human-facing setup page. During the initial rollout it does not authenticate the existing keyless session: follow its instructions to configure or re-add Firecrawl with `https://mcp.firecrawl.dev/v2/mcp-oauth`, then repeat the request through that account connection. If you instead add an API key as a Bearer header on the existing `/v2/mcp` connection, retry the original tool call there.
...
# With an API key, send it as an Authorization header
