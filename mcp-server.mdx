...
<Warning>
The dedicated account endpoint and recovery behavior described below are not live yet. Today, use `/v2/mcp` without credentials or with an API key in the `Authorization` header.
</Warning>

<CardGroup cols={3}>
  <Card title="Try it now" icon="bolt">
    Connect to the hosted keyless endpoint. No account or API key is required. Keyless mode exposes exactly **Search, Scrape, and Parse** and is rate-limited per IP.
  </Card>
  <Card title="Connect your account" icon="user">
    With the account-endpoint rollout, interactive connectors use a dedicated URL. Your client opens a browser so you can sign in, choose a team, approve access, and revoke the connection later.
  </Card>
  <Card title="Run unattended" icon="server">
    Use an API key for CI, servers, and other workloads that run without you. Send the key in the `Authorization` header, never in the URL.
  </Card>
</CardGroup>
