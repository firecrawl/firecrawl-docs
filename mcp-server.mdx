...
On `/v2/mcp`, a `401` with `error="invalid_token"` is reserved for callers that actually present an invalid, expired, or revoked Firecrawl API key or OAuth token. Anonymous/keyless capability and quota failures remain non-auth-shaped: they use structured tool results, or `429` for hard rate limiting.
