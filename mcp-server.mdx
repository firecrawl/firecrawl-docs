...
To connect the Firecrawl MCP server in n8n, use the remote hosted URL which works out of the box with n8n's MCP integration.

#### Step 1: Get your API Key

First, get your Firecrawl API key from [https://firecrawl.dev/app/api-keys](https://www.firecrawl.dev/app/api-keys)

#### Step 2: Use the Remote Hosted URL

In your n8n workflow, use the following MCP server URL:
...
#### Step 3: Configure the MCP Connection in n8n
...
4. Enter the MCP server URL: `https://mcp.firecrawl.dev/{YOUR_FIRECRAWL_API_KEY}/v2/mcp`
5. The server will automatically expose all available Firecrawl tools (scrape, crawl, map, search, extract, etc.)

#### Alternative: Self-Hosted Server

If you prefer to run your own MCP server locally or on your infrastructure, you'll need to enable HTTP transport mode:

```bash
env HTTP_STREAMABLE_SERVER=true FIRECRAWL_API_KEY=fc-YOUR_API_KEY npx -y firecrawl-mcp
```

This will start the server on `http://localhost:3000/v2/mcp`. You can then use this URL in your n8n workflow.

**Note:** When self-hosting, the `HTTP_STREAMABLE_SERVER=true` environment variable is required since n8n needs HTTP/SSE transport (the default stdio transport used by desktop applications won't work with n8n).
