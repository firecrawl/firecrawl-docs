...
> **Windows users:** If you see a `spawn npx ENOENT` error, Cursor may not be inheriting your system PATH. To fix this, replace `"command": "npx"` with the full path to `npx.cmd` (find it by running `where npx` in Command Prompt):
> ```json
> "command": "C:\\Program Files\\nodejs\\npx.cmd"
> ```
> Alternatively, bypass npx entirely by using the [remote hosted URL](#remote-hosted-url) with the `"url"` config format instead.
