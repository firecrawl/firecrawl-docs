...
{/* RELEASE GATE: Remove this Warning only after local integrated E2E, the protected production canary, the production legacy-token audit confirms that accepted tokens have an unambiguous MCP audience, real Claude OAuth connections, and a real ChatGPT test confirm that offline_access is advertised and accepted, a refresh token is issued, and refresh after access-token expiry succeeds. Before publishing, regenerate the GT localizations for every source page changed by this rollout. */}

<Warning>
  The dedicated account endpoint and recovery behavior described below are not
  live yet. Today, use `/v2/mcp` without credentials or with an API key in the
  `Authorization` header.
</Warning>

<CardGroup cols={3}>
  <Card title="Try it now" icon="bolt">
    Connect to the hosted keyless endpoint. No account or API key is required.
    Keyless mode exposes exactly **Search, Scrape, and Parse** and is
    rate-limited per IP.
  </Card>
  <Card title="Connect your account" icon="user">
    With the account-endpoint rollout, interactive connectors use a dedicated
    URL. Your client opens a browser so you can sign in, choose a team, approve
    access, and revoke the connection later.
  </Card>
  <Card title="Run unattended" icon="server">
    Use an API key for CI, servers, and other workloads that run without you.
    Send the key in the `Authorization` header, never in the URL.
  </Card>
</CardGroup>

### Try it now

Local coding agents such as Claude Code, Codex, Cursor, and OpenCode should start here:

```bash
https://mcp.firecrawl.dev/v2/mcp
```

Keyless connections expose Search, Scrape, and Parse. If a request needs an account-only tool, Firecrawl explains how to connect an account or add an API key. Quota errors include retry guidance.

If a configured credential is invalid, Firecrawl returns a credential error instead of silently falling back to keyless mode. Expired or revoked OAuth connections ask compatible clients to reconnect.

### Connect your account

After the account-endpoint rollout is live, ChatGPT, Claude web/desktop connectors, and other account-attached integrations should use:

```bash
https://mcp.firecrawl.dev/v2/mcp-oauth
```

This endpoint will have its own OAuth resource identity and will always ask an anonymous client to sign in. Compatible clients identify themselves automatically using Client ID Metadata Documents or Dynamic Client Registration, so users do not need to create an OAuth client manually. The endpoint also accepts Firecrawl API keys in an `Authorization` header.

### Run unattended

Send a Firecrawl API key to either hosted endpoint as a Bearer header. Keep the key in an environment variable and use the client's native environment binding rather than writing it into a shared configuration file. For example, the Firecrawl CLI can configure Codex without persisting the key:

```bash
export FIRECRAWL_API_KEY=fc-your-api-key
firecrawl setup mcp --agent codex --global --yes
```

Compatible existing OAuth tokens explicitly bound to `https://mcp.firecrawl.dev/v2/mcp` remain accepted there. Tokens with a missing or ambiguous audience fail closed. After rollout, new interactive connections should use the account endpoint above.

Audience compatibility is intentionally one-way during rollout: compatible existing tokens explicitly bound to `/v2/mcp` also work on `/v2/mcp-oauth`, while new `/v2/mcp-oauth` tokens do not work on `/v2/mcp`. This preserves compatible existing connections without weakening the resource binding of newly issued account tokens.

Existing `https://mcp.firecrawl.dev/<API_KEY>/v2/mcp` configurations also continue working during the migration. New configurations should use OAuth or a Bearer header so the credential does not appear in the URL.

### Upgrade from keyless mode

When a keyless request needs an account-only tool, the result lists the available keyless tools and two ways to continue:
...
To continue, connect through `https://mcp.firecrawl.dev/v2/mcp-oauth` or add an API key as a Bearer header on the existing `/v2/mcp` connection, then retry the request.

## Installation

You can use the hosted endpoints above or run the server locally. Create an API key at [firecrawl.dev/app/api-keys](https://www.firecrawl.dev/app/api-keys) when you need the full tool set or higher limits.
...
After adding, refresh the MCP server list to see the new tools. Open Composer with Command+L (Mac), select **Agent**, enable Firecrawl MCP, and describe the web data you need.
...
This starts with the keyless tools and keeps an API key out of the Desktop configuration. After the account-endpoint rollout is live, use the browser OAuth flow above for the full account-connected tool set.
...
Start keyless, or use the Firecrawl CLI to install an environment-backed Bearer header for the full tool set:

```bash
# Remote hosted URL (recommended, works without an API key)
claude mcp add --transport http firecrawl https://mcp.firecrawl.dev/v2/mcp

# Full account limits without storing the key in the URL
export FIRECRAWL_API_KEY=fc-your-api-key
...
For the full tool set, keep the key in an environment variable and tell Codex which variable supplies the Bearer token:
...
<img
  src="/images/guides/mcp/antigravity-mcp-installation.gif"
  alt="Antigravity MCP Installation"
/>
...
8. For **Tools to include**, select from the tools returned by the server. Keyless mode returns only Search, Scrape, and Parse; authenticated mode returns the full tool set.
...
- Hosted keyless mode exposes exactly Search, Scrape, and Parse at both tool discovery and tool-call time, and is rate-limited per IP.
...
- Structured keyless recovery actions for account-gated tools
...
For hosted keyless mode, inspect the structured result's `code` and `next_actions`. Use `connect_account` only when a person can complete a browser flow. Use `configure_api_key` for CI, servers, and unattended agents. Anonymous callers are never routed into an authentication challenge from this endpoint.
