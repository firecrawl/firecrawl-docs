...
5. Enter the MCP server Endpoint: `https://mcp.firecrawl.dev/{YOUR_FIRECRAWL_API_KEY}/v2/mcp` (replace `{YOUR_FIRECRAWL_API_KEY}` with your actual API key)
6. The server will automatically expose all available Firecrawl tools (scrape, crawl, map, search, extract, etc.)

For self-hosted deployments, run the mcp server with npx, and enable HTTP transport mode:

```bash
env HTTP_STREAMABLE_SERVER=true FIRECRAWL_API_KEY=fc-YOUR_API_KEY FIRECRAWL_API_URL=YOUR_FIRECRAWL_INSTANCE npx -y firecrawl-mcp
```

This will start the server on `http://localhost:3000/v2/mcp` which you can use in your n8n workflow. The `HTTP_STREAMABLE_SERVER=true` environment variable is required since n8n needs HTTP/SSE transport.
