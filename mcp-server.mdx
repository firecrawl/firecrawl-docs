...
<Note>
**No API key? Scrape, search, and interact still work.** When you run the server locally without `FIRECRAWL_API_KEY`, the `firecrawl_scrape`, `firecrawl_search`, and `firecrawl_interact` tools fall back to the keyless free tier — free, but capped per IP per day by both a request limit and a credit limit, and unavailable for other tools (crawl, extract, map, etc.). Set `FIRECRAWL_API_KEY` to unlock every tool plus higher limits. See [Rate Limits](/rate-limits#keyless-no-api-key).
</Note>
