...
<Note>
The `proxy` parameter no longer changes behavior. Every request now runs in **`auto`** mode: Firecrawl starts on a basic proxy and automatically escalates to enhanced proxies only if the basic proxy fails. The `basic`, `enhanced`, and `auto` values are still accepted for backward compatibility, but all behave identically, and enhanced retries carry no extra charge. [Learn more about Enhanced Mode →](/features/enhanced-mode)
</Note>
