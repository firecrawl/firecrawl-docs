...
Use `error.code` for programmatic handling. Use `error.message` for human-readable text; messages are subject to change, so prefer `error.code` for branching. Uncaught server faults respond with `500` and `code: "unknown_error"`. If `error.data` is present, treat it as opaque and do not forward it to end users unless you have a documented reason to do so.
...
|--------|--------------|---------|
| `401` | `partner_unauthorized` or `partner_provisioning_unauthorized` | Partner authentication failed |
| `400` | `invalid_request_body` or `partner_provisioning_invalid_params` | Request body is invalid |
| `500` | `partner_provisioning_user_lookup_failed` | User could not be loaded; transient or data issue |
| `500` | `partner_provisioning_team_lookup_failed` | Team data could not be loaded; transient or data issue |
| `500` | `partner_provisioning_no_api_key` | Expected API key missing for the provisioned account |
| `500` | `partner_provisioning_create_failed` | A provisioning step failed on the server |
| `500` | `unknown_error` | Unexpected server error |

For `500` responses, retry with backoff; contact Firecrawl if the problem persists.
...
| Status | `error.code` | Meaning |
|--------|--------------|---------|
| `401` | `partner_unauthorized` | Partner authentication failed |
| `400` | `invalid_request_body` | Request body is invalid |
| `400` | `partner_api_key_invalid_format` | API key value is not acceptable |
| `404` | `partner_validate_api_key_not_identifiable` | Key is not valid for this partner integration |
| `500` | `partner_db_lookup_failed` | Server could not complete a data lookup |
| `500` | `partner_validate_invalid_state` | Server could not complete validation |
| `500` | `unknown_error` | Unexpected server error |
...
| `400` | `invalid_request_body` | Request body is invalid |
| `400` | `partner_api_key_invalid_format` | API key value is not acceptable |
| `404` | `partner_rotate_api_key_not_found` | Key is not valid for rotation under this partner integration |
| `500` | `partner_db_lookup_failed` | Server could not complete a data lookup |
| `500` | `partner_rotate_create_failed` | Server could not issue a replacement key |
| `500` | `partner_rotate_format_failed` | Server could not return the new key in the expected form |
| `500` | `unknown_error` | Unexpected server error |
