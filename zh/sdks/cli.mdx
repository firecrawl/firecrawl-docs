...
<Note>
  **`scrape`、`search` 和 `interact` 无需登录即可使用。** 如果未配置 API 密钥，这些命令会自动使用无需 API 密钥的免费档位——免费，但会按 IP 限流。所有其他命令 (crawl、map、agent 等) 都需要 API 密钥。[注册获取免费密钥](https://firecrawl.dev)，即可获得 1,000 额度和更高限额；配置完成后，CLI 会自动使用它。请参见[限流](/zh/rate-limits#keyless-no-api-key)。
</Note>
