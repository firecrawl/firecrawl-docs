...
当有更多数据可用时，Firecrawl 的 crawl 和 batch 端点会返回一个 `next` URL。Python SDK 默认会自动分页并汇总所有文档；此时 `next` 为 `None`。你可以禁用自动分页或设置上限。`next` URL 是不透明的；将其传回 SDK 以获取下一页。
...
* 先启动任务，然后将 `auto_paginate=False`，并使用 `get_crawl_status_page` 按页逐次获取。
...
* 先启动作业，然后将 `auto_paginate=False`，并使用 `get_batch_scrape_status_page` 一次获取一页。
