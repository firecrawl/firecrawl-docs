...
#### 手动分页辅助方法 {#manual-pagination-helpers}
...
<div id="simple-crawl-auto-pagination-default">
  ##### 简单抓取 (自动分页，默认) {#simple-crawl-auto-pagination-default} 
</div>

* 参见[抓取网站](#crawl-a-website)中的默认流程。

##### 手动抓取并控制分页 {#manual-crawl-with-pagination-control}
...
##### 手动抓取并设定限制 (自动分页 + 提前停止) {#manual-crawl-with-limits-auto-pagination-early-stop} 
...
<div id="simple-batch-scrape-auto-pagination-default">
  ##### 简单批量爬取 (自动分页，默认) {#simple-batch-scrape-auto-pagination-default} 
</div>

* 参见默认流程：[Batch Scrape](/zh/features/batch-scrape)。

##### 手动批量抓取并控制分页 {#manual-batch-scrape-with-pagination-control}
...
##### 受限的手动批量抓取 (自动分页 + 提前停止) {#manual-batch-scrape-with-limits-auto-pagination-early-stop} 
...
### 绑定到抓取的交互式会话 {#scrape-bound-interactive-session}
