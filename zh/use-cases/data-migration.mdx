...
用户借助 Firecrawl 构建迁移工具，从各类平台提取数据：

<div id="cms-content-extraction">
  ### CMS 内容抽取
</div>

* 从 WordPress、Drupal、Joomla 站点抽取内容
* 从自定义 CMS 平台获取数据
* 保留内容结构与元数据
* 导出以便导入到 Contentful、Strapi、Sanity 等新系统

<div id="e-commerce-data-extraction">
  ### 电商数据抽取
</div>

* 从 Magento、WooCommerce 商店抽取产品目录
* 拉取库存和定价数据
* 获取产品描述和规格
* 将数据整理为可导入 Shopify、BigCommerce 或其他平台的格式
...
* [产品与电商](/zh/use-cases/product-ecommerce) - 商品目录迁移
* [内容生成](/zh/use-cases/content-generation) - 内容改造
