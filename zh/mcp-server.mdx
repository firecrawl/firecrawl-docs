...
<Note>
  **没有 API 密钥？抓取、搜索、交互和解析功能可在免密钥远程 URL 上使用。** 连接到 `https://mcp.firecrawl.dev/v2/mcp` 即可使用免密钥免费档位——虽然免费，但会按 IP 限流，且其他工具 (crawl、extract、map 等) 无法使用。设置 `FIRECRAWL_API_KEY` 即可解锁全部工具和更高的限额。请参见 [Rate Limits](/zh/rate-limits#keyless-no-api-key)。
</Note>
...
或者，无需 API 密钥也可连接并开始使用——在远程免密钥免费档位中，scrape、search、交互和解析功能均可使用 (按 IP 限流) ：
...
<div id="10-create-browser-session-firecrawl_browser_create">
  ### 10. 创建浏览器会话 (`firecrawl_browser_create`)
</div>
...
<div id="11-execute-code-in-browser-firecrawl_browser_execute">
  ### 11. 在浏览器中执行代码 (`firecrawl_browser_execute`)
</div>
...
<div id="12-delete-browser-session-firecrawl_browser_delete">
  ### 12. 删除浏览器会话 (`firecrawl_browser_delete`)
</div>
...
<div id="13-list-browser-sessions-firecrawl_browser_list">
  ### 13. 列出浏览器会话 (`firecrawl_browser_list`)
</div>
...
<div id="14-interact-with-scraped-page-firecrawl_interact">
  ### 14. 与已抓取的页面交互 (`firecrawl_interact`)
</div>
...
<div id="15-stop-interact-session-firecrawl_interact_stop">
  ### 15. 停止交互会话 (`firecrawl_interact_stop`)
</div>
