...
<Note>
  **没有 API 密钥？抓取、搜索和交互功能仍然可用。** 当你在本地运行服务器且未设置 `FIRECRAWL_API_KEY` 时，`firecrawl_scrape`、`firecrawl_search` 和 `firecrawl_interact` 工具会自动回退到免密钥的免费档位——虽然免费，但会按 IP 限流，且其他工具 (crawl、extract、map 等) 无法使用。设置 `FIRECRAWL_API_KEY` 即可解锁全部工具和更高的限额。请参见 [Rate Limits](/zh/rate-limits#keyless-no-api-key)。
</Note>
