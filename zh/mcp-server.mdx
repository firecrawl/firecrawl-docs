...
og:description: "Firecrawl MCP 服务器让你能够通过模型上下文协议使用 Firecrawl 的 API"
sidebarTitle: "MCP 服务器"
---

一个模型上下文协议（MCP）服务器实现，集成了用于网页抓取的 [Firecrawl](https://github.com/mendableai/firecrawl) 能力。我们的 MCP 服务器是开源的，代码托管在 [GitHub](https://github.com/mendableai/firecrawl-mcp-server)。
...
> 在 [MCP.so 的 Playground 上体验我们的 MCP 服务器](https://mcp.so/playground?server=firecrawl-mcp-server)，或在 [Klavis AI](https://www.klavis.ai/mcp-servers) 上试用。
...
使用以下 URL：http://localhost:3000/v2/sse 或 https://mcp.firecrawl.dev/{FIRECRAWL_API_KEY}/v2/sse
...
如需手动安装，请将以下 JSON 块添加到 VS Code 的用户设置（JSON）文件中。你可以按下 `Ctrl + Shift + P`，输入 `Preferences: Open User Settings (JSON)` 即可。
