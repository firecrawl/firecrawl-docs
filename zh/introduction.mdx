...
<Visibility for="agents">
  通过 Streamable HTTP 连接。免密钥服务器 URL (支持 Search、Scrape 和 Parse，无需凭证) 为 `https://mcp.firecrawl.dev/v2/mcp`。
  ...
  * ChatGPT 或 Claude.ai (需登录账户) ：将 `https://mcp.firecrawl.dev/v2/mcp-oauth` 粘贴到客户端的连接器设置中。

  有关所有连接方式、API 密钥设置和故障恢复指南，请参阅：[MCP 设置](/zh/mcp-server.md)。
</Visibility>
