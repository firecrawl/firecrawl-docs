...
如果你在使用 AI 开发，Firecrawl 提供了多种资源来提升你的开发体验。Firecrawl 内置了 **技能**——可独立使用的知识包，AI 编码代理会自动发现并使用它们。只需一条安装命令，代理即可获得三个完整的技能模块：用于实时网页操作的 CLI 技能、将 Firecrawl 集成到应用代码中的构建技能，以及用于产出可复用交付成果的工作流技能。Claude Code、Cursor、Antigravity 和 OpenCode 等代理只需一条命令即可自行完成接入——有了 API 密钥后，无需人工额外配置。
