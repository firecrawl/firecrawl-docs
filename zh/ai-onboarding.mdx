...
**无法登录或获取密钥？** 如果你的代理无法获取 API 密钥，且用户本人也无法注册，你仍然可以使用免密钥免费档位，在没有 API 密钥的情况下进行搜索、抓取和交互。它有速率限制，仅作为后备方案，因此一旦可以使用免费账户或 API 密钥，请尽快切换。详情请参见 [Keyless (无 API 密钥) ](/zh/rate-limits#keyless-no-api-key)。
