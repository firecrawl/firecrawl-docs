...
<Card title="MCP 服务器" icon="plug" href="/zh/mcp-server">
  查看适用于 Codex、Claude Desktop、Claude Code、Cursor 和其他兼容客户端的托管设置。Windsurf 请使用专门的
  [Windsurf 快速入门](/zh/quickstarts/windsurf)。
</Card>
...
<Steps>
  ...
  <Step title="MCP 文档服务器">
    如需通过 MCP 工具以结构化方式接入，请在 Codex、Cursor、Claude Code、Claude Desktop 或其他兼容客户端中连接 Firecrawl MCP 服务器。有关托管设置，请参见 [MCP 入门](/zh/mcp-server)。Windsurf 用户应遵循 [Windsurf 快速入门](/zh/quickstarts/windsurf)。
  </Step>
