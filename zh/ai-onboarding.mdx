...
* **Stripe Projects：**在已使用 Stripe CLI 的项目中，`stripe projects add firecrawl/api --name firecrawl` 会自动开通账户并将 `FIRECRAWL_API_KEY` 写入 `.env`。请参见 [Stripe Projects](/zh/integrations/stripe-projects)。
...
**无法登录或获取密钥？** CLI、SDKs 和 REST API 支持免密钥搜索、抓取、交互和解析。托管 MCP 提供功能范围更有限的免密钥搜索、抓取和解析；其中交互需要账户连接或 API 密钥。免密钥访问会受到限流，仅作为后备方案，因此一旦可以使用免费账户或 API 密钥，请尽快切换。详情请参见 [Keyless (无 API 密钥)](/zh/rate-limits#keyless-no-api-key)。

获得 API 密钥后，继续阅读下方的 [CLI 和代理技能](#cli)。

<span id="cli" />

## CLI 和代理技能 {#cli-and-agent-skills}
...
### 安装后会提供什么 {#what-the-install-gives-you}
...
### 代理如何串联使用工具 {#how-agents-chain-tools-together}
