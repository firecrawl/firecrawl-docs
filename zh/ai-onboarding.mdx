...
MCP 是一种开放协议，规范了应用如何向 LLM 提供上下文。除此之外，它还为 LLM 提供了可代表你执行操作的工具。我们的 [MCP 服务器](https://github.com/firecrawl/firecrawl-mcp-server) 是开源的，覆盖了完整的 API 工具集——search、scrape、交互、爬取、map、extract 和 agent。

使用远程托管 URL (在免密钥免费档位下无需 API 密钥即可使用) ：

```
https://mcp.firecrawl.dev/v2/mcp
```

如需使用 API 密钥，请将其作为 `Authorization: Bearer fc-YOUR-API-KEY` 请求头发送，即可解锁所有工具和更高的限制。
...
<Card title="MCP 服务器" icon="plug" href="/zh/mcp-server/human-mcp">
  查看适用于 Codex、Claude Desktop、Claude Code、Cursor 和其他兼容客户端的托管设置。Windsurf 请使用专门的
  [Windsurf 快速入门](/zh/quickstarts/windsurf)。如需配置无人值守代理或 CI，
  请参见 [Agent MCP](/zh/mcp-server/agent-mcp)。
</Card>
...
<Steps>
  ...
  <Step title="MCP 文档服务器">
    如需通过 MCP 工具以结构化方式接入，请在 Codex、Cursor、Claude Code、Claude Desktop 或其他兼容客户端中连接 Firecrawl MCP 服务器。有关交互式托管设置，请参见 [Human MCP](/zh/mcp-server/human-mcp)；如需无人值守的代理，请参见 [Agent MCP](/zh/mcp-server/agent-mcp)。Windsurf 用户应遵循 [Windsurf 快速入门](/zh/quickstarts/windsurf)。
  </Step>
