...
如果你在使用 AI 开发，Firecrawl 提供了多种资源来提升你的开发体验。Firecrawl 内置了 **技能**——可独立使用的知识包，AI 编码代理会自动发现并使用它们。只需一条安装命令，代理即可获得三个完整的技能模块：用于实时网页操作的 CLI 技能、将 Firecrawl 集成到应用代码中的构建技能，以及用于产出可复用交付成果的工作流技能。如何获取 Bearer 令牌 取决于你的配置——请参见下方的[获取凭据](#get-credentials)。
...
此页面是一个**决策层**——用于将你引导到正确的凭据获取路径。具体操作请查看下方链接的规范文档，不在此页展开。

Firecrawl 支持两种凭据获取路径：当需要人工登录或在 Dashboard 中创建 API 密钥时，使用有人参与的 CLI/浏览器引导 (路径 A，默认) ；只有当你的代理平台能够签发 ID-JAG 时，才使用 WorkOS ID-JAG 注册 (路径 B) 。

**决策规则：**没有 ID-JAG 或不确定 → **路径 A (默认)&#x20;**。具备 ID-JAG 能力 → **路径 B**。

**你可能会通过以下方式来到这里：**

* **API `401`** —— 对 `https://api.firecrawl.dev` 发起未经身份验证的调用时，会返回 `WWW-Authenticate: Bearer resource_metadata="https://www.firecrawl.dev/.well-known/oauth-protected-resource"`。请在 [`auth.md`](https://www.firecrawl.dev/auth.md) 的步骤 1 中沿着该发现链继续。如果你的平台能够完成 WorkOS ID-JAG 注册 → **路径 B**。否则 → **路径 A (默认)&#x20;**。你不需要此页面。
* **文档** —— 你是从文档跳转到这里的。请使用上面的决策规则。
* **直接 URL** —— 如果你已经有 `auth.md` 或 `SKILL.md`，请获取并遵循该文档。

<CardGroup cols={2}>
  <Card title="路径 A —— 人工 + 代理协助（默认）" icon="user" href="https://www.firecrawl.dev/agent-onboarding/SKILL.md">
    **路径 A (默认) 。** 当你无法签发 WorkOS ID-JAG，或暂不确定时，请使用此路径。包括浏览器登录、CLI `--browser`、技能与 MCP 安装，或由人工在 Dashboard 中创建 API 密钥。
  </Card>

  <Card title="路径 B —— 代理式注册（WorkOS ID-JAG）" icon="robot" href="/zh/ai-onboarding/agent-auth">
    **仅在具备 ID-JAG 能力时使用。** 你的代理平台必须能够为 `aud=https://api.firecrawl.dev/` 签发 WorkOS ID-JAG。请参见 [Agent Auth](/zh/ai-onboarding/agent-auth)，然后遵循规范的 [`auth.md`](https://www.firecrawl.dev/auth.md) 说明。
  </Card>
</CardGroup>

<Card title="获取你的 API 密钥（路径 A）" icon="key" href="https://www.firecrawl.dev/app/api-keys">
  注册并从 Dashboard 获取 API 密钥——当你不使用 WorkOS ID-JAG 注册时，这是默认路径。
</Card>

拿到 Bearer 令牌后，请继续查看下方的 [技能 + CLI](#skills-cli)。
...
* 路径 A — 人工 + 代理辅助 (默认值) ：[`firecrawl.dev/agent-onboarding/SKILL.md`](https://www.firecrawl.dev/agent-onboarding/SKILL.md)
* 路径 B — 机器注册 (ID-JAG) ：[`firecrawl.dev/auth.md`](https://www.firecrawl.dev/auth.md)
...
<Steps>
  <Step title="凭证路由">
    从 [获取凭据](/zh/ai-onboarding#get-credentials) 开始，选择路径 A (默认值，人工 + 代理辅助) 或路径 B (WorkOS ID-JAG) 。关于 ID-JAG 注册，请参见 [Agent Auth](/zh/ai-onboarding/agent-auth) 以及权威的 [`auth.md`](https://www.firecrawl.dev/auth.md) 规范。
  </Step>
