...
### 最佳用例：自主研究与探索 {#best-use-case-autonomous-research-discovery}
...
## 2. `/extract` 端点 {#2-extract-endpoint}
...
### 最佳使用场景：有针对性的多页面提取 {#best-use-case-targeted-multi-page-extraction}
...
## 3. 使用 JSON 模式的 `/scrape` 端点 {#3-scrape-endpoint-with-json-mode}

使用 JSON 模式的 `/scrape` 端点是可控性最高的方案——它使用 LLM 将页面内容解析为你指定的 schema，从单个已知 URL 中提取结构化数据。

### 关键特性 {#key-characteristics-3}
...
### 最佳适用场景：单页精确抽取 {#best-use-case-single-page-precision-extraction}
...
### 示例：“查找 Firecrawl 的创始人” {#example-find-the-founders-of-firecrawl}
...
## 迁移：`/extract` → `/agent` {#migration-extract-agent}
