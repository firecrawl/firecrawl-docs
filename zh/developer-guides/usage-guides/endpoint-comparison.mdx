...
<Info>
  **研究预览**：Agent 当前处于早期体验阶段，未来会持续大幅改进。[分享反馈 →](mailto:product@firecrawl.com)
</Info>
...
<Tip>
  `/extract` 现在支持在没有 URL 的情况下进行提取——只需提供一个 prompt 即可。不过，对于这种场景，使用 `/agent` 通常能获得更好的效果。
</Tip>
