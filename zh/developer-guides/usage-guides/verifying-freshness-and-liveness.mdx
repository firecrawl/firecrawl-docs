...
## 新鲜度与性能的权衡 (`maxAge`) {#the-freshness-tradeoff-maxage}
...
### `maxAge` 的适用位置 {#where-maxage-applies}
...
## 新鲜度不代表存活状态 {#freshness-is-not-liveness}
...
## 对新鲜度敏感的操作的检查清单 {#freshness-sensitive-action-checklist}
...
## 实战示例：收集当前页面证据 {#worked-example-collect-current-page-evidence}
...
## 按场景的建议 {#recommendations-by-scenario}
