---
title: '抓取'
description: 'Firecrawl 可递归遍历某个 URL 的子域，汇集其内容'
og:title: '抓取 | Firecrawl'
og:description: 'Firecrawl 可递归遍历某个 URL 的子域，汇集其内容'
---
...
Firecrawl 可高效爬取网站，在绕过反爬与拦截的同时提取全面数据。流程：

1. **URL 分析：** 扫描 sitemap 并爬取站点以识别链接
2. **遍历：** 递归跟踪链接以发现所有子页面
3. **抓取：** 从每个页面提取内容，处理 JS 与速率限制
4. **输出：** 将数据转换为干净的 Markdown 或结构化格式

这确保能从任意起始 URL 全面收集数据。

<div id='crawling'>## 爬虫</div>

<div id='crawl-endpoint'>### /crawl 端点</div>

用于爬取一个 URL 及其所有可访问的子页面。此操作会提交一个爬取任务，并返回任务 ID 以便查看爬取状态。

<Warning>
  默认情况下，如果页面中的子链接不是你提供的 URL 的子路径，/crawl
  会忽略这些链接。例如，当你爬取 website.com/blogs/
  时，website.com/other-parent/blog-1 不会被返回。如果需要获取
  website.com/other-parent/blog-1，请使用 `crawlEntireDomain` 参数。若要在爬取
  website.com 时同时爬取 blog.website.com 等子域名，请使用 `allowSubdomains`
  参数。
</Warning>

<div id='installation'>### 安装</div>
...
<div id='api-response'>### API 响应</div>

如果你使用 cURL 或 SDK 的 `async crawl` 函数，将返回一个 `ID`，可用于查询抓取任务的状态。

<Note>如果你使用 SDK，请查看下方的[SDK 响应](#sdk-response)部分。</Note>

<AsyncCrawlOutput />

<div id='check-crawl-job'>### 检查爬取任务</div>

用于查看爬取任务的状态并获取其结果。

<Note>该端点仅适用于正在进行或近期刚完成的爬取任务。</Note>
...
<div id='response-handling'>#### 响应处理</div>

响应内容会根据爬取任务的状态而变化。

对于未完成的任务，或超过 10MB 的大型响应，会返回一个 `next` URL 参数。你需要请求该 URL 以获取下一段 10MB 的数据。如果没有 `next` 参数，则表示爬取数据已结束。

skip 参数用于设置每个结果分块返回的最大条目数。

<Info>
  skip 和 next 参数仅在直接调用 API 时适用。如果你使用
  SDK，我们会代为处理并一次性返回全部结果。
</Info>
...
<div id='sdk-response'>### SDK 响应</div>
...
<div id='faster-crawling'>## 更快的爬取</div>
...
<div id='crawl-websocket'>## 爬取 WebSocket</div>
...
<div id='crawl-webhook'>## 爬取 Webhook</div>

你可以配置 Webhook，在爬取过程中实时接收通知。这样你可以在页面被抓取后立即处理，而无需等待整个爬取完成。

<CrawlWebhookCURL />

有关包含事件类型、负载结构和实现示例的完整 Webhook 文档，请参阅[Webhook 文档](/zh/webhooks/overview)。

<div id='quick-reference'>### 快速参考</div>

**事件类型：**

- `crawl.started` - 抓取开始时触发
- `crawl.page` - 每成功抓取一个页面时触发
- `crawl.completed` - 抓取完成时触发
- `crawl.failed` - 抓取出错时触发
...
<Note>
  如需了解 webhook 的详细配置、安全最佳实践和故障排查，请访问 [Webhooks
