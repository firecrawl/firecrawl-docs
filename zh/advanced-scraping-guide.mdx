...
- **PDF 不支持 actions**：actions 需要浏览器引擎，不能用于返回 PDF 内容的 URL（包括重定向到 PDF 的 URL）。如果你在请求中包含 `actions`，且该 URL 解析为 PDF，请求将失败。抓取 PDF 时请从请求中移除 `actions`。
...
- **类型**: `array`
- **说明**: 要包含的正则表达式模式。默认情况下，这些模式只会匹配 URL 的**路径名**（不包括查询参数）。若要匹配包含查询字符串在内的完整 URL，请使用 `regexOnFullURL: true`。
- **示例**: `["^/blog/.*$", "^/docs/.*$"]`
...
- **类型**: `array`
- **描述**: 用于排除的正则表达式模式。默认情况下，这些模式仅针对 URL 的 pathname 进行匹配（不包括查询参数）。要针对包含查询字符串在内的完整 URL 进行匹配，请使用 `regexOnFullURL: true`。
- **示例**: `["^/admin/.*$", "^/private/.*$"]`
...
- **类型**: `boolean`
- **描述**: 设为 `true` 时，`includePaths` 和 `excludePaths` 的匹配规则会作用于完整 URL（包括查询参数），而不仅仅是 pathname 部分。适用于基于查询字符串过滤 URL。
- **默认值**: `false`
...
- **Description**: 控制爬虫如何使用网站的 sitemap。`"include"`（默认）会在进行链接发现的同时使用 sitemap。`"skip"` 会完全忽略 sitemap。`"only"` 会将抓取限制为 sitemap 中的 URL（以及起始 URL）。
- **Default**: `"include"`
...
- **Type**: `boolean`
- **Description**: 当为 `true` 时，crawler 会将指向同一页面的不同 URL 变体视为重复——例如，带 `www.` 与不带 `www.`、`http` 与 `https`、是否带结尾斜杠、以及 `index.html`/`index.php` 等后缀，都会在检查重复前进行规范化处理。此选项不会移除查询参数（相关行为请参见 `ignoreQueryParameters`）。
- **Default**: `true`
...
- **Type**: `boolean`
- **Description**: 当为 `true` 时，会在去重之前从 URL 中移除查询参数，因此 `/page?a=1` 和 `/page?a=2` 会被视为相同的 URL，并且只抓取一次。
- **Default**: `false`
