---
title: '爬取'
description: '递归爬取网站并提取每个页面的内容'
og:title: '爬取 | Firecrawl'
og:description: '递归爬取网站并提取每个页面的内容'
icon: 'spider'
---
...
[Scrape 端点](/zh/api-reference/endpoint/scrape)的所有选项都可通过 `scrapeOptions` (JS) / `scrape_options` (Python) 在 crawl 中使用。它们将应用于爬虫抓取的每个页面，包括 formats、proxy、caching、actions、location 和 tags。
...
| `maxDiscoveryDepth`     | `integer`  |  (无)        | 基于发现顺序、相对于根 URL 的最大深度。根站点和通过站点地图发现的页面，其发现深度为 0。                 |
...
| `delay`                 | `number`   |  (无)        | 每次抓取之间的延迟时间 (秒) ，以遵守速率限制                                        |
