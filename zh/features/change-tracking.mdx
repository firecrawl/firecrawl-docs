...
if 'diff' in result.change_tracking:
    ...
    print(result.change_tracking.diff.json)  # 结构化的 diff 数据

# 访问 JSON 比对的变更
if 'json' in result.change_tracking:
    print(result.change_tracking.json.title.previous)  # 先前标题
    print(result.change_tracking.json.title.current)  # 当前标题
```
...
`json` 模式会对从内容中提取的特定字段进行结构化对比。这有助于追踪具体数据点的变化，而不是比较整段内容。
...
if result.change_tracking.change_status == 'changed':
    print(f"价格已从 {result.change_tracking.json.price.previous} 变为 {result.change_tracking.json.price.current}")
```
