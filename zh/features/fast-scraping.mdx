---
title: "更快的抓取"
description: "使用 maxAge（缓存）参数将抓取速度提升 significantly"
og:title: "更快的抓取 | Firecrawl"
og:description: "使用 maxAge（缓存）参数将抓取速度提升 significantly"
---
...
当你不需要绝对最新的数据时，结果可快至**significantly**。通过 `maxAge` 控制新鲜度：
...
使用 `maxAge` 进行爬取时，如果该页面存在近期缓存数据，你爬取的每个页面都可获得高达 significantly 的速度提升。
