...
每次 check 都会执行同样的流程：获取 goal 及其 `queries`，应用一个时间新近窗口，运行 search，按 canonical URL 对结果去重，再由可选的 AI judge 判断哪些新结果对 goal 有意义，并通过与 scrape 和 crawl 监控相同的 webhook 和 email 渠道发送告警。调度、goals、判定 和通知的工作方式，均与[监控概览](/zh/features/monitoring)中所述完全一致。
