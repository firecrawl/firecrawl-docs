...
<BatchScrapeOutput />`- 调用`startBatchScrape`/`start&#95;batch&#95;scrape`会返回一个作业 ID。你可以通过`getBatchScrapeStatus`/`get&#95;batch&#95;scrape&#95;status`、API 端点 `/batch/scrape/{id}`，或 webhooks 进行追踪。该端点用于处理中阶段的查询或完成后的即时查询，**因为批处理作业会在 24 小时后过期**。
...
有关完整的 Webhook 文档（包括事件类型、载荷结构和实现示例），请参阅[Webhook 文档](/zh/webhooks/overview)。
...
<Note>
  有关 webhook 的详细配置、安全最佳实践和故障排查，请参阅 [Webhooks 文档](/zh/webhooks/overview)。
</Note>
