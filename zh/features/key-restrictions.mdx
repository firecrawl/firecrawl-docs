...
当某个 密钥 带有 allowed-formats 列表时，所有会产生抓取结果的 endpoint 都会执行该限制：`/v2/scrape`、`/v2/batch/scrape`、`/v2/crawl` (通过 `scrapeOptions`) 以及 `/v2/search` (通过 `scrapeOptions`) ，另外也包括它们对应的 v1 版本。
...
* 密钥限制与基于 IP 的限制相互独立；一个密钥可以同时设置这两种限制。