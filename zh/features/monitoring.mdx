...
Firecrawl 监控可检测网站内容何时发生变化，并通过 Webhook 或电子邮件接收通知。它会定期执行抓取或爬取，并将每次结果与上一次保留的快照进行比较。你可以使用监控来跟踪产品页面、文档、博客、更新日志、竞争对手网站，或任何变化值得关注的页面。
...
监控不单独按每个监控收费。每次检查都按其执行的底层 抓取 或 爬取 计费；此外，如果启用了有效变更判定，则每个发生变更且经判定器确认的页面还会额外消耗 1 个额度。
