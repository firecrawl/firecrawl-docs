...
### Markdown 模式 (默认) {#markdown-mode-default}
...
### 混合模式 (JSON + git-diff) {#mixed-mode-json-git-diff}
...
#### 收件人确认流程 {#recipient-confirmation-process}
...
## 被跳过的 检查 与暂停的监控 {#skipped-checks-and-paused-monitors}
...
### 没有可直接读取的失败计数器 {#there-is-no-failure-counter-to-read}
