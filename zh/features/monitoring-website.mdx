...
网站监控面向整个网站，而不是固定的一组 URL。每次检查都会针对目标 `url` 执行一次 `crawl`，抓取所有发现的页面，并将结果与最近一次保留的快照进行比对——因此它能捕捉到新增、变更和删除的页面，而不只是你已指定页面上的修改。对于文档站点、博客、更新日志、帮助中心和竞争对手网站来说，这是更合适的选择。
