---
title: "研究索引"
description: "搜索论文、阅读论文片段、查找相关工作，并搜索相关的 GitHub 仓库"
og:title: "研究索引 | Firecrawl"
og:description: "使用 Firecrawl 的研究索引进行论文检索、语义扩展、论文阅读和 GitHub 搜索。"
icon: "book-open"
---

import ResearchFeedbackCTA from "/snippets/zh/research-feedback-cta.mdx";

Firecrawl Research 是一个专为科学与工程研究代理构建的专用索引。它提供一套面向研究的工具集，用于搜索论文、查看论文元数据、阅读相关的全文片段、通过结构化扩展发现相关论文，以及搜索与研究相关的 GitHub 仓库。

* 按主题、方法、基准、作者或类别查找论文
* 查看标准论文元数据和来源 ID
* 阅读单篇论文中能够回答特定问题的片段
* 从高质量的种子论文扩展到相关论文、引用它们的论文或参考文献
* 搜索 GitHub 历史记录和 README，查找实现说明、缺陷和设计讨论
...
| 搜索 GitHub 历史记录 | [`GET /search/research/github`](/zh/api-reference/endpoint/research-github-search)               |
...
搜索 GitHub 上的 issue、PR、discussion 和仓库 README，查找实现细节和工程上的既有方案。
...
GitHub 结果包含仓库、URL、issue/PR 元数据 (如有) 、摘要片段，以及匹配的 Markdown 内容 (如有) 。