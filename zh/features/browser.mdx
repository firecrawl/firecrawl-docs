...
Firecrawl Browser Sandbox 为你的智能体提供一个安全、全托管的浏览器运行环境。无需本地配置、无需安装 Chromium，也不存在驱动兼容性问题。环境中已预装 agent-browser 和 Playwright。每个会话都在隔离的一次性沙箱中运行，并且可以自动扩缩容，无需自行管理基础设施。
...
| `activityTtl` | 120s（2 分钟） | 会话空闲达到该时长后自动关闭（10-3600s） |

<div id="persistent-sessions">
  ### 持久会话
</div>

默认情况下，每个浏览器会话都会从全新环境开始。通过 `persistentSession`，你可以保存 cookies、localStorage 和其他浏览器状态，以便在后续会话中继续使用。这对于保持登录状态、保留偏好设置或恢复多步工作流非常有用。
...
| `writeMode` | `"readwrite"` | `"readwrite"` 会将更改写回存储；`"readonly"` 只会加载已有数据而不写入——在需要多个并发只读访问时很有用。 |

<Note>
  同一时间只有一个会话可以对某个配置文件进行写入。如果已有其他会话在写入，你会收到 `409` 错误。你仍然可以以 `"readonly"` 模式打开同一配置文件，或者稍后重试。
</Note>
