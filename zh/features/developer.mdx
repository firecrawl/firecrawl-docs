...
在 `/search` 的 `categories` 数组中仅传入 `developer`。开发者结果会以 `category: "developer"` 标记返回在标准 `web` 分组中。`developer` 类别不能与其他类别组合使用。
...
开发者结果包含 `url`、`title`、`description` 和 `position`，结构与网页结果相同，并额外包含 `category: "developer"`。SDK 用户从 `result.web` 获取。
