...
<Note>
  要让您的代理访问开发者索引，我们强烈建议将我们的 [CLI](/zh/sdks/cli) 或 [MCP](/zh/mcp-server/agent-mcp) 与[**专用开发者技能**](https://github.com/firecrawl/skills/blob/main/skills/firecrawl-developer-index/SKILL.md)结合使用。您可以通过以下命令安装该技能：
...
<Note>
  下方所示的 Python 和 Node SDKs 通过 `developer` 类别访问开发者索引。它们未提供此端点的专用方法，因此请通过 HTTP、[CLI](/zh/sdks/cli) 或 [MCP](/zh/mcp-server/agent-mcp) 调用。
</Note>
...
<Note>
  托管的 [MCP server](/zh/mcp-server/agent-mcp) 同时提供这两种功能，且两者均不会写入任何内容。请参见 [MCP tools](/zh/mcp-server/tools)，了解 `firecrawl_developer_search`、如何通过 `firecrawl_search` 获取开发者结果，以及两者中哪个可通过免密钥工具集使用。
</Note>