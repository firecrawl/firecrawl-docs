...
* `repos` 限定索引中 GitHub 部分的搜索范围，`sources` 限定文档部分的搜索范围
...
这七个仓库筛选条件用于描述 GitHub 仓库。因此，在未限定 `sources` 范围的情况下发送其中任一筛选条件，不会返回 `doc` 结果，并会在 `coverage` 中将 `doc` 标记为 `unavailable`。发送前请阅读[仓库筛选条件如何限定搜索范围](/zh/api-reference/endpoint/developer-search#how-the-repository-filters-scope-a-search)。
