...
Firecrawl 代理 提供三种模型。**Spark 2** 是默认值：在准确率相当的前提下，比任一 Spark 1 模型都更便宜、更快。两种 Spark 1 模型仍然可用，且未作改动。

| 模型             | 成本              | 准确率      | 最适合                 |
| -------------- | --------------- | -------- | ------------------- |
| `spark-2`      | 最低              | 与 Pro 相当 | 所有任务 (默认值)          |
| `spark-1-mini` | 比 Pro **低 60%** | 标准       | 对成本敏感的 Spark 1 工作负载 |
| `spark-1-pro`  | 标准              | 更高       | 复杂研究、关键提取           |

<Tip>
  **Spark 2 是默认值**，因此可以省略 `model` 参数——在大多数任务中，它的成本更低，完成速度也比两种 Spark 1 模型更快。Spark 1 模型仍然可用：建议先从 Mini 开始，仅在进行复杂的多域研究或准确率至关重要时切换到 Pro。
</Tip>
...
`spark-2` 是我们最新的代理模型，也是所有请求的默认选择。它覆盖了此前需要在 Mini 和 Pro 之间取舍的全部任务，因此无需再权衡准确率与成本。
...
`spark-2` 是默认值，因此 `model` 参数可选。通过传入该参数可选择其他模型：
...
| `model`      | string | 否     | 要使用的模型：`spark-2` (默认)、`spark-1-mini` 或 `spark-1-pro`                                                                                                                                                                                                       |
