...
Firecrawl 代理 提供三种模型。**Spark 1 Pro** 是默认值。**Spark 2** 在准确率相当的前提下，比任一 Spark 1 模型都更便宜、更快——将 `model` 设置为 `spark-2` 即可使用它。

| 模型             | 成本              | 准确率      | 最适合                 |
| -------------- | --------------- | -------- | ------------------- |
| `spark-2`      | 最低              | 与 Pro 相当 | 低成本、运行时间短           |
| `spark-1-mini` | 比 Pro **低 60%** | 标准       | 对成本敏感的 Spark 1 工作负载 |
| `spark-1-pro`  | 标准              | 更高       | 复杂研究、关键提取 (默认值)     |

<Tip>
  **Spark 1 Pro 是默认值**，因此未指定 `model` 参数的请求会使用 `spark-1-pro`。将 `model` 设置为 `spark-2`，即可在准确率相当的情况下获得更低成本和更短运行时间。对于简单的 Spark 1 任务，将 `model` 设置为 `spark-1-mini` 可降低 60% 成本。
</Tip>
...
`spark-2` 是我们最新的代理模型。它覆盖了此前需要在 Mini 和 Pro 之间取舍的全部任务，因此无需再权衡准确率与成本。将 `model` 设置为 `spark-2` 即可使用它。

**以下情况请选择 Spark 2：**

* 希望将单次运行成本降至最低
* 重视运行时间
* 原本会因准确率而选择 Spark 1 Pro
...
`spark-1-pro` 是我们的旗舰模型，专为在复杂数据提取任务中实现最高准确率而设计。它是默认值：未设置 `model` 的请求会使用 `spark-1-pro`。
...
`spark-1-pro` 是默认值，因此 `model` 参数可选。通过传入该参数可选择其他模型：
...
| `model`      | string | 否     | 要使用的模型：`spark-1-pro` (默认)、`spark-1-mini` 或 `spark-2`                                                                                                                                                                                                       |
