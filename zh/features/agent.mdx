...
`/agent` 端点是我们的 [`/extract`](/zh/features/extract) 端点的下一代演进版本。它使用 AI 代理从全网智能收集并结构化数据。与 `/extract` 不同，Agent 只需要一个提示词即可——URL 是可选的——这让它在处理各种类型的查询时都极其灵活。

<Info>Agent 是 `/extract` 的继任者：更可靠、更快、也更便宜。</Info>

<div id="why-agent-over-extract">
  ## 为什么选择 Agent 而不是 `/extract`？
</div>

Agent 构建在 `/extract` 的全部优势之上，并更进一步：

* **仅需 Prompt**：只要描述你的需求——无需提供任何 URL
* **更智能**：AI Agent 能理解上下文并自动找到相关数据
* **更快速**：优化后的处理流程，结果返回更快
* **更省成本**：更高的资源利用效率意味着更低的成本
* **更灵活**：适用于多种查询类型和使用场景
...
要获得结构化输出，请提供一个 JSON schema。SDK 支持使用 Pydantic（Python）和 Zod（Node）来定义类型安全的 schema：
