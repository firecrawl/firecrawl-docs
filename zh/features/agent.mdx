...
Firecrawl Agent 提供两种模型。`spark-1-pro` 是默认模型，可为复杂任务提供最高准确率。`spark-1-mini` 的成本低 60%，适合优先考虑成本效率的场景。

| Model | Cost | Accuracy | Best For |
|-------|------|----------|----------|
| `spark-1-mini` | **成本低 60%** | 标准 | 注重成本效率的任务 |
| `spark-1-pro` | 标准 | 更高 | 复杂调研、关键数据抽取 (默认) |

<Tip>
  Agent 默认使用 Spark 1 Pro。如果你希望降低简单任务的成本，请明确选择 Spark 1 Mini。
</Tip>
...
| `model`      | string | 否     | 要使用的模型：`spark-1-pro` (默认) 或 `spark-1-mini`                                                                                                                                                                                                                 |
...
- [启动 Agent 任务](/zh/api-reference/endpoint/agent)
- [获取 Agent 状态](/zh/api-reference/endpoint/agent-get)
- [取消 Agent 任务](/zh/api-reference/endpoint/agent-delete)
