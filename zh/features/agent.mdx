...
## 提供 URL (可选) {#providing-urls-optional} 
...
## 任务状态与完成 {#job-status-and-completion}
...
#### 等待中示例 {#pending-example}

<AgentStatusPending />

#### 已完成示例 {#completed-example}

<AgentStatusCompleted />

## 列出代理运行记录 {#listing-agent-runs}
...
## 跟踪正在进行的运行 {#following-a-run-in-progress}
...
## 执行追踪和快照 {#execution-traces-and-snapshots}
...
## 获取代理的源数据 {#getting-the-agents-source-data}
...
## Agent 与 Extract：有哪些改进 {#agent-vs-extract-whats-improved}
...
## 在 Agent Playground 中上传 CSV {#csv-upload-in-agent-playground}
...
## 使用 Ask 排查问题 {#troubleshooting-with-ask}
...
### Agent 计费方式 {#how-agent-pricing-works}
...
### 并行 Agent 计费 {#parallel-agents-pricing}
