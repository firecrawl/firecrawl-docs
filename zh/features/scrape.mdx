...
import ScrapeQueryPython from "/snippets/zh/v2/scrape/query/python.mdx";
import ScrapeQueryNode from "/snippets/zh/v2/scrape/query/js.mdx";
import ScrapeQueryCURL from "/snippets/zh/v2/scrape/query/curl.mdx";
...
<Info>
  每次抓取会消耗 1 个额度。某些选项会产生额外消耗：JSON 模式每页额外消耗 4 个额度，查询格式每页额外消耗 4 个额度，高级代理 (enhanced proxy) 每页额外消耗 4 个额度，PDF 解析每个 PDF 页面额外消耗 1 个额度，音频提取每页额外消耗 4 个额度。
</Info>
...
`query` 格式支持你针对页面提出一个自然语言问题。Firecrawl 会抓取页面，并对内容运行 LLM，在响应的 `answer` 字段中返回答案。适用于无需编写 JSON schema、直接针对某个 URL 进行一次性问答的场景。

<Info>
  `query` 格式每页消耗 5 个额度 (1 个基础额度 + 4 个 LLM 调用的额外额度) 。
</Info>

format object 中的选项：

* `prompt` (必填) ：要回答的问题。最多 10,000 个字符。
* `mode` (可选) ：`"freeform"` 会基于页面内容回答问题。`"directQuote"` 会返回页面中的相关原文。默认值为 `"freeform"`。

你可以将 `query` 与其他 formats 结合使用——例如，同时请求 `markdown` 和 `query`，即可在一次调用中同时获得页面内容和答案。
...
`query` 格式也可通过 `scrapeOptions` 在 `/search` 中使用，这会对每个搜索结果执行相同的问答。
