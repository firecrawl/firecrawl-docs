...
<Info>
  每次抓取会消耗 1 个额度。某些选项会产生额外消耗：JSON 模式每页额外消耗 4 个额度，question 和 highlights formats 每种格式每页额外消耗 4 个额度，高级代理 (enhanced proxy) 每页额外消耗 4 个额度，PDF 解析每个 PDF 页面额外消耗 1 个额度，音频提取每页额外消耗 4 个额度。
</Info>
