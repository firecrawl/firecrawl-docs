...
| 模型             | 成本              | 准确率     | 最适用场景               |
| -------------- | --------------- | ------- | ------------------- |
| `spark-2`      | 最低              | 可媲美 Pro | 所有任务 (默认值)          |
| `spark-1-mini` | 比 Pro **低 60%** | 标准      | 对成本敏感的 Spark 1 工作负载 |
| `spark-1-pro`  | 标准              | 更高      | 复杂研究、关键提取           |

<Tip>
  **Spark 2 为默认值**，因此可以省略 `model` 参数——在大多数任务中，它的成本更低，完成速度也比两种 Spark 1 模型更快。Spark 1 模型仍可使用：建议先选用 Mini，仅在进行复杂的多域研究或准确率至关重要时切换到 Pro。
</Tip>
...
`spark-2` 是我们最新的代理模型，也是所有请求的默认值。它无需在准确率与成本之间取舍：相比两种 Spark 1 模型，它更便宜、更快，同时拥有与 Spark 1 Pro 相当的准确率，且能完成更多运行任务。
...
`spark-2` 是默认值，因此 `model` 参数是可选的。通过传递该参数来选择其他模型：
