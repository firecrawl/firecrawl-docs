...
当搜索结果未被抓取（未指定任何抓取选项）时，费用为每 10 条搜索结果 2 个积分。启用抓取后，每个搜索结果的基础抓取不额外收取超出标准抓取费用的费用。

但请注意以下成本因素：

- **PDF 解析**：每个 PDF 页面 1 个积分（多页 PDF 可能显著增加成本）
- **隐身代理模式**：每个搜索结果额外 +4 个积分
- ***JSON 模式***：每个搜索结果额外 +4 个积分

控制成本的建议：

- 如不需要 PDF 内容，设置 `parsers: []`
- 尽可能使用 `proxy: "basic"` 而非 `"stealth"`
- 使用 `limit` 参数限制搜索结果数量
