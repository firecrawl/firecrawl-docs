...
当客户端需要启动本地进程、需要使用本地 HTTP 传输，或 MCP 服务器必须连接自托管 Firecrawl API 时，请在本地运行 Firecrawl MCP。对于托管服务，请使用[Agent MCP](/zh/mcp-server/agent-mcp)或[Human MCP](/zh/mcp-server/human-mcp)。
...
<Tabs>
  <Tab title="stdio">
    ...
    将客户端配置为运行 `npx -y firecrawl-mcp@3.23.4`，并通过客户端的受保护环境变量或密钥机制提供 `FIRECRAWL_API_KEY`。
  </Tab>
