...
请先使用 [MCP 设置](/zh/mcp-server)，选择免密钥、OAuth 或 API 密钥访问方式。某些可选工具可通过环境配置或团队策略禁用。
...
| 从已知 URL 提取结构化数据 | 使用 JSON 格式的 `firecrawl_scrape`                     | 您知道哪个页面包含数据，并希望输出遵循 prompt 或 schema。             |
| 在来源未知时查找并提取数据   | `firecrawl_agent` 和 `firecrawl_agent_status`       | 任务需要先在一个或多个来源中进行发现，再生成结构化结果。                     |
...
* **某个工具缺失：**确认 [MCP 设置](/zh/mcp-server) 的连接模式，重新连接或重启客户端，并检查团队策略是否禁用了可选工具。
