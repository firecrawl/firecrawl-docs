...
请先使用 [MCP 设置](/zh/mcp-server)，选择 OAuth、API 密钥或免密钥访问方式。某些可选工具可通过环境配置或团队策略禁用。
...
<AccordionGroup>
  ...
  <Accordion title="检查正在运行的爬取任务">
    `firecrawl_crawl` 通常会启动爬取并等待其进入终态。如果等待超时，请使用爬取 ID 和 `firecrawl_check_crawl_status` 继续查询。`firecrawl_agent` 是异步任务，应使用 `firecrawl_agent_status` 查询。
  </Accordion>
