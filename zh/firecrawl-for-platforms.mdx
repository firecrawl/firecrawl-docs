...
description: "供平台为其用户创建和管理 Firecrawl API 密钥的 Firecrawl for Platforms API 参考"
og:title: "Firecrawl for Platforms | Firecrawl"
og:description: "供平台为其用户创建和管理 Firecrawl API 密钥的 Firecrawl for Platforms API 参考"
---
...
Firecrawl for Platforms 让你的平台可以直接在自己的后端为用户创建和管理 Firecrawl API 密钥。用户无需离开你的平台即可开始使用 Firecrawl。

<Note>
  你可以在 [Firecrawl dashboard](https://www.firecrawl.dev/app/partner-api) 中自行配置 Firecrawl for Platforms。组织中的任何管理员或成员都可以创建集成：命名、选择一种集成类型、接受该类型对应的协议，然后复制平台密钥。该密钥仅显示一次。之后你可以在 Settings 中签发和吊销密钥。无需申请或审批。
</Note>

共有两种集成类型。两者开通账户的流程相同，都通过下方的接口完成；区别在于由谁为用户的使用量付费：

* **Standard**：你的用户向 Firecrawl 付费。每个开通的账户各自保留自己的 Firecrawl 方案和计费。用户从 Free 套餐起步，直接向 Firecrawl 升级。
* **Gateway**：由你的集成创建的账户会被纳入 Gateway，因此在用户自身的额度用尽后，其符合条件的使用量将计费到你的组织。Gateway 账户仅支持 API 调用，没有自己的 dashboard 登录入口。

集成类型在创建集成时选定，之后不易更改，因此我们建议在接受协议前仔细比较两种选项。请参见[Firecrawl for Platforms 页面](https://www.firecrawl.dev/firecrawl-for-platforms)了解概况。
...
所有 Platforms API 请求都必须在 `Authorization` 标头中附带您的平台密钥：

```bash
Authorization: Bearer <platform key>
```

平台密钥与标准 Firecrawl API 密钥不同。您可以在 [Firecrawl dashboard](https://www.firecrawl.dev/app/partner-api) 的 Settings &gt; Firecrawl for Platforms 中创建和吊销平台密钥。

<div id="security-requirements">
  ## 安全要求
</div>

* **仅限服务器端**：平台密钥只能用于服务器端代码。切勿在前端代码、客户端 JavaScript 或移动应用中暴露平台密钥。
...
在 [Firecrawl Dashboard](https://www.firecrawl.dev/app/partner-api) 中创建你的集成：登录后选择 Standard 或 Gateway，接受协议，然后复制你的平台密钥。该集成类型在完成后不易更改，请在接受协议前确认你的选择。之后即可从你的服务器调用上述接口。对配置方式有疑问？欢迎联系 [help@firecrawl.com](mailto:help@firecrawl.com)。想做更大规模的集成？欢迎联系 [partnerships@firecrawl.dev](mailto:partnerships@firecrawl.dev)。