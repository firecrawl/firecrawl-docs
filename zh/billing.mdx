...
少数域名的基础设施成本显著更高，因此会在基础抓取成本之外，按页收取**Premium 附加费**。Premium 附加费可与上述标准抓取选项的附加费用叠加计算。

| 域名      |       额外成本 |
| ------- | ---------: |
| `x.com` | +29 额度 / 页 |

例如，抓取 `x.com` 上的一个页面，每页成本为 `1（基础）+ 29（Premium）= 30 额度`。如果还启用了 JSON 格式，则每页总成本为 `1 + 29 + 4 = 34 额度`。
