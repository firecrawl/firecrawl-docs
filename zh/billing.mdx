...
Firecrawl 采用**基于额度的计费方式**。你发起的每一次 API 调用都会消耗额度，具体消耗数量取决于你使用的端点和选项。你的方案会为你提供每月额度配额。当配额用尽时，Auto-reload 可以购买更多额度。
...
订阅套餐支持按月或按年计费。自助套餐采用按量付费模式。当套餐额度用尽时，自动充值会补充额度。请参见[自动充值](#auto-reload)。
...
Firecrawl 自助套餐采用按量付费模式。当套餐额度用尽时，自动充值可确保你的请求继续运行。

自动充值每次以 5 美元为一批购买额度。当额度余额降至零时，自动充值会购买一批额度，并从已登记的银行卡扣款。

自动充值需要使用付费自助套餐。Free 套餐无法使用自动充值。

你也可以随时自行购买额度。在[计费设置](https://www.firecrawl.dev/app/settings?tab=billing)中点击**加载更多额度**。输入 5 美元的整数倍金额，并使用已登记的银行卡付款。无论自动充值是否开启，均可使用此方式。

<div id="credits-in-a-batch">
  ### 每批购买的额度
</div>

每批购买可获得的额度取决于你的套餐。自动充值和手动购买的费率相同。
...
你可以在以下两个位置设置**每月自动充值限额**：

* 在你的[计费设置](https://www.firecrawl.dev/app/settings?tab=billing)的 **Billing** 标签页中。
...
限额是自动充值在一个月内最多可消费的金额，并会向下取整为 5 美元的整数批次。

例如，限额为 25 美元时，每月可充值五个批次。限额为 22 美元时，每月可充值四个批次，因为 22 美元会向下取整为 20 美元。
...
如果额度耗尽且自动充值已关闭，消耗额度的请求将返回 **HTTP 402 (需要付费) **错误。

如果已开启自动充值，当余额降至零时，系统会购买一批价值 5 USD 的新额度。请求将继续执行。

要在强制停止后恢复使用，你可以：

1. 设置**每月自动充值限额**，以自动购买额度。请参见[自动充值](#auto-reload)。
2. 手动升级到更高的套餐
3. 等待额度在下一个计费周期重置
...
<AccordionGroup>
  ...
  <Accordion title="我购买的额度会过期吗？">
    无论是你自行购买，还是通过自动充值购买，额度都会保留在你的账户中，直到你使用为止。取消订阅后，这些额度会在当前计费周期结束时过期。续订不会影响这些额度。
  </Accordion>

  <Accordion title="每月自动充值限额如何运作？">
    你的限额会限制自动充值每月的消费金额。它会向下取整为完整的 5 美元批次。限额为 25 美元时，每月可进行五个批次。将限额留空，自动充值将没有每月限额。将限额设为 `0`，自动充值将关闭。
  </Accordion>
