...
| Prompt 注入检查               | +4 credits / page    | 面向 JSON 格式的可选 `checkPromptInjection` 防护检查 (请参见 [Prompt 注入检测](/zh/features/llm-extract#prompt-injection-detection)) 。如果检查执行后抓取失败——包括检测到注入且请求因 403 被封禁——将收取 5 额度，而非抓取失败通常收取的 0 额度。 |
...
<AccordionGroup>
  ...
  <Accordion title="如何管理订阅、更改方案或取消订阅？">
    ...
    如需更改方案，请点击 **Change Plan** 并选择新的档位。升级会立即生效；降级会安排在当前计费周期结束时生效，在此之前可撤销——请参见 [升级和降级](#upgrading-and-downgrading)。
