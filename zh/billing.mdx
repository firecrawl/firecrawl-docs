...
Firecrawl 使用**基于点数的计费系统**。你发起的每一次 API 调用都会消耗点数，具体消耗数量取决于你使用的端点和选项。你会根据自己的套餐每月获得一定的点数配额，如需更多点数，可以通过自动充值包购买额外点数。

有关当前套餐定价，请访问 [Firecrawl 定价页面](https://www.firecrawl.dev/pricing)。
...
<Note>
  对于超出 Growth 方案的大规模用量，Firecrawl 提供 **Scale** 和 **Enterprise** 方案，包含 100 万+ 额度、150+ 并发浏览器、专属支持、SLA、批量折扣、零数据留存以及 SSO。详情请访问 [Enterprise 页面](https://www.firecrawl.dev/enterprise)。
</Note>
...
如果你偶尔需要的额度超出当前方案包含的数量，可以在 Dashboard 中启用**自动充值**。当你的剩余额度降到某个阈值以下时，Firecrawl 会自动购买一个额外额度包并将其添加到你的余额中。默认阈值为 **0 额度**，但你可以在[计费设置](https://www.firecrawl.dev/app/settings?tab=billing)中自定义它，以便更早触发充值——例如，设置更高的阈值可以确保你始终保有一定的额度缓冲；如果你运行大型爬取任务，这会很有帮助。请注意，这项检查会在你发起 API 请求时进行——在下一次请求触发充值之前，你的余额不会自动补足。
...
如果你用完了额度配额且未启用自动充值，消耗额度的 API 请求将返回 **HTTP 402 (需要支付)&#x20;**&#x20;错误。如果你启用了自动充值，系统会在自动购买充值包的同时继续提供服务——但如果达到每月充值包上限或某次充值失败，在下一个计费周期之前，你的余额可能会变为负数。在被强制停止后要恢复使用，你可以：
...
免费套餐提供**一次性 500 点额度**，无需信用卡。这些额度不会重置——一旦用完，你需要升级到付费套餐才能继续使用 Firecrawl。与付费套餐相比，免费套餐的速率限制和并发上限更低 (参见 [速率限制](/zh/rate-limits)) 。
...
<AccordionGroup>
  <Accordion title="未使用的额度会结转到下个月吗？">
    **套餐额度**默认不会结转——你的每月额度会在每个月重置。**年度 Scale (扩展) 套餐的未使用套餐额度可结转 1 个月**，**年度 Enterprise 套餐可结转 2 个月**。通过**自动充值包**购买的额度不受计费周期限制，可以在任何套餐下结转。自动充值额度自购买之日起 **1 年**后到期。
  </Accordion>
  ...
  <Accordion title="在哪里使用优惠码？">
    这取决于优惠券类型。**额度优惠券**可在Dashboard的计费 部分中使用。**订阅优惠券** (即套餐价格折扣) 只能在订阅或更改套餐时于 Stripe 结账页面使用。
  </Accordion>

  <Accordion title="我需要更高的并发或定制套餐，该联系谁？">
    请联系 [help@firecrawl.dev](mailto:help@firecrawl.dev)，或访问 [Enterprise 页面](https://www.firecrawl.dev/enterprise) 了解更多定制套餐信息。
  </Accordion>
