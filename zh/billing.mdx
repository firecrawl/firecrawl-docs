...
| **Search** | 2 credits / 10 results | 搜索全网，并可选地抓取搜索结果。对每个被抓取的结果将按页面收取额外的抓取额度。 |
...
如果你偶尔需要的额度超出当前套餐包含的数量，可以在控制台中启用**自动充值**。当你的剩余额度低于可配置的阈值时，Firecrawl 会自动购买一份额外额度包并将其添加到你的余额中。

* 所有付费套餐都支持自动充值额度包
* 额度包的大小和价格因套餐而异 (可在[定价页面](https://www.firecrawl.dev/pricing)中查看)
* 你可以随时配置充值阈值，并开启或关闭自动充值
* 自动充值每月最多限购 **4 个额度包**。

自动充值最适合应对偶发的用量高峰。如果你发现自己经常超出当前套餐的额度，升级到更高档位的套餐通常能获得更划算的单额度价格。
...
* **额度优惠券**会为你的账户增加额外额度。你可以在 Dashboard 的 **计费** 部分，通过 [firecrawl.dev/app/billing](https://www.firecrawl.dev/app/billing) 进行兑换。前往计费页面，找到优惠券输入框并输入代码即可使用。
...
* **月度计划**：点数会在每月续费日重置
* **年度计划**：按年计费，但点数仍会在每月的虚拟月度续费日重置
* **未使用的点数不会结转** —— 在每个计费周期开始时，你的配额会重置为对应计划的额度
...
<AccordionGroup>
  <Accordion title="未使用的额度会结转到下个月吗？">
    不会。你的额度会在每个计费周期开始时重置，未使用的额度不会结转到下一个周期。
  </Accordion>
