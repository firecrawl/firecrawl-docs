...
<AccordionGroup>
  <Accordion title="未使用的额度会结转到下个月吗？">
    **套餐额度**不会结转——你的每月额度会在每个计费周期开始时重置。不过，通过**自动充值包**购买的额度不受计费周期限制，可以结转。自动充值额度自购买之日起 **1 年**后到期。
  </Accordion>
  ...
  <Accordion title="在哪里使用优惠码？">
    这取决于优惠券类型。**额度优惠券**可在仪表盘的计费 部分中使用。**订阅优惠券** (即套餐价格折扣) 只能在订阅或更改套餐时于 Stripe 结账页面使用。
  </Accordion>
