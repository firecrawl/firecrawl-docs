...
## 选择本地开发还是自行托管 {#choose-local-development-or-self-hosting}
...
## 启动 Firecrawl 开发环境 {#start-the-firecrawl-development-environment}

### 安装前置依赖 {#install-the-prerequisites}
...
### 克隆 Firecrawl 并安装依赖 {#clone-firecrawl-and-install-dependencies}
...
### 启动 Redis 和 Firecrawl {#start-redis-and-firecrawl}
...
### 验证本地抓取 {#verify-one-local-scrape}
...
## 修改并测试 Firecrawl {#change-and-test-firecrawl}
...
## 排查开发环境问题 {#troubleshoot-the-development-environment}

### Redis 无法连接 {#redis-does-not-connect}

确认 Redis 正在监听 `localhost:6379`，并确保 `apps/api/.env` 中的两个 Redis URL 都使用该地址。

### 测试框架无法启动 PostgreSQL 或 RabbitMQ {#the-harness-cannot-start-postgresql-or-rabbitmq}

启动 Docker 或 Podman，然后重新运行 `pnpm start`。如果这些服务由你自行管理，请显式设置其连接 URL，而不要依赖测试框架管理的容器。

### 端口 3002 已被占用 {#port-3002-is-already-in-use}

请停止其他进程，或在 `apps/api/.env` 中更改 `PORT`，然后在验证请求中使用相同的端口。

### 基本抓取可用，但浏览器渲染不可用 {#basic-fetch-works-but-browser-rendering-does-not}
