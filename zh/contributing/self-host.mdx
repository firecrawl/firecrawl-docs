...
## 选择自托管或使用 Firecrawl Cloud {#choose-self-hosting-or-firecrawl-cloud}

### 在以下情况下自托管 {#self-host-firecrawl-when}
...
### 自托管需要承担的事项 {#what-self-hosting-requires}
...
## 使用 Docker Compose 自托管 Firecrawl {#self-host-firecrawl-with-docker-compose}

### 采用以下默认设置 {#start-with-these-defaults}
...
### 克隆经验证的版本 {#clone-the-verified-release}
...
### 配置评估环境部署 {#configure-the-evaluation-deployment}
...
### 构建并启动 Firecrawl {#build-and-start-firecrawl}
...
### 检查 API 是否可访问 {#check-api-reachability}
...
### 运行功能冒烟测试 {#run-a-functional-smoke-test}
...
## 自托管功能支持 {#self-hosted-feature-support}
...
### 你正在绕过身份验证 {#youre-bypassing-authentication}
...
### Docker 容器无法启动 {#docker-containers-fail-to-start}
...
### Redis 连接问题 {#connection-issues-with-redis}
...
### API 端点无响应 {#api-endpoint-does-not-respond}
...
### 抓取请求超时 {#scrape-request-times-out}
