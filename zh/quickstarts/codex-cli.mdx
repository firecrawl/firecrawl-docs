...
* 使用[免密钥 MCP](/zh/mcp-server/keyless-api-key#try-instantly)试用 Search、Scrape 和 Parse，无需账户或密钥。
* 使用 `bearer_token_env_var` [配置 API 密钥](/zh/mcp-server/keyless-api-key#add-an-api-key)，以便无人值守运行。请勿在代理聊天或源代码管理中泄露密钥。
