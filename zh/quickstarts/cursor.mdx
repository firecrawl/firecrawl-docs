...
此配置使用免密钥的 Firecrawl 工具集。如需连接账户的设置，请参见[Human MCP](/zh/mcp-server/human-mcp)，或如需无人值守设置，请参见[Agent MCP](/zh/mcp-server/agent-mcp)；请将 API 密钥保存在 Authorization 请求头或安全密钥存储中，切勿放在 URL 中。
