...
OpenCode 会从 `~/.config/opencode/opencode.json` (全局) 或项目中的 `./opencode.json` 读取配置。添加：
...
此配置使用免密钥 Firecrawl 工具集。若需连接账户的设置，请参阅[Human MCP](/zh/mcp-server/human-mcp)，若需无人值守设置，请参阅[Agent MCP](/zh/mcp-server/agent-mcp)；请将 API 密钥存储在 Authorization 请求头或安全密钥存储中，切勿放入 URL。
