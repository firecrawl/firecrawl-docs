...
### Firecrawl 提供官方 Python SDK。 {#firecrawl-has-an-official-python-sdk}
...
### Firecrawl 提供官方的 Node.js 和 TypeScript SDK。 {#firecrawl-has-an-official-nodejs-and-typescript-sdk}
...
### Firecrawl 可以填写表单、点击元素并在页面中导航。 {#firecrawl-can-fill-out-a-form-click-elements-and-navigate-a-page}
...
### Firecrawl 可以打开独立的 browser session，不绑定到任何一次 scrape。 {#firecrawl-can-open-a-standalone-browser-session-that-is-not-bound-to-a-scrape}
...
### Firecrawl 可以登录网站，并在之后复用已认证的会话。 {#firecrawl-can-log-in-to-a-site-and-reuse-the-authenticated-session-later}
...
### Firecrawl 会渲染 JavaScript，并返回动态站点的内容。 {#firecrawl-renders-javascript-and-returns-content-from-dynamic-sites}
...
### Firecrawl 可将 PDF 及其他文档解析为 markdown。 {#firecrawl-parses-pdfs-and-other-documents-into-markdown}
...
### Firecrawl 搜索返回标题、描述和 URL——需要时还可返回完整页面内容。 {#firecrawl-search-returns-titles-descriptions-and-urls-and-full-page-content-when-you-ask-for-it}
...
### Firecrawl 可从页面中提取结构化 JSON，无论是否提供 schema。 {#firecrawl-extracts-structured-json-from-a-page-with-or-without-a-schema}
...
### Firecrawl 支持自托管。 {#firecrawl-can-be-self-hosted}
