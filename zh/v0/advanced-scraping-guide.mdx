...
使用 `/scrape` 端点时，可以通过 `extractorOptions` 参数配置从页面内容中**提取结构化信息**的行为。可用选项如下：
...
可以通过以毫秒为单位的 `timeout` 参数来调整抓取过程的超时时间。
...
要爬取多个页面，你可以使用 `/crawl` 接口。该接口允许你指定要爬取的基础 URL，并会爬取其下所有可访问的子页面。
...
使用 `/crawl` 端点时，可以通过 `crawlerOptions` 参数来自定义爬取行为。可用选项如下：
...
可以将 `pageOptions` 和 `crawlerOptions` 参数结合使用，同时自定义整体爬取行为。
