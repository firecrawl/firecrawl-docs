...
当请求来自 Firecrawl 官方客户端——[MCP server](/zh/mcp-server)、[CLI](/zh/sdks/cli) 或 SDK——时，抓取、搜索和交互**无需 API 密钥**即可使用。除此之外，其他端点 (如 crawl、extract、map、batch scrape 等) 在没有密钥的情况下均不可用。
