...
对于 Firecrawl 官方客户端、CLI、SDKs 和 REST API，无密钥访问还包括**交互**。在启用了研究索引的 Firecrawl Cloud 上，研究端点也可以在无需 API 密钥的情况下使用。除此之外，其他端点 (如 crawl、extract、map、batch scrape 等) 在没有密钥的情况下均不可用。
