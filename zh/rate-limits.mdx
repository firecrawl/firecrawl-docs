...
请注意，在队列中等待的时间也会计入该请求的 [`timeout`](/zh/advanced-scraping-guide#timing-and-cache) 参数，因此你可以设置较低的 `timeout` 以快速失败，而不是等待。若要在发送任务前查看当前可用情况，请调用 [Queue Status](/zh/api-reference/endpoint/queue-status) 端点。
