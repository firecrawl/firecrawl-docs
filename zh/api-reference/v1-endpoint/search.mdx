---
title: 'Search'
openapi: '/zh/api-reference/v1-openapi.json V1-OPENAPI POST /search'
---

> 注意：现已推出该 API 的全新 [v2 版本](/zh/api-reference/endpoint/search)，功能与性能均有升级。

该 /search 端点将网页搜索（SERP）与 Firecrawl 的抓取能力相结合，可为任意查询返回完整页面内容。

在请求中包含 `scrapeOptions` 并设置 `formats: ["markdown"]`，即可为每个搜索结果获取完整的 markdown 内容；否则将默认仅返回 SERP 结果（url、title、description）。
