...
该 /search 端点将网页搜索（search）与 Firecrawl 的抓取能力相结合，可为任意查询返回完整页面内容。

在请求中包含 `scrapeOptions` 并设置 `formats: ["markdown"]`，即可为每个搜索结果获取完整的 markdown 内容；否则将默认仅返回 search 结果（url、title、description）。
