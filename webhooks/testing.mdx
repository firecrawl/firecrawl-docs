...
Two issues account for most signature mismatches:

- **Parsed body** - Compute the HMAC over the raw request body, not `JSON.stringify(req.body)`. Re-serializing changes whitespace and key order, which breaks the signature.
- **Wrong secret** - Confirm your secret matches the value in your [account settings](https://www.firecrawl.dev/app/settings?tab=advanced).
