...
| Field         | Description                                           |
| ------------- | ----------------------------------------------------- |
| `creditsUsed` | Estimated credits consumed so far                     |
| `action`      | The tool that was executed (e.g., `scrape`, `search`) |
| `input`       | The parameters passed to the tool                     |

<Note>
  The `creditsUsed` value in `action` events is an **estimate**. The final
  accurate credit count is only available in the `completed`, `failed`, or
  `cancelled` events.
</Note>
