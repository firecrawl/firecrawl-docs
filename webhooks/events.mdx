...
Scrape, batch scrape, crawl, search, and webhook document payloads use the same document metadata shape. An attested cache hit can include `metadata.cache`; its absence does not prove a miss, fresh scrape, bypass, or page liveness.
