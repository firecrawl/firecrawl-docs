...
- **API `401`** — an unauthenticated call to `https://api.firecrawl.dev` returns `WWW-Authenticate: Bearer resource_metadata="https://www.firecrawl.dev/.well-known/oauth-protected-resource"`. Follow that metadata chain in [`auth.md`](https://www.firecrawl.dev/auth.md) Step 1 (Path B). You do not need this page.
- **Docs** — you landed here. Use the decision rule below.
