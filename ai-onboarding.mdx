...
Once you have an API key, continue with [CLI](#cli) below.
...
<Card title="MCP Server" icon="plug" href="/mcp-server/human-mcp">
  View hosted setup for Codex, Claude Desktop, Claude Code, Cursor, and other
  compatible clients. For Windsurf, use the dedicated
  [Windsurf quickstart](/quickstarts/windsurf). For an unattended agent or CI
  setup instead, see [Agent MCP](/mcp-server/agent-mcp).
</Card>
...
<Steps>
  ...
  <Step title="MCP docs server">
    For a structured approach using MCP tools, connect the Firecrawl MCP server in Codex, Cursor, Claude Code, Claude Desktop, or another compatible client. See [Human MCP](/mcp-server/human-mcp) for interactive hosted setup, or [Agent MCP](/mcp-server/agent-mcp) for an unattended agent. Windsurf users should follow the [Windsurf quickstart](/quickstarts/windsurf).
  </Step>
