...
This page routes you to the right place for a bearer token. Registration steps live in canonical docs — not here.

**How you might arrive:**

- **API `401`** — an unauthenticated call to `https://api.firecrawl.dev` returns `WWW-Authenticate: Bearer resource_metadata="https://www.firecrawl.dev/.well-known/oauth-protected-resource"`. Follow that metadata chain to [`auth.md`](https://www.firecrawl.dev/auth.md). You do not need this page.
- **Docs** — you landed here. Use the paths below.
- **Direct URL** — fetch [`auth.md`](https://www.firecrawl.dev/auth.md) and follow it.

<CardGroup cols={2}>
  <Card
    title="Agents — WorkOS registration"
    icon="robot"
    href="https://www.firecrawl.dev/auth.md"
  >
    Register via WorkOS ID-JAG and `POST /agent/auth`. See [Agent Auth](/ai-onboarding/agent-auth) for a short overview, then follow [`auth.md`](https://www.firecrawl.dev/auth.md) Steps 1–4.
  </Card>
  <Card
    title="Humans — dashboard API key"
    icon="key"
    href="https://www.firecrawl.dev/app/api-keys"
  >
    Create an account and API key in the dashboard. Paste the key into your project or hand it to your agent.
  </Card>
</CardGroup>
...
Once you have a bearer token, continue with [Skills + CLI](#skills-cli) below for CLI install, skills, and MCP setup.
...
Agent registration lives at [`firecrawl.dev/auth.md`](https://www.firecrawl.dev/auth.md) — agents can fetch it directly for WorkOS ID-JAG registration.
...
<Steps>
  <Step title="Get credentials">
    **Agents:** register via [`auth.md`](https://www.firecrawl.dev/auth.md) (see [Agent Auth](/ai-onboarding/agent-auth) for an overview). **Humans:** create an [API key in the dashboard](https://www.firecrawl.dev/app/api-keys), then continue with [Skills + CLI](#skills-cli) for CLI install and MCP setup.
  </Step>
