...
Firecrawl supports two credential paths: human-in-the-loop CLI/browser onboarding when a person needs to sign in or create a dashboard API key, and WorkOS ID-JAG registration for identity-aware agents.
...
**Can you mint a WorkOS ID-JAG for `aud=https://api.firecrawl.dev/`?** No or unsure → Path A (default). Yes → Path B.
...
<CardGroup cols={2}>
  ...
  >
    **No or unsure → Path A (default).** Browser sign-in, CLI `--browser`, skills and MCP install, or a human-created API key from the dashboard. Use this when you cannot mint a WorkOS ID-JAG.
  </Card>
  <Card
