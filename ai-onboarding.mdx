...
WorkOS `auth.md` replaces the deprecated **agent-onboarding signup endpoint** for agentic self-serve. `agent-onboarding/SKILL.md` remains active for human-in-the-loop onboarding (CLI browser sign-in); the dashboard is a manual fallback within that path.

**How you might arrive:**

- **API `401`** — an unauthenticated call to `https://api.firecrawl.dev` returns `WWW-Authenticate: Bearer resource_metadata="https://www.firecrawl.dev/.well-known/oauth-protected-resource"`. Follow that metadata chain to the machine registration doc. You do not need this page.
- **Docs** — you landed here. Use the decision below.
- **Direct URL** — if you already have `auth.md` or `SKILL.md`, fetch and follow that doc.

**Can you mint a WorkOS ID-JAG for `aud=https://api.firecrawl.dev/`?** Yes → Path A. No or unsure → Path B (default).

<CardGroup cols={2}>
  <Card
    title="Path B — Human + agent assist (default)"
    icon="user"
    href="https://www.firecrawl.dev/agent-onboarding/SKILL.md"
  >
    **No or unsure → Path B (default).** Browser sign-in, CLI `--browser`, skills and MCP install, or a human-created API key from the dashboard. Use this when you cannot mint a WorkOS ID-JAG.
  </Card>
  <Card
    title="Path A — Agentic registration (WorkOS ID-JAG)"
    icon="robot"
    href="/ai-onboarding/agent-auth"
  >
    **Yes — ID-JAG capable.** Identity-aware agents register via `POST /agent/auth`. See the [Agent Auth](/ai-onboarding/agent-auth) overview, then follow the canonical [`auth.md`](https://www.firecrawl.dev/auth.md) spec for claim requirements and steps.
  </Card>
</CardGroup>

<Card
  title="Get your API key (Path B)"
  icon="key"
...
<Note>
  Firecrawl does **not** currently support anonymous registration, verified-email-only registration, claim flows, or revocation endpoints via `/agent/auth`. See [`auth.md`](https://www.firecrawl.dev/auth.md) for supported methods.
</Note>
...
- Human + agent assist (default): [`firecrawl.dev/agent-onboarding/SKILL.md`](https://www.firecrawl.dev/agent-onboarding/SKILL.md)
- Machine registration (ID-JAG): [`firecrawl.dev/auth.md`](https://www.firecrawl.dev/auth.md)

Agents can fetch either doc directly for self-onboarding.
...
<Steps>
  <Step title="Credential routing">
    Start at [Get credentials](/ai-onboarding#get-credentials) to pick Path B (default, human + agent assist) or Path A (WorkOS ID-JAG). For ID-JAG registration, see [Agent Auth](/ai-onboarding/agent-auth) and the canonical [`auth.md`](https://www.firecrawl.dev/auth.md) spec.
  </Step>
