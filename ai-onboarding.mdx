...
Firecrawl users can get an API key in three ways. Most users should sign in through the dashboard or CLI. If your agent platform supports WorkOS ID-JAG, it can register directly with Firecrawl. Projects that already use the Stripe CLI can provision Firecrawl through Stripe Projects.
...
- **Stripe Projects:** `stripe projects add firecrawl/api --name firecrawl` provisions the account and writes `FIRECRAWL_API_KEY` to `.env`. See [Stripe Projects](/integrations/stripe-projects).
...
<CardGroup cols={2}>
  ...
  <Card
    title="Provision through Stripe Projects"
    icon="stripe"
    href="/integrations/stripe-projects"
  >
    **For projects on the Stripe CLI.** `stripe projects add firecrawl/api --name firecrawl` creates the account, writes `FIRECRAWL_API_KEY` to `.env`, and bills paid plans through Stripe.
  </Card>
</CardGroup>
