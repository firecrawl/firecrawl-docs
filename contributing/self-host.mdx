...
3. **Browser Sandbox is cloud-only:** Browser Sandbox sessions (the `/browser` endpoints and agent-browser workflows) depend on the managed browser service and are not available in the self-hosted stack.
