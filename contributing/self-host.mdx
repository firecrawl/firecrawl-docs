...
1. **Engine selection and limitations:** Firecrawl uses a single live engine selected by environment variables: `FIRE_ENGINE_BETA_URL` → Fire Engine, otherwise `PLAYWRIGHT_MICROSERVICE_URL` → Playwright, otherwise fetch. If Fire Engine is not configured, actions are not supported and other unsupported features return partial results with a `warning`.
2. **Markdown parser required:** HTML → Markdown conversion requires `HTML_TO_MARKDOWN_SERVICE_URL` or the Go parser (`USE_GO_MARKDOWN_PARSER=true` with the native library available). Without one, scrapes fail.
3. **Manual configuration required:** If you need services beyond basic fetch, such as Playwright or proxies, you will need to configure them in the `.env` file. This requires a deeper understanding of the technologies and might involve more setup time.

| Capability | Cloud | Self-hosting |
| --- | --- | --- |
| All API endpoints supported | Yes | Not always; `/agent` is not supported in self-hosting |
...
## === Scrape engines ===
# Engine selection is deterministic: FIRE_ENGINE_BETA_URL > PLAYWRIGHT_MICROSERVICE_URL > fetch
# FIRE_ENGINE_BETA_URL=http://fire-engine:3000/scrape
# PLAYWRIGHT_MICROSERVICE_URL=http://playwright-service:3000/scrape

## === Markdown parser (required for HTML -> Markdown conversion) ===
# HTML_TO_MARKDOWN_SERVICE_URL=http://html-to-markdown:8080
...
# These are now autoconfigured by the docker-compose.yaml. You shouldn't need to set them.
