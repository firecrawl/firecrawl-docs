...
<Note>
  **Sem chave de API? Scraping, busca e interação ainda funcionam.** Conecte-se à URL remota sem chave `https://mcp.firecrawl.dev/v2/mcp` (ou execute o servidor localmente sem `FIRECRAWL_API_KEY`), e as ferramentas `firecrawl_scrape`, `firecrawl_search` e `firecrawl_interact` recorrem ao plano gratuito sem chave — gratuito, mas com limite de taxa por IP e indisponível para outras ferramentas (rastreamento, extração, mapeamento etc.). Defina `FIRECRAWL_API_KEY` para desbloquear todas as ferramentas e limites mais altos. Veja [Rate Limits](/pt-BR/rate-limits#keyless-no-api-key).
</Note>
...
Ou conecte-se sem uma API key para começar — scraping, busca e interação funcionam no tier Free sem chave (com limite de requisições por IP):
