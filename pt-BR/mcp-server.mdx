...
<Note>
  **Sem chave de API? Scraping, busca, interação e parse funcionam na URL remota sem chave.** Conecte-se a `https://mcp.firecrawl.dev/v2/mcp` para usar o plano gratuito sem chave — gratuito, mas com limite de taxa por IP e indisponível para outras ferramentas (rastreamento, extração, mapeamento etc.). Defina `FIRECRAWL_API_KEY` para desbloquear todas as ferramentas e limites mais altos. Veja [Rate Limits](/pt-BR/rate-limits#keyless-no-api-key).
</Note>
...
Com uma API key (desbloqueia todas as ferramentas, além de limites maiores):

```bash
https://mcp.firecrawl.dev/{FIRECRAWL_API_KEY}/v2/mcp
```

Ou conecte-se sem uma API key para começar — scraping, busca, interação e parse funcionam no tier Free remoto sem chave (com limite de requisições por IP):
...
<div id="10-create-browser-session-firecrawl_browser_create">
  ### 10. Criar sessão do navegador (`firecrawl_browser_create`)
</div>
...
<div id="11-execute-code-in-browser-firecrawl_browser_execute">
  ### 11. Executar código no navegador (`firecrawl_browser_execute`)
</div>
...
<div id="12-delete-browser-session-firecrawl_browser_delete">
  ### 12. Excluir sessão do navegador (`firecrawl_browser_delete`)
</div>
...
<div id="13-list-browser-sessions-firecrawl_browser_list">
  ### 13. Listar sessões do navegador (`firecrawl_browser_list`)
</div>
...
<div id="14-interact-with-scraped-page-firecrawl_interact">
  ### 14. Interagir com página extraída (`firecrawl_interact`)
</div>
...
<div id="15-stop-interact-session-firecrawl_interact_stop">
  ### 15. Encerrar sessão de interação (`firecrawl_interact_stop`)
</div>
