...
Ao usar o endpoint `/scrape`, você pode especificar opções para **extrair informações estruturadas** do conteúdo da página usando o parâmetro `extract`. Aqui estão as opções disponíveis:
...
Ao usar o endpoint `/scrape`, o Firecrawl permite que você execute várias ações em uma página web antes de extrair seu conteúdo. Isso é especialmente útil para interagir com conteúdo dinâmico, navegar entre páginas ou acessar conteúdo que requer interação do usuário.
...
Usado para verificar o status de um crawl job e recuperar seu resultado.
...
Se o conteúdo for maior que 10 MB ou se o job de crawl ainda estiver em execução, a resposta incluirá um parâmetro `next`. Esse parâmetro é uma URL para a próxima página de resultados. Você pode usar esse parâmetro para obter a próxima página de resultados.
...
<div id="mapping-website-links-with-map">
  ## Mapeando links de um site com `/map`
</div>

O endpoint `/map` é especializado em identificar URLs contextualmente relacionadas a um determinado site. Esse recurso é essencial para entender o contexto de links de um site, o que pode contribuir significativamente para a análise estratégica e o planejamento da navegação.
