...
Inclua `scrapeOptions` com `formats: [{"type": "markdown"}]` para obter o conteúdo completo em markdown para cada resultado de busca; caso contrário, por padrão, você receberá apenas os resultados da search (url, título, descrição). Você também pode usar outros formatos, como `{"type": "summary"}`, para conteúdo condensado.
