...
Pesquise na web, em imagens e em notícias de uma só vez:

```json
{
  "query": "firecrawl web scraping",
  "sources": [
    {"type": "web", "tbs": "qdr:w", "location": "EUA"},
    {"type": "images"},
    {"type": "news"}
  ]
}
...
v1: lista simples de resultados. v2: organizado por tipo de fonte:
...
- Resultados limitados a 500 durante a fase alpha

O endpoint de busca combina pesquisa na web (SERP) com os recursos de scraping do Firecrawl para retornar o conteúdo completo da página para qualquer consulta.

Inclua `scrapeOptions` com `formats: [{"type": "markdown"}]` para obter o conteúdo completo em markdown para cada resultado de busca; caso contrário, por padrão, você receberá apenas os resultados da SERP (url, título, descrição). Você também pode usar outros formatos, como `{"type": "summary"}`, para conteúdo condensado.
