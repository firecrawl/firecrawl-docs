...
O endpoint /search combina a pesquisa na web (SERP) com os recursos de scraping do Firecrawl para retornar o conteúdo completo da página para qualquer consulta.

Inclua `scrapeOptions` com `formats: [{"type": "markdown"}]` para obter o conteúdo completo em Markdown de cada resultado da busca; caso contrário, por padrão você receberá apenas os resultados da SERP (url, title, description). Você também pode usar outros formatos, como `{"type": "summary"}`, para conteúdo condensado.
