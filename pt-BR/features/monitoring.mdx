...
Os monitores do Firecrawl executam scraping ou rastreamentos recorrentes e comparam cada resultado com o último snapshot retido. Use os monitores para acompanhar páginas de produto, documentação, blogs, changelogs, sites de concorrentes ou qualquer página em que as mudanças sejam importantes.
...
Use `judgeEnabled: false` se quiser armazenar um objetivo sem avaliar as mudanças ainda. O avaliador só é executado quando o monitor tem `judgeEnabled` e um `goal` não vazio.
...
Bons objetivos são curtos e explícitos: diga o que deve acionar um alerta, reforce qualquer escopo, como top N, preço, tipo de cargo, empresa, região, tópico, status ou entidade, e inclua exclusões apenas quando fizerem parte da intenção. Se o objetivo for amplo, mantenha-o amplo; por exemplo, &quot;qualquer mudança&quot; não deve adicionar filtros de ruído que ocultem mudanças.

Por exemplo, um monitor com este objetivo:
...
pode extrair resultados úteis do webhook `monitor.page`:
...
app.post("/webhooks/firecrawl", async (req, res) => {
  ...
  for (const page of event.data) {
    if (!page.isMeaningful || !page.judgment?.meaningful) continue;
...
O intervalo mínimo é de 15 minutos. As respostas da API sempre retornam a expressão cron normalizada. Os agendamentos em texto são distribuídos com base no ID do monitor antes de serem convertidos para cron, para que vários monitores não sejam executados todos no mesmo instante.
