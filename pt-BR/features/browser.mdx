...
Por padrão, cada sessão do navegador começa em um estado limpo. Com `persistentSession`, você pode salvar cookies, localStorage e outros estados do navegador para que sejam mantidos em sessões futuras. Isso é útil para permanecer logado, preservar preferências ou retomar fluxos de trabalho com várias etapas.
...
| `writeMode` | `"readwrite"` | `"readwrite"` salva as alterações de volta. `"readonly"` carrega os dados existentes sem gravar — útil quando você precisa de vários leitores concorrentes. |

<Note>
  Apenas uma sessão pode gravar em um perfil por vez. Se outra sessão já estiver gravando, você receberá um erro `409`. Você ainda pode abrir o mesmo perfil em modo `"readonly"` ou tentar novamente mais tarde.
</Note>
