...
Por padrão, cada sessão do navegador começa em um estado limpo. Com `profile`, você pode salvar e reutilizar o estado do navegador entre sessões. Isso é útil para permanecer logado, preservar preferências ou retomar fluxos de trabalho com várias etapas.
...
O estado da sessão do navegador só é salvo quando a sessão é encerrada. Portanto, recomendamos fechar a sessão do navegador quando terminar de usá-la, para que ela possa ser reutilizada. Para salvar e fechar:
