...
O endpoint `/agent` é a próxima evolução do nosso endpoint [`/extract`](/pt-BR/features/extract). Ele usa agentes de IA para coletar e estruturar dados de forma inteligente em toda a web. Diferente de `/extract`, o Agent requer apenas um prompt — URLs são opcionais — o que o torna extremamente flexível para uma grande variedade de consultas.

<Info>Agent é o sucessor de `/extract`: mais confiável, mais rápido e mais barato.</Info>

<div id="why-agent-over-extract">
  ## Por que usar Agent em vez de `/extract`?
</div>

Agent aproveita tudo que há de melhor em `/extract` e vai além:

* **Só prompt**: Basta descrever o que você precisa — sem precisar informar URLs
* **Mais inteligente**: Agentes de IA entendem o contexto e encontram dados relevantes automaticamente
* **Mais rápido**: Processamento otimizado para resultados mais ágeis
* **Mais econômico**: Uso mais eficiente de recursos significa custos menores
* **Flexível**: Funciona com uma ampla variedade de consultas e casos de uso

<div id="using-agent">
  ## Usando `/agent`
</div>

O único parâmetro obrigatório é `prompt`. Basta descrever os dados que deseja extrair.

<div id="basic-usage-prompt-only">
  ### Uso básico (somente prompt)
</div>
...
Para obter uma saída estruturada, forneça um schema JSON. Os SDKs oferecem suporte a Pydantic (Python) e Zod (Node) para definições de schema com segurança de tipos:
