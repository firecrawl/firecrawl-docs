...
## Fornecendo URLs (Opcional) {#providing-urls-optional}
...
## Status e conclusão de jobs {#job-status-and-completion}
...
### Estados possíveis {#possible-states}
...
#### Exemplo pendente {#pending-example}

<AgentStatusPending />

#### Exemplo concluído {#completed-example}

<AgentStatusCompleted />

## Listando execuções de agentes {#listing-agent-runs}
...
## Acompanhando uma execução em andamento {#following-a-run-in-progress}
...
## Rastros de execução e snapshots {#execution-traces-and-snapshots}
...
## Obtendo os dados de origem do agente {#getting-the-agents-source-data}
...
## Compartilhar execuções de agentes {#share-agent-runs}
...
### Especificando um modelo {#specifying-a-model}
...
## Agent vs Extract: O que melhorou {#agent-vs-extract-whats-improved}
...
## Exemplos de Casos de Uso {#example-use-cases}
...
## Upload de CSV no Agent Playground {#csv-upload-in-agent-playground}
...
## Solução de problemas com Ask {#troubleshooting-with-ask}
...
### Como funciona o preço do Agent {#how-agent-pricing-works}
...
### Preços para Agentes em Paralelo {#parallel-agents-pricing}
...
### Gerenciando custos {#managing-costs}
