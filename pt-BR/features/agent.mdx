...
| `cancelled` | O job foi cancelado pelo usuário via `DELETE /v1/agent/{jobId}` |
...
Envie seu arquivo CSV, escreva um prompt descrevendo quais dados deseja que o agente encontre para cada linha, defina seus campos de saída e execute. O agente processa cada linha em paralelo e preenche os resultados.
