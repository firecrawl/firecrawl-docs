...
<Note>Rastros e snapshots são registrados em execuções do Spark 2, ou seja, em todas as novas execuções — jobs iniciados em modelos Spark 1 antes de serem descontinuados não os incluem. Consulte as referências da API de [rastro](/pt-BR/api-reference/endpoint/agent-trace) e [snapshot](/pt-BR/api-reference/endpoint/agent-snapshot) para conferir o esquema completo de eventos.</Note>
