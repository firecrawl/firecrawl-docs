...
O agente Firecrawl oferece três modelos. **Spark 1 Pro** é o padrão. **Spark 2** é mais barato e mais rápido que os dois modelos Spark 1, com precisão comparável — defina `model` como `spark-2` para usá-lo.

| Modelo         | Custo                         | Precisão          | Ideal para                                       |
| -------------- | ----------------------------- | ----------------- | ------------------------------------------------ |
| `spark-2`      | Menor                         | Comparável ao Pro | Baixo custo e curto tempo de execução            |
...
<Tip>
  **Spark 1 Pro é o padrão**, portanto, uma solicitação sem o parâmetro `model` usa `spark-1-pro`. Defina `model` como `spark-2` para reduzir o custo e o tempo de execução, com precisão comparável. Defina `model` como `spark-1-mini` para reduzir o custo em 60% em tarefas simples do Spark 1.
</Tip>
...
`spark-2` é nosso modelo de agente mais recente. Ele abrange toda a gama de tarefas que antes exigiam escolher entre Mini e Pro, eliminando a necessidade de abrir mão da precisão para reduzir custos. Defina `model` como `spark-2` para usá-lo.

**Use o Spark 2 quando:**

* Você quiser o menor custo por execução
* O tempo de execução for importante
* Você normalmente escolheria a precisão do Spark 1 Pro
...
`spark-1-pro` é o nosso principal modelo, projetado para máxima precisão em tarefas complexas de extração. Ele é o padrão: uma solicitação que não define `model` usa `spark-1-pro`.

**Use o Pro quando:**

* Realizar análises de concorrência complexas
...
`spark-1-pro` é o padrão, portanto o parâmetro `model` é opcional. Informe-o para selecionar um modelo diferente:
...
| `model`      | string | Não         | Modelo a ser utilizado: `spark-1-pro` (padrão), `spark-1-mini` ou `spark-2`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
