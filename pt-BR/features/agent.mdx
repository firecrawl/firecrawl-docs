...
O Firecrawl Agent oferece três modelos. **Spark 2** é o padrão: mais barato e mais rápido que os dois modelos Spark 1, com precisão comparável. Ambos os modelos Spark 1 continuam disponíveis e inalterados.

| Modelo         | Custo                         | Precisão          | Ideal para                                      |
| -------------- | ----------------------------- | ----------------- | ----------------------------------------------- |
| `spark-2`      | Menor                         | Comparável ao Pro | Todas as tarefas (padrão)                       |
...
<Tip>
  **Spark 2 é o padrão**, então você pode omitir o parâmetro `model` — ele custa menos e conclui as tarefas mais rápido que os dois modelos Spark 1 na maioria dos casos. Os modelos Spark 1 continuam disponíveis: entre eles, comece pelo Mini e use o Pro apenas para pesquisas complexas em vários domínios ou quando a precisão for crítica.
</Tip>
...
`spark-2` é nosso modelo de agente mais recente e o padrão para todas as solicitações. Ele abrange toda a gama de tarefas que antes exigiam escolher entre Mini e Pro, eliminando a necessidade de abrir mão da precisão para reduzir custos.
...
`spark-2` é o padrão, portanto o parâmetro `model` é opcional. Informe-o para selecionar um modelo diferente:
...
| `model`      | string | Não         | Modelo a ser utilizado: `spark-2` (padrão), `spark-1-mini` ou `spark-1-pro`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
