...
import ScrapeQueryPython from "/snippets/pt-BR/v2/scrape/query/python.mdx";
import ScrapeQueryNode from "/snippets/pt-BR/v2/scrape/query/js.mdx";
import ScrapeQueryCURL from "/snippets/pt-BR/v2/scrape/query/curl.mdx";
...
<Info>
  Cada scrape consome 1 crédito. Créditos adicionais são cobrados para certas opções: modo JSON custa 4 créditos adicionais por página, o formato de consulta custa 4 créditos adicionais por página, proxy avançado custa 4 créditos adicionais por página e o processamento de PDFs custa 1 crédito por página de PDF, e a extração de áudio custa 4 créditos adicionais por página.
</Info>
...
O formato `query` permite fazer uma pergunta em linguagem natural sobre a página. O Firecrawl faz o scraping da página e executa um LLM no conteúdo, retornando a resposta no campo `answer` da resposta. Use-o para fazer uma pergunta única sobre uma URL sem precisar definir um schema JSON.

<Info>
  O formato `query` custa 5 créditos por página (1 base + 4 adicionais pela chamada ao LLM).
</Info>

Opções dentro do objeto de formato:

* `prompt` (obrigatório): a pergunta a ser respondida. Máximo de 10.000 caracteres.
* `mode` (opcional): `"freeform"` responde à pergunta usando o conteúdo da página. `"directQuote"` retorna o trecho relevante da página. O padrão é `"freeform"`.

Você pode combinar `query` com outros formatos — por exemplo, solicitar `markdown` e `query` juntos para obter tanto o conteúdo da página quanto a resposta em uma única chamada.
...
O formato `query` também está disponível em `/search` por meio de `scrapeOptions`, que executa o mesmo processo de perguntas e respostas em cada resultado de busca.
