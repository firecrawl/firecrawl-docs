...
A qualidade de um monitor em escala da web depende de duas alavancas com funções diferentes:

* **`queries` controlam o recall** — o que cada busca traz. Se forem estreitas demais, eventos reais nunca aparecem; se forem amplas demais, o judge gasta credits filtrando ruído.
* **`goal` controla a precisão** — quais resultados recuperados realmente geram alert. O judge avalia cada novo resultado em relação ao goal, então é o goal que separa uma correspondência real de um resultado relacionado ao tema, mas irrelevante.

Ajuste os dois. Um goal perfeito não pode gerar alert para um resultado que as queries nunca recuperaram, e queries amplas combinadas com um goal vago produzem alerts constantes de baixo valor.

**Boas queries se parecem com termos de busca, não com frases:**

* Use palavras-chave, não prosa — `OpenAI new model release`, não `tell me when OpenAI releases a new model`.
* Coloque entidades com várias palavras entre aspas (`"Llama 4"`) e agrupe sinônimos com `OR` (`launch OR release OR announcement`).
* Mantenha cada query enxuta (cerca de 2 a 6 termos). Uma query ampla geralmente é melhor do que várias estreitas — queries extras dividem o orçamento de `maxResults` sem aumentar a cobertura.
* Use uma query por assunto *distinto*. Um único assunto com várias facetas (&quot;launches, benchmarks, docs&quot;) ainda é uma query; só divida quando o goal realmente nomear entidades separadas (por exemplo, &quot;OpenAI, Anthropic, and Google&quot;).
...
* Desambigue termos com mais de um sentido: *&quot;Firecrawl (the web scraping API)&quot;* evita que o judge confunda com equipamentos de combate a incêndio.
* Adicione `Ignore ...` apenas para não correspondências específicas da intenção: *&quot;Ignore opinion pieces, tutorials, and unrelated AI news.&quot;* Não repita ruído genérico — o judge já lida com formatação, parâmetros de rastreamento e reindexações.
* Se a intenção for ampla, mantenha-a ampla. Goals restritivos demais suprimem correspondências reais.

**Como é um monitor saudável.** Um monitor em escala da web bem ajustado, na maior parte do tempo, não reporta nada — a maioria dos checks retorna `new: 0` — e só gera alert quando aparece algo genuinamente novo e alinhado ao goal. Leia o summary do check e o `searchStatus` de cada resultado (veja [Statuses and dedup](#statuses-and-dedup)) para saber se ele está bem ajustado:

* Um fluxo constante de resultados `ignored` significa que as queries trazem ruído que o goal depois rejeita — refine as queries (ou reduza `searchWindow`) para parar de pagar para o judge avaliar resultados que nunca geram alert.
* `watching` frequente significa que o goal está ambíguo — deixe os critérios de correspondência mais claros para que o judge possa decidir.
* Longos períodos sem resultados em um tópico ativo significam que as queries estão estreitas demais ou que `searchWindow` está curta demais — amplie os termos ou aumente a janela.
* Alerts que o usuário descarta significam que o goal está amplo demais — adicione um `Ignore ...` específico da intenção.

O resultado ideal é alta precisão com recall suficiente: cada alert vale a pena, e nada importante deixa de ser capturado.
...
* `new` — um resultado que correspondeu ao objetivo pela primeira vez. É isso que gera alertas.
...
| `ignored`      | `same`             | Novo resultado que o judge classificou como não relevante para o objetivo.                                         |
...
* [Objetivos e julgamento](/pt-BR/features/monitoring#goals-and-judging) — obrigatório para alvos de busca, a menos que `judgeEnabled: false`.
