...
Quando os resultados de pesquisa não são extraídos (nenhuma opção de scraping especificada), o custo é de 2 créditos a cada 10 resultados de pesquisa. Quando o scraping está habilitado, não há cobrança adicional por extrações básicas de cada resultado além dos custos padrão de scraping.

No entanto, fique atento a estes fatores de custo:

- **Processamento de PDF**: 1 crédito por página de PDF (pode aumentar significativamente os custos em PDFs de várias páginas)
...
- Defina `parsers: []` se você não precisar do conteúdo de PDFs
