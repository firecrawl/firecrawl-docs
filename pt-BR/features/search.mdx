...
* `developer`: Pesquise no [Índice para desenvolvedores](/pt-BR/features/developer) — issues do GitHub, pull requests mescladas, READMEs de repositórios e sites de documentação selecionados
...
Os resultados para desenvolvedores são retornados em seu próprio grupo `developer`, ao lado de `web`, cada um com `category: "developer"`. Para obter resultados ranqueados com as passagens correspondentes e usar os filtros de repositório e fonte de documentação, use o [endpoint de busca para desenvolvedores](/pt-BR/features/developer#developer-search-endpoint).
