...
<Info>
  Cada página rastreada consome 1 crédito (mais créditos adicionais para opções como modo JSON, enhanced proxy ou análise de PDF). O `limit` padrão é 10.000 páginas. Defina um `limit` menor para controlar o uso de créditos — por exemplo, `limit: 100` para limitar o rastreamento a 100 páginas.
</Info>
