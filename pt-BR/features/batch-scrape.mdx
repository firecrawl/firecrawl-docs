...
<BatchScrapeOutput />`- Chamar`startBatchScrape`/`start&#95;batch&#95;scrape`retorna
um ID de tarefa que você pode acompanhar via`getBatchScrapeStatus`/`get&#95;batch&#95;scrape&#95;status`, usando
o endpoint da API `/batch/scrape/{id}`, ou webhooks. Este endpoint é destinado a
verificações em andamento ou imediatamente após a conclusão, **pois tarefas em lote expiram após
24 horas**.
