...
* **Modelos disponíveis**: `spark-2` (padrão, mais barato e mais rápido), `spark-1-mini` (60% mais barato que o Pro) e `spark-1-pro` (maior precisão)
...
result = app.agent(
    ...
    schema=schema,
    model="spark-2"  # padrão; "spark-1-mini" ou "spark-1-pro" também disponíveis
)
```
