...
* **Modelos disponíveis**: `spark-1-pro` (padrão, maior precisão), `spark-1-mini` (60% mais barato que o Pro) e `spark-2` (mais barato e mais rápido)
...
result = app.agent(
    ...
    schema=schema,
    model="spark-2"  # "spark-1-pro" é o padrão; "spark-1-mini" também está disponível
)
```
