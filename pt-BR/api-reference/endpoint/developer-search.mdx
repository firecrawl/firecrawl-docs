...
Por isso, uma solicitação que envia um desses filtros sem especificar `sources` não retorna resultados `doc`. A resposta contém apenas evidências do GitHub: os tipos `issue`, `pull_request` e `readme`. O mapa `coverage` informa `doc` como `unavailable`, pois a parte da documentação do índice não foi consultada. Isso é intencional, não uma falha do índice.
