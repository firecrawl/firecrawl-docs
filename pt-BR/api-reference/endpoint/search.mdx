...
Inclua `scrapeOptions` com `formats: [{"type": "markdown"}]` para obter o conteúdo completo em markdown para cada resultado de pesquisa; caso contrário, você receberá por padrão apenas os resultados da search (url, title, description). Você também pode usar outros formatos, como `{"type": "summary"}` para conteúdo condensado.
