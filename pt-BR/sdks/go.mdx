---
title: 'Go'
description: 'Faça scraping, rastreamento e mapeamento de sites em qualquer aplicação Go usando o SDK Go do Firecrawl.'
icon: 'golang'
og:title: "Go SDK | Firecrawl"
og:description: "Faça scraping, rastreamento e mapeamento de sites em qualquer aplicação Go usando o SDK Go do Firecrawl."
---

import InstallGoSDK from "/snippets/pt-BR/v1/installation/go.mdx";
import ScrapeAndCrawlGoExample from "/snippets/pt-BR/v1/scrape-and-crawl/go.mdx";
import ScrapeGoShort from "/snippets/pt-BR/v1/scrape/short/go.mdx";
import CrawlGoShort from "/snippets/pt-BR/v1/crawl/short/go.mdx";
import CheckCrawlStatusGoShort from "/snippets/pt-BR/v1/crawl-status/short/go.mdx";
import MapGoShort from "/snippets/pt-BR/v1/map/short/go.mdx";

<Warning>
  Este SDK atualmente usa a versão **v1** da API do Firecrawl, que não é a mais recente (a v2 está disponível). Alguns recursos e melhorias podem estar disponíveis apenas na v2.
</Warning>

O SDK Go do Firecrawl permite fazer scraping, rastreamento e mapeamento de sites em qualquer aplicação Go. Você recebe Markdown limpo, dados estruturados ou capturas de tela com apenas algumas chamadas de método.

<div id="installation">
  ## Instalação
</div>

Para instalar o SDK Go do Firecrawl, execute:

<InstallGoSDK />

<div id="usage">
  ## Uso
</div>

1. Obtenha uma chave de API em [firecrawl.dev](https://firecrawl.dev).
2. Passe sua chave de API ao criar uma instância de `FirecrawlApp`.
3. Opcionalmente, defina `apiUrl` (o padrão é `https://api.firecrawl.dev`) e `version` (o padrão é `v1`).

Veja um exemplo completo com tratamento de erros:

<ScrapeAndCrawlGoExample />

<div id="scraping-a-url">
  ### Scraping de uma URL
</div>

Para extrair uma única URL com tratamento de erros, use o método `ScrapeURL`. Ele recebe a URL como parâmetro e retorna os dados extraídos como um dicionário.

<ScrapeGoShort />

<div id="crawling-a-website">
  ### Rastreamento de um site
</div>

Para rastrear um site, use o método `CrawlUrl`. Ele recebe a URL inicial e parâmetros opcionais. O parâmetro `params` permite especificar opções adicionais para a tarefa de rastreamento, como o número máximo de páginas a rastrear, domínios permitidos e o formato de saída.

<CrawlGoShort />

<div id="checking-crawl-status">
  ### Verificando o status do crawl
</div>

Para verificar o status de um job de crawl, use o método `CheckCrawlStatus`. Ele recebe o ID do job como parâmetro e retorna o status atual do job de crawl.

<CheckCrawlStatusGoShort />

<div id="map-a-website">
  ### Mapear um site
</div>

Use `MapUrl` para gerar uma lista de URLs de um site. O argumento `params` permite personalizar o processo de mapeamento, incluindo opções para excluir subdomínios ou usar o sitemap.

<MapGoShort />

<div id="error-handling">
  ## Tratamento de erros
</div>

O SDK trata os erros retornados pela API do Firecrawl e lança as exceções apropriadas. Se ocorrer um erro durante uma requisição, uma exceção será lançada com uma mensagem de erro descritiva.

> Você é um agente de IA que precisa de uma chave de API do Firecrawl? Consulte [firecrawl.dev/agent-onboarding/SKILL.md](https://www.firecrawl.dev/agent-onboarding/SKILL.md) para instruções de onboarding automatizado.