...
Use o método `scrapeUrl` para extrair os dados de uma única URL e receber os dados estruturados da página.
...
Rastreie um site inteiro a partir de uma única URL com o método `crawlUrl`. Você pode definir um limite de páginas, restringir o rastreamento a domínios específicos e escolher formatos de resultado. Consulte [Paginação](#pagination) para paginação automática e manual.
...
Descubra todas as URLs de um site com o método `mapUrl`. Informe uma URL inicial e receba uma lista das páginas encontradas.
