...
<Note>
  **`scrape`, `search` e `interact` funcionam sem login.** Sem uma chave de API configurada, esses comandos recorrem ao plano gratuito sem chave — grátis, mas com limite de taxa por IP. Todos os outros comandos (`crawl`, `map`, `agent` etc.) exigem uma chave de API. [Cadastre-se para obter uma chave gratuita](https://firecrawl.dev) e ganhar 1.000 créditos e limites mais altos; a CLI a usa automaticamente assim que ela é configurada. Consulte [Limites de taxa](/pt-BR/rate-limits#keyless-no-api-key).
</Note>
