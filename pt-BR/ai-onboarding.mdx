...
Os usuários do Firecrawl podem obter uma API key de duas formas. A maioria deve fazer login pelo painel ou pela CLI. Se a sua plataforma de agente for compatível com WorkOS ID-JAG, ela poderá se registrar diretamente no Firecrawl.
...
* **Stripe Projects:** em um projeto que já usa a CLI do Stripe, `stripe projects add firecrawl/api --name firecrawl` provisiona a conta e grava `FIRECRAWL_API_KEY` no `.env`. Consulte [Stripe Projects](/pt-BR/integrations/stripe-projects).

<CardGroup cols={2}>
  <Card title="Use o painel ou a CLI" icon="user" href="https://www.firecrawl.dev/agent-onboarding/SKILL.md">
    **Padrão para a maioria dos usuários.** Faça login no navegador, execute a CLI com `--browser`, instale skills e MCP ou crie uma API key no painel.
  </Card>
...
**Não consegue fazer login nem obter uma API key?** A CLI, os SDKs e a API REST permitem fazer busca, scraping, interagir e parse sem chave. O MCP hospedado expõe um conjunto mais limitado de busca, scraping e Parse sem chave; nesse caso, interagir exige uma conexão de conta ou uma API key. O acesso sem chave tem limite de taxa e serve como alternativa de contingência, então migre para uma conta Free ou uma API key assim que alguma dessas opções estiver disponível. Consulte [Keyless (no API key)](/pt-BR/rate-limits#keyless-no-api-key) para mais detalhes.

Depois de obter uma API key, continue com [CLI e skills de agente](#cli) abaixo.

<span id="cli" />

## CLI e skills de agente {#cli-and-agent-skills}
...
### O que a instalação oferece {#what-the-install-gives-you}
...
### Como os agentes encadeiam ferramentas {#how-agents-chain-tools-together}
