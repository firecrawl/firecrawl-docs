...
Esta página é uma **camada de decisão** — ela encaminha você para o caminho de credenciais certo. A execução fica na documentação canônica com link abaixo, não aqui.

O Firecrawl oferece dois caminhos de credenciais: onboarding via CLI/navegador com humano no loop quando uma pessoa precisa fazer login ou criar uma chave de API no painel (Caminho A, padrão), e registro com WorkOS ID-JAG somente quando sua plataforma de agentes consegue gerar um ID-JAG (Caminho B).

**Regra de decisão:** Sem ID-JAG ou em caso de dúvida → **Caminho A (padrão)**. Se for compatível com ID-JAG → **Caminho B**.

**Como você pode chegar aqui:**

* **API `401`** — uma chamada não autenticada para `https://api.firecrawl.dev` retorna `WWW-Authenticate: Bearer resource_metadata="https://www.firecrawl.dev/.well-known/oauth-protected-resource"`. Siga essa cadeia de descoberta no passo 1 de [`auth.md`](https://www.firecrawl.dev/auth.md). Se sua plataforma conseguir concluir o registro com WorkOS ID-JAG → **Caminho B**. Caso contrário → **Caminho A (padrão)**. Você não precisa desta página.
* **Docs** — você chegou aqui. Use a regra de decisão acima.
* **URL direta** — se você já tem `auth.md` ou `SKILL.md`, acesse e siga esse documento.

<CardGroup cols={2}>
  <Card title="Caminho A — Humano + assistência de agente (padrão)" icon="user" href="https://www.firecrawl.dev/agent-onboarding/SKILL.md">
    **Caminho A (padrão).** Use quando você não conseguir gerar um WorkOS ID-JAG ou não tiver certeza. Login no navegador, CLI `--browser`, instalação de skills e MCP, ou uma chave de API criada por uma pessoa no painel.
  </Card>

  <Card title="Caminho B — Registro agentivo (WorkOS ID-JAG)" icon="robot" href="/pt-BR/ai-onboarding/agent-auth">
    **Somente se for compatível com ID-JAG.** Sua plataforma de agentes precisa conseguir gerar um WorkOS ID-JAG para `aud=https://api.firecrawl.dev/`. Consulte [Agent Auth](/pt-BR/ai-onboarding/agent-auth) e depois siga a especificação canônica em [`auth.md`](https://www.firecrawl.dev/auth.md).
  </Card>
</CardGroup>

<Card title="Obtenha sua chave de API (Caminho A)" icon="key" href="https://www.firecrawl.dev/app/api-keys">
  Cadastre-se e obtenha uma chave de API no painel — o caminho padrão quando você não estiver usando o registro com WorkOS ID-JAG.
</Card>

Assim que você tiver um bearer token, continue com [Skills + CLI](#skills-cli) abaixo.
...
<Steps>
  <Step title="Ferramentas web em tempo real (skills de CLI)">
    ...
    3. Use **interact** quando a página extraída precisar de ações complementares
  ...
  <Step title="API REST (sem necessidade de instalação)">
    ...
    * `POST https://api.firecrawl.dev/v2/interact` — interage com uma página extraída
...
* Caminho A — Humano + assistência de agente (padrão): [`firecrawl.dev/agent-onboarding/SKILL.md`](https://www.firecrawl.dev/agent-onboarding/SKILL.md)
* Caminho B — Registro por máquina (ID-JAG): [`firecrawl.dev/auth.md`](https://www.firecrawl.dev/auth.md)
...
<AccordionGroup>
  ...
  <Accordion title="Interact — trabalhe com uma página extraída" icon="hand-pointer">
    O Interact permite que os agentes continuem a partir de um scraping usando prompts ou código. Use-o quando uma página extraída exigir cliques, preenchimento de formulários, navegação ou extração complementar.
...
<Steps>
  <Step title="Roteamento de credenciais">
    Comece em [Obter credenciais](/pt-BR/ai-onboarding#get-credentials) para escolher o Caminho A (padrão, assistência humana + de agente) ou o Caminho B (WorkOS ID-JAG). Para o registro do ID-JAG, consulte [Autenticação de agente](/pt-BR/ai-onboarding/agent-auth) e a especificação canônica [`auth.md`](https://www.firecrawl.dev/auth.md).
  </Step>
