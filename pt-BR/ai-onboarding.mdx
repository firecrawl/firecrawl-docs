...
* [servidor Firecrawl MCP](#firecrawl-mcp-server)
...
Depois de obter uma chave de API, continue com [CLI](#cli) abaixo.
...
MCP é um protocolo aberto que padroniza como os aplicativos fornecem contexto aos LLMs. Entre outros benefícios, ele oferece aos LLMs ferramentas para agir em seu nome. Nosso [servidor MCP](https://github.com/firecrawl/firecrawl-mcp-server) é de código aberto e abrange toda a nossa API — busca, scraping, interagir, rastreamento, mapeamento, extração e agente.

Use a URL hospedada remotamente (funciona sem uma chave de API no tier Free sem chave):

```
https://mcp.firecrawl.dev/v2/mcp
```

Com uma chave de API, envie-a no cabeçalho `Authorization: Bearer fc-YOUR-API-KEY` para desbloquear todas as ferramentas e limites mais altos.
...
<Card title="servidor MCP" icon="plug" href="/pt-BR/mcp-server/human-mcp">
  Veja a configuração hospedada para Codex, Claude Desktop, Claude Code, Cursor e outros
  clientes compatíveis. Para o Windsurf, use o
  [guia de início rápido do Windsurf](/pt-BR/quickstarts/windsurf). Para uma configuração de
  agente não assistido ou CI, consulte o [MCP de Agente](/pt-BR/mcp-server/agent-mcp).
</Card>
...
<Steps>
  ...
  <Step title="Servidor de docs MCP">
    Para uma abordagem estruturada com ferramentas MCP, conecte o servidor Firecrawl MCP no Codex, Cursor, Claude Code, Claude Desktop ou outro cliente compatível. Consulte [MCP para humanos](/pt-BR/mcp-server/human-mcp) para a configuração hospedada interativa ou [MCP para agentes](/pt-BR/mcp-server/agent-mcp) para um agente não assistido. Usuários do Windsurf devem seguir o [guia de início rápido do Windsurf](/pt-BR/quickstarts/windsurf).
  </Step>
