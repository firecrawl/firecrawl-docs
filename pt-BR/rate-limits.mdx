...
Os endpoints de `/browser` têm seu próprio limite de sessões. Cada equipe pode ter até 20 sessões de navegador ativas ao mesmo tempo. Se você exceder esse limite, novas solicitações de sessão retornarão um código de status `429` até que as sessões existentes sejam encerradas.

<Note>
  Apesar do nome, o limite de “navegadores concorrentes” não se aplica ao próprio endpoint `/browser`, mas se aplica às raspagens internas realizadas como parte da execução da sessão do navegador. Veja [Browser](/pt-BR/features/browser#rate-limits) para mais detalhes.
</Note>
