...
O número de sessões de navegador simultâneas permitidas é determinado pelo limite de concorrência do plano da sua equipe. Se você exceder esse limite, novas requisições de sessão retornarão um código de status `429` até que as sessões existentes sejam encerradas.
