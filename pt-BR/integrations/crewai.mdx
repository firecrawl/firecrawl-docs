...
- `url`: A URL base de onde começar o rastreamento.
...
- `crawler_options`: Opcional. Opções para controlar o comportamento do rastreamento.
  ...
  - `maxDepth`: Opcional. Profundidade máxima do rastreamento. Profundidade 1 é a URL base, profundidade 2 inclui a URL base e suas páginas filhas diretas, e assim por diante.
...
- `extractor_options`: Opcional. Opções para extração baseada em LLM de informações estruturadas a partir do conteúdo da página.
  - `mode`: O modo de extração a ser usado; atualmente oferece suporte a `llm-extraction`
...
- `api_key`: Opcional. Especifica a chave de API do Firecrawl. O padrão é a variável de ambiente `FIRECRAWL_API_KEY`.
- `query`: A string de busca a ser usada na pesquisa.
- `page_options`: Opcional. Opções para formatação do resultado.
  ...
  - `includeHtml`: Opcional. Inclui o conteúdo HTML bruto da página. Irá incluir uma chave `html` na resposta.
  - `fetchPageContent`: Opcional. Busca o conteúdo completo da página.
- `search_options`: Opcional. Opções para controlar o comportamento do rastreamento.
  - `limit`: Opcional. Número máximo de páginas a serem rastreadas.