...
Ao utilizar o endpoint `/scrape`, você pode especificar opções para **extrair informações estruturadas** do conteúdo da página usando o parâmetro `extractorOptions`. Aqui estão as opções disponíveis:
...
Para rastrear várias páginas, você pode usar o endpoint `/crawl`. Esse endpoint permite que você defina uma URL base que deseja rastrear, e todas as subpáginas acessíveis serão rastreadas.
...
<div id="check-crawl-job">
  ### Verificar tarefa de rastreamento
</div>

Usado para verificar o status de uma tarefa de rastreamento e obter seu resultado.
...
Ao usar o endpoint `/crawl`, você pode personalizar o comportamento de rastreamento com o parâmetro `crawlerOptions`. Aqui estão as opções disponíveis:
...
<div id="page-options-crawler-options">
  ## Opções de Página + Opções de Crawler
</div>

Você pode combinar os parâmetros `pageOptions` e `crawlerOptions` para personalizar completamente o comportamento de crawling.
...
<div id="extractor-options-crawler-options">
  ## Opções do Extractor e do Crawler
</div>
