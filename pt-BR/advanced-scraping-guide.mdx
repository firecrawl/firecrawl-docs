...
- **Description**: Controla como o crawler utiliza o sitemap do site. `"include"` (padrão) usa o sitemap em conjunto com a descoberta de links. `"skip"` ignora completamente o sitemap. `"only"` restringe o rastreamento a URLs encontradas no sitemap (mais a URL inicial).
- **Default**: `"include"`
...
- **Type**: `boolean`
- **Description**: Quando `true`, o crawler trata variantes de URL que apontam para a mesma página como duplicadas — por exemplo, `www.` vs não-`www.`, `http` vs `https`, barras no final da URL e sufixos `index.html`/`index.php` são todos normalizados antes de verificar duplicatas. Isso não remove parâmetros de consulta (veja `ignoreQueryParameters` para isso).
- **Default**: `true`
...
- **Tipo**: `boolean`
- **Descrição**: Quando `true`, os parâmetros de consulta (query string) são removidos das URLs antes da deduplicação, então `/page?a=1` e `/page?a=2` são tratados como a mesma URL e são rastreados apenas uma vez.
- **Padrão**: `false`
