...
description: "Um guia para configurar uma interface de ingestão web usando o Firecrawl com React."
og:title: "Template de UI do Conector Web | Firecrawl"
og:description: "Aprenda a configurar uma interface de ingestão web usando o Firecrawl com React."
---
...
Este template simplifica a criação de uma interface de conector web para o Firecrawl usando React.
Ele inclui um componente pré-pronto que se integra à API do Firecrawl, permitindo configurar rapidamente uma interface de crawling e scraping para que os usuários extraiam dados para suas aplicações de IA.
![Firecrawl UI Template](/images/firecrawl-ui.png)

Este template permite que os usuários informem uma URL para crawling. Se subpáginas forem detectadas, ele as rastreará e exibirá uma lista de URLs descobertas. Os usuários podem então selecionar quais URLs serão raspadas, e o conteúdo raspado será ingerido na aplicação de IA e exibido na interface.

> **Consideração de segurança: Este exemplo expõe chaves da API do Firecrawl no código do lado do cliente. Para uso em produção, é fortemente recomendado mover as interações com a API para uma implementação no lado do servidor, a fim de proteger suas chaves.**
...
* npm
* Certifique-se de que o [shadcn](https://github.com/shadcn-ui/ui) está instalado
* Certifique-se de que o [tailwindcss](https://tailwindcss.com/docs/installation) está instalado
...
* Instale os [componentes shadcn](https://github.com/shadcn-ui/ui) de que você vai precisar:
...
2. Crie o arquivo `lib/utils.ts` e adicione o seguinte código:
...
3. Copie e cole nosso componente principal de ingestão. **Certifique-se de substituir `FIRECRAWL_API_KEY` pela sua chave.**

<Accordion title="Componente de Ingestão (ingestion.tsx)">
  ```jsx
  ...
  //! Valores fixos (não recomendado para produção)
  //! É altamente recomendado mover todas as chamadas à API do Firecrawl para o backend (ex.: rota de API do Next.js)
      ...
      try {
        ...
        if (formData.crawlSubPages) {
          ...
          if (jobId) {
            ...
            do {
              ...
              if (statusResponse.ok) {
                ...
                if (statusData.status !== "completed") {
                  ...
                  console.log("Crawl concluído com status:", statusData.status);
                  console.log(statusData);
                }
              } else {
            ...
            console.error("Nenhum jobId recebido na solicitação de crawl");
          }
        } else {
        ...
        <Card>
          <CardHeader className="flex items-start justify-between mb-0 pb-4">
            <CardTitle className="flex items-center justify-between w-full space-x-2">
              <span className="text-base">Extrair conteúdo da web</span>
              <a
            ...
            <div className="text-sm text-gray-500 w-11/12 items-center">
              Use este componente para dar rapidamente aos seus usuários a capacidade de conectar
              seus apps de IA a dados da web com o Firecrawl. Saiba mais na{" "}
              <a
                href="https://docs.firecrawl.dev/"
                className="text-sm text-blue-500"
              >
                documentação do Firecrawl!
              </a>
            </div>
          </CardHeader>
                ...
                <CollapsibleContent className="space-y-4 mt-4 px-2">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <label htmlFor="extractMainContent" className="text-sm">
                      Extrair apenas o conteúdo principal (sem cabeçalhos, menus, rodapés etc.)
                    </label>
                  </div>
                </CollapsibleContent>
              </Collapsible>
            ...
            {showCrawlStatus && (
              <div className="flex items-center justify-between mb-2 space-x-2 bg-gray-100 p-2 rounded-md">
                ...
                <div className="text-sm text-gray-600">
                  {isScraping
                    ? `Páginas com scrape: ${crawlStatus.current} em ${elapsedTime}s`
                    : `Páginas rastreadas: ${crawlStatus.current} em ${elapsedTime}s`}
                </div>
              </div>
            )}
        ...
        {Object.keys(scrapeResults).length > 0 && (
          <div className="mt-4">
            ...
            <p className="text-sm text-gray-500">
              Você pode fazer o que quiser com os resultados do scrape. Aqui vai
              uma demonstração básica do markdown.
            </p>
...
Como isto não é uma biblioteca, você pode personalizar a interface como preferir. Basta editar o código, como no shadcn.

<div id="security-considerations">
  ## Considerações de segurança {#security-considerations}
</div>

Para uso em produção, considere as seguintes medidas de segurança:

* Mova as interações com a API para uma implementação no lado do servidor, a fim de proteger sua chave de API do Firecrawl.

<div id="learn-more">
  ## Saiba mais {#learn-more}
</div>

Para obter mais informações sobre o Firecrawl e sua API, visite a [documentação do Firecrawl](https://docs.firecrawl.dev/).
...
O modelo de UI de Ingestão do Firecrawl é licenciado sob a licença MIT.