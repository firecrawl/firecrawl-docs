...
Errors specific to [`/agent`](/features/agent) and its status, trace, snapshot, and cancel endpoints.
...
Terminal and `error.occurred` trace events carry a structured `error` object whose `code` is one of five values. Each also carries a `retryable` boolean — treat it the same way as the **Retryable** column above.
