...
Use the `lang` parameter to specify the language for search results using ISO 639-1 language codes. The `lang` parameter must be used together with the `country` parameter for correct results.

Examples: `"en"`, `"de"`, `"fr"`, `"ja"`, `"es"`, `"pt"`.
