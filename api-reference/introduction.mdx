...
408 | The request timed out (e.g., the page took too long to load, or a large PDF is still processing — retry the same URL when `details.state` is `processing_continues`).
...
--- | --- | ---
`SCRAPE_TIMEOUT` | 408 | The scrape exceeded the request `timeout`. For large PDFs this often means the document is still processing — retry the same URL after `Retry-After` when `details.state` is `processing_continues`. Otherwise increase `timeout` (up to 300000ms). See [Large PDF timeouts](/api-reference/errors#large-pdf-timeouts).
