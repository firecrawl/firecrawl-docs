...
If you are an agent without a key, follow the canonical [`auth.md`](https://www.firecrawl.dev/auth.md) spec for WorkOS ID-JAG registration. Humans can create a key in the [Firecrawl dashboard](https://www.firecrawl.dev/app/api-keys) or see [Get credentials](/ai-onboarding#get-credentials) on the Build with AI page.
