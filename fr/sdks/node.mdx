...
Récupérez les données structurées d’une page à partir d’une URL avec la méthode `scrapeUrl`.
...
Crawlez l’ensemble d’un site web à partir d’une seule URL avec la méthode `crawlUrl`. Vous pouvez définir une limite de pages, restreindre le crawl à des domaines spécifiques et choisir les formats de sortie. Consultez [Pagination](#pagination) pour la pagination automatique et manuelle.
...
Découvrez toutes les URL d’un site web avec la méthode `mapUrl`. Fournissez une URL de départ et obtenez en retour la liste des pages découvertes.
