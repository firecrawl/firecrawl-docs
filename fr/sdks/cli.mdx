...
<Note>
  **`scrape`, `search`, and `interact` fonctionnent sans vous connecter.** Si aucune clé API n’est configurée, ces commandes utilisent le niveau gratuit sans clé — gratuit, mais limité par IP. Toutes les autres commandes (crawl, cartographie, agent, etc.) nécessitent une clé API. [Inscrivez-vous pour obtenir une clé gratuite](https://firecrawl.dev) et bénéficier de 1 000 crédits ainsi que de limites plus élevées ; la CLI l’utilise automatiquement une fois configurée. Voir [Limites de débit](/fr/rate-limits#keyless-no-api-key).
</Note>
