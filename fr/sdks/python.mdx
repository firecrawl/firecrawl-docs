...
### Extraction d’une URL {#scraping-a-url}
...
### Analyse des fichiers envoyés {#parsing-uploaded-files}
...
### Exploration du sitemap uniquement {#sitemap-only-crawl}
...
### Exécuter un agent {#run-an-agent}
...
<div id="crawling-a-website-with-websockets">
  ### Exploration d’un site web avec WebSockets {#crawling-a-website-with-websockets}
</div>
...
#### Aides à la pagination manuelle {#manual-pagination-helpers}
...
<div id="simple-crawl-auto-pagination-default">
  ##### Crawl simple (pagination automatique, par défaut) {#simple-crawl-auto-pagination-default}
</div>

* Voir le flux par défaut dans [Explorer un site web](#crawl-a-website).

##### Crawl manuel avec contrôle de la pagination {#manual-crawl-with-pagination-control}
...
##### Exploration manuelle avec limites (pagination automatique + arrêt anticipé) {#manual-crawl-with-limits-auto-pagination-early-stop}
...
<div id="simple-batch-scrape-auto-pagination-default">
  ##### Extraction par lot simple (pagination automatique, par défaut) {#simple-batch-scrape-auto-pagination-default}
</div>

* Voir le parcours par défaut dans [Batch Scrape](/fr/features/batch-scrape).

##### Scraping par lot manuel avec contrôle de la pagination {#manual-batch-scrape-with-pagination-control}
...
##### Extraction par lots manuelle avec limites (pagination automatique + arrêt anticipé) {#manual-batch-scrape-with-limits-auto-pagination-early-stop}
...
### Connexion via le CDP {#connect-via-cdp}
...
### Session interactive liée au scrape {#scrape-bound-interactive-session}
