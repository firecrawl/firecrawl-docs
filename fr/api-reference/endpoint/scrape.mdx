...
- L’extraction JSON utilise désormais un format d’objet : `{ type: "json", prompt, schema }`
- Le format capture d’écran utilise désormais un format d’objet : `{ type: "screenshot", fullPage, quality, viewport }`
