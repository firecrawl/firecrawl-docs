...
Dans l’interface en ligne de commande (CLI) de Factory Droid, ajoutez Firecrawl avec la commande `/mcp add` :

```bash
/mcp add firecrawl "npx -y firecrawl-mcp" -e FIRECRAWL_API_KEY=votre-cle-api-ici
```

Remplacez `your-api-key-here` par votre clé API Firecrawl.
