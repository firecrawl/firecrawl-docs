...
Le MCP sans clé expose exactement recherche, scraping et Parse, avec des limites partagées. Utilisez [OAuth](/fr/mcp-server/oauth) ou une [clé API](/fr/mcp-server/keyless-api-key#add-an-api-key) pour accéder à un ensemble d’outils plus étendu. Conservez les clés API dans une variable d’environnement ou un magasin de secrets, jamais dans une URL.
