...
* **Modèles disponibles** : `spark-2` (par défaut, le moins cher et le plus rapide), `spark-1-mini` (60 % moins cher que Pro) et `spark-1-pro` (précision plus élevée)
...
result = app.agent(
    ...
    schema=schema,
    model="spark-2"  # par défaut ; "spark-1-mini" ou "spark-1-pro" également disponibles
)
```
