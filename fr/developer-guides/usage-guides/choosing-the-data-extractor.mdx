...
* **Modèles disponibles** : `spark-1-pro` (par défaut, précision plus élevée), `spark-1-mini` (60 % moins cher que Pro) et `spark-2` (le moins cher et le plus rapide)
...
result = app.agent(
    ...
    schema=schema,
    model="spark-2"  # "spark-1-pro" est la valeur par défaut ; "spark-1-mini" est également disponible
)
```
