---
title: "Recherche web et scraping MCP dans Windsurf"
description: "Ajoutez le scraping et la recherche web à Windsurf en 2 minutes"
---

Ajoutez des fonctionnalités de scraping et de recherche web à Windsurf avec Firecrawl MCP.

<div id="quick-setup">
  ## Configuration rapide
</div>

<div id="1-get-your-api-key">
  ### 1. Obtenez votre clé API
</div>

Inscrivez-vous sur [firecrawl.dev/app](https://firecrawl.dev/app) et copiez votre clé API.

<div id="2-add-to-windsurf">
  ### 2. Ajouter à Windsurf
</div>

Ajoutez ceci à votre `~/.codeium/mcp_config.json` (ou utilisez **Settings → Tools → Windsurf Settings → Ajouter un serveur** / **Afficher la configuration brute**) :

```json
{
  "mcpServers": {
    "firecrawl": {
      "command": "npx",
      "args": ["-y", "firecrawl-mcp"],
      "env": {
        "FIRECRAWL_API_KEY": "VOTRE_CLÉ_API"
      }
    }
  }
}
```

Remplacez `VOTRE_CLÉ_API` par votre clé API Firecrawl.

<div id="3-restart-windsurf">
  ### 3. Redémarrez Windsurf
</div>

C’est fait ! Windsurf peut désormais effectuer des recherches et scraper le web.

<div id="quick-demo">
  ## Démonstration rapide
</div>

Essayez ceci dans Windsurf :

**Recherche :**

```
Rechercher les dernières fonctionnalités de Tailwind CSS
```

**Extraction :**

```
Scraper firecrawl.dev et expliquer ce que c'est
```

**Accéder à la documentation :**

```
Trouver et scraper la documentation d'authentification Supabase
```

Les agents IA de Windsurf utiliseront automatiquement les outils de Firecrawl.
