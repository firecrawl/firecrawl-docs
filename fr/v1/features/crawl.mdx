...
<div id='crawling'>## Exploration</div>
...
<Warning>
  Par défaut, l’exploration ignore les liens d’une page s’ils ne sont pas des
  descendants directs de l’URL fournie. Ainsi, website.com/other-parent/blog-1
  ne sera pas renvoyé si vous explorez website.com/blogs/. Si vous souhaitez
  inclure website.com/other-parent/blog-1, utilisez le paramètre
  `crawlEntireDomain`. Pour explorer des sous-domaines comme blog.website.com
  lors de l’exploration de website.com, utilisez le paramètre `allowSubdomains`.
</Warning>
...
<Note>
  Si vous utilisez le SDK, consultez la section Réponse du SDK
...
<Note>
  Ce point de terminaison ne fonctionne que pour les crawls en cours ou
  récemment terminés.
</Note>
...
<div id='response-handling'>#### Gestion des réponses</div>
...
<Info>
  Les paramètres skip et next ne sont pertinents que lorsque vous interrogez
  directement l’API. Si vous utilisez le SDK, nous nous en chargeons et
  renverrons tous les résultats en une seule fois.
</Info>
...
<div id='sdk-response'>### Réponse du SDK</div>
...
<div id='faster-crawling'>## Exploration plus rapide</div>
...
Pour une documentation complète sur les webhooks — types d’événements, structure des charges utiles et exemples d’implémentation — consultez la [documentation des webhooks](/fr/webhooks/overview).
...
<Note>
  Pour une configuration détaillée des webhooks, des bonnes pratiques de
  sécurité et des solutions de dépannage, consultez la [documentation
