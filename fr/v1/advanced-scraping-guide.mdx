...
Lorsque vous utilisez l’endpoint `/scrape`, vous pouvez spécifier des options pour **extraire des informations structurées** à partir du contenu d’une page en utilisant le paramètre `extract`. Voici les options disponibles :
...
Si le contenu dépasse 10&nbsp;Mo ou si la tâche de crawl est toujours en cours d'exécution, la réponse inclura un paramètre `next`. Ce paramètre est l’URL de la page suivante de résultats. Vous pouvez utiliser ce paramètre pour récupérer la page suivante de résultats.
...
<div id="mapping-website-links-with-map">
  ## Cartographier les liens d’un site web avec `/map`
</div>

L’endpoint `/map` est particulièrement efficace pour identifier les URL contextuellement liées à un site donné. Cette fonctionnalité est essentielle pour comprendre l’environnement de liens contextuels d’un site, ce qui peut grandement aider à l’analyse stratégique et à la planification de la navigation.
