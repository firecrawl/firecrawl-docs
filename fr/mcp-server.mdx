...
<Note>
  **Pas de clé API ? Le scrape, la recherche et l’interaction fonctionnent quand même.** Connectez-vous à l’URL distante sans clé `https://mcp.firecrawl.dev/v2/mcp` (ou exécutez le serveur en local sans `FIRECRAWL_API_KEY`), et les outils `firecrawl_scrape`, `firecrawl_search` et `firecrawl_interact` basculent sur l’offre gratuite sans clé — gratuite, mais limitée par IP en termes de débit, et indisponible pour les autres outils (crawl, extract, map, etc.). Définissez `FIRECRAWL_API_KEY` pour activer tous les outils ainsi que des limites plus élevées. Consultez [Limite de débit](/fr/rate-limits#keyless-no-api-key).
</Note>
...
Ou connectez-vous sans clé API pour démarrer — scrape, search et interact fonctionnent avec l’offre Free sans clé (avec limitation de débit par IP) :
