...
<Note>
  **Pas de clé API ? Le scrape, la recherche, l’interaction et l’analyse fonctionnent sur l’URL distante sans clé.** Connectez-vous à `https://mcp.firecrawl.dev/v2/mcp` pour utiliser l’offre gratuite sans clé — gratuite, mais limitée par IP en termes de débit, et indisponible pour les autres outils (crawl, extract, map, etc.). Définissez `FIRECRAWL_API_KEY` pour activer tous les outils ainsi que des limites plus élevées. Consultez [Limite de débit](/fr/rate-limits#keyless-no-api-key).
</Note>
...
Ou connectez-vous sans clé API pour démarrer — scrape, search, interact et parse fonctionnent avec l’offre Free sans clé à distance (avec limitation de débit par IP) :
...
Exécute du code dans une session de navigateur active. Prend en charge les commandes agent-browser (bash), Python ou JavaScript.
