...
Utilisez l’URL suivante en local : http://localhost:3000/v2/mcp, ou la version hébergée https://mcp.firecrawl.dev/v2/mcp
...
* Le mode sans clé hébergé n’expose que les outils pris en charge en mode sans clé et est soumis à une limitation de débit par IP.
* La lecture de fichiers locaux n’est disponible que lorsque vous exécutez le serveur MCP localement.
* Les Webhooks et les chemins de fichiers locaux doivent être configurés à partir d’un serveur MCP local ou auto-hébergé lorsque l’agent doit accéder à des ressources locales.

Pour une surface MCP fixe en lecture seule avec des outils de recherche et de recherche approfondie, consultez le [point de terminaison MCP Search Only](/fr/mcp-server/search-only).
