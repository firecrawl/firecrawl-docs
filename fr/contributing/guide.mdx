...
Commencez par installer les dépendances en utilisant pnpm.

```bash
# cd apps/api # pour vous assurer que vous êtes dans le bon dossier
pnpm install # assurez-vous d'avoir pnpm version 9+ !
```
...
Vous devrez ouvrir trois terminaux.
...
Placez-vous dans le répertoire apps/api/ puis exécutez :

```bash
pnpm start
# si vous utilisez la [fonctionnalité llm-extract](https://github.com/firecrawl/firecrawl/pull/586/), vous devez également exporter OPENAI_API_KEY=sk-______
```

Cela va lancer les workers chargés de traiter les tâches de crawl.
...
Très bien, envoyons maintenant notre première requête.
...
2. Copiez le fichier `.env.example` dans `.env` dans le répertoire `/apps/api/` et configurez-le selon vos besoins
...
Cela va lancer automatiquement Redis, le serveur d&#39;API et les workers avec la configuration appropriée.
...
La meilleure façon de procéder est d’exécuter le test avec `npm run test:snips`.