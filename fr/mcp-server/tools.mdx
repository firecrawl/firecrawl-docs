...
Commencez par la [configuration MCP](/fr/mcp-server) pour choisir un accès sans clé, OAuth ou par clé API. Certains outils facultatifs peuvent être désactivés par l’environnement ou la politique de l’équipe.
...
| Extraire des données structurées d’une URL connue                  | `firecrawl_scrape` avec le format JSON               | Vous savez quelle page contient les données et souhaitez une sortie conforme à un prompt ou à un schéma.                                                    |
| Trouver et extraire des données lorsque les sources sont inconnues | `firecrawl_agent` and `firecrawl_agent_status`       | La tâche nécessite d’identifier une ou plusieurs sources avant de produire des résultats structurés.                                                        |
...
* **Un outil est manquant :** vérifiez le mode de connexion dans [la configuration MCP](/fr/mcp-server), reconnectez-vous ou redémarrez le client, puis vérifiez si la politique de l’équipe désactive les outils facultatifs.
