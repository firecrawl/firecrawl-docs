...
| Local avec l’API cloud Firecrawl           | Outils basés sur l’API ; l’analyse directe de fichiers locaux n’est pas disponible |
| Local avec une API Firecrawl auto-hébergée | Outils pris en charge par les services activés dans ce déploiement                 |

Commencez par la [configuration de Firecrawl MCP](/fr/mcp-server) pour choisir un accès OAuth, par clé API ou sans clé. Certains outils facultatifs peuvent être désactivés par l’environnement ou la politique de l’équipe.
...
| Analyser un fichier                       | `firecrawl_parse`                                    | Vous avez besoin du contenu d’un fichier PDF, d’un document, d’une feuille de calcul ou d’un fichier HTML.                                                  |
...
| Extraire des données structurées          | `firecrawl_extract`                                  | Vous avez des URL et souhaitez obtenir des données correspondant à un prompt ou à un schéma JSON.                                                           |
...
<AccordionGroup>
  ...
  <Accordion title="Vérifier un crawl en cours">
    `firecrawl_crawl` et `firecrawl_agent` renvoient des identifiants de tâche. Utilisez `firecrawl_check_crawl_status` ou `firecrawl_agent_status` jusqu’à ce que la tâche soit terminée ou échoue.
  </Accordion>
...
* **Un outil est manquant :** vérifiez le [mode de connexion](/fr/mcp-server#setup-firecrawl-mcp-server), reconnectez-vous ou redémarrez le client, puis vérifiez si la politique de l’équipe désactive les outils facultatifs.
