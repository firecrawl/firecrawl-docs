...
Le nombre de sessions de navigateur simultanées autorisées est déterminé par la limite de concurrence de votre plan d'équipe. Si vous dépassez cette limite, les nouvelles requêtes de session renverront un code d'état `429` jusqu'à ce que les sessions existantes soient détruites.
