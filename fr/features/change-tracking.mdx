...
if 'diff' in result.change_tracking:
    print(result.change_tracking.diff.text)  # Texte de diff au style Git
...
Le mode `json` fournit une comparaison structurée de champs spécifiques extraits du contenu. C’est utile pour suivre les modifications sur des points de données précis plutôt que sur l’ensemble du contenu.
...
if result.change_tracking.change_status == 'changed':
    print(f"Le prix est passé de {result.change_tracking.json.price.previous} à {result.change_tracking.json.price.current}")
```
