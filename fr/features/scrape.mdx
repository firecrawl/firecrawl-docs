...
import ScrapeQueryPython from "/snippets/fr/v2/scrape/query/python.mdx";
import ScrapeQueryNode from "/snippets/fr/v2/scrape/query/js.mdx";
import ScrapeQueryCURL from "/snippets/fr/v2/scrape/query/curl.mdx";
...
Le format `query` vous permet de poser une question en langage naturel sur une page. Firecrawl scrape la page et exécute un LLM sur le contenu, puis renvoie la réponse dans le champ `answer` de la réponse. Utilisez-le pour un Q&amp;R ponctuel sur une URL sans avoir à écrire de schéma JSON.

<Info>
  Le format `query` coûte 5 crédits par page (1 de base + 4 supplémentaires pour l’appel au LLM).
</Info>

Options dans l’objet format :

* `prompt` (obligatoire) : la question à laquelle répondre. Maximum : 10 000 caractères.
* `mode` (facultatif) : `"freeform"` répond à la question à partir du contenu de la page. `"directQuote"` renvoie le texte source pertinent de la page. La valeur par défaut est `"freeform"`.

Vous pouvez combiner `query` avec d’autres formats — par exemple, demander `markdown` et `query` ensemble pour obtenir à la fois le contenu de la page et la réponse en un seul appel.
...
Le format `query` est également disponible dans `/search` via `scrapeOptions`, ce qui exécute le même Q&amp;R sur chaque résultat de recherche.
