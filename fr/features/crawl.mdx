---
title: 'Exploration'
description: 'Explorer récursivement un site web et extraire le contenu de chaque page'
og:title: 'Exploration | Firecrawl'
og:description: 'Explorer récursivement un site web et extraire le contenu de chaque page'
icon: 'spider'
---
...
Toutes les options de l’[endpoint Scrape](/fr/api-reference/endpoint/scrape) sont disponibles dans crawl via `scrapeOptions` (JS) / `scrape_options` (Python). Elles s’appliquent à chaque page que le crawler extrait, y compris les formats, le proxy, la mise en cache, les actions, la localisation et les tags.
...
| `maxDiscoveryDepth`     | `integer`  | (aucun)       | Profondeur maximale depuis l’URL racine selon l’ordre de découverte. Le site racine et les pages du sitemap ont une profondeur de découverte égale à 0. |
