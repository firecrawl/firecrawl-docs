...
Firecrawl Agent propose trois modèles. **Spark 1 Pro** est le modèle par défaut. **Spark 2** est moins cher et plus rapide que les deux modèles Spark 1, avec une précision comparable — définissez `model` sur `spark-2` pour l&#39;utiliser.

| Modèle         | Coût                        | Précision        | Idéal pour                                               |
| -------------- | --------------------------- | ---------------- | -------------------------------------------------------- |
| `spark-2`      | Le plus faible              | Comparable à Pro | Faible coût et durée d&#39;exécution courte              |
| `spark-1-mini` | **60 % moins cher** que Pro | Standard         | Charges de travail Spark 1 sensibles aux coûts           |
| `spark-1-pro`  | Standard                    | Supérieure       | Recherches complexes, extractions critiques (par défaut) |
...
`spark-2` est notre tout dernier modèle d’agent. Il couvre l’ensemble des tâches qui nécessitaient auparavant de choisir entre Mini et Pro, sans compromis entre précision et coût. Définissez `model` sur `spark-2` pour l’utiliser.

**Utilisez Spark 2 si :**

* Vous souhaitez minimiser le coût par exécution
* La durée d’exécution est importante
* Vous auriez autrement privilégié la précision de Spark 1 Pro
...
`spark-1-pro` est notre modèle phare, conçu pour une précision maximale sur les tâches d&#39;extraction complexes. C&#39;est le modèle par défaut : une requête qui ne définit pas `model` utilise `spark-1-pro`.
...
`spark-1-pro` est le modèle par défaut, le paramètre `model` est donc facultatif. Utilisez-le pour choisir un autre modèle :
...
| `model`      | string | Non     | Modèle à utiliser : `spark-1-pro` (par défaut), `spark-1-mini` ou `spark-2`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
