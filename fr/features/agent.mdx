...
Firecrawl Agent propose trois modèles. **Spark 2** est le modèle par défaut : il est moins cher et plus rapide que les deux modèles Spark 1, avec une précision comparable. Les deux modèles Spark 1 restent disponibles et inchangés.
...
| `spark-1-pro`  | Standard                    | Supérieure       | Recherches complexes, extractions critiques    |

<Tip>
  **Spark 2 est le modèle par défaut** : vous pouvez donc omettre le paramètre `model` — il coûte moins cher et termine plus rapidement que les deux modèles Spark 1 dans la plupart des cas. Les modèles Spark 1 restent disponibles : parmi eux, commencez par Mini et passez à Pro uniquement pour les recherches complexes sur plusieurs domaines ou lorsque la précision est critique.
</Tip>
...
`spark-2` est notre tout dernier modèle d’agent et celui utilisé par défaut pour chaque requête. Il couvre l’ensemble des tâches qui nécessitaient auparavant de choisir entre Mini et Pro, sans compromis entre précision et coût.
...
`spark-2` est le modèle par défaut, le paramètre `model` est donc facultatif. Utilisez-le pour choisir un autre modèle :
...
| `model`      | string | Non     | Modèle à utiliser : `spark-2` (par défaut), `spark-1-mini` ou `spark-1-pro`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
