...
## Utilisation de `/agent` {#using-agent}
...
## Fournir des URL (facultatif) {#providing-urls-optional}
...
## Statut et fin de la tâche {#job-status-and-completion}
...
### États possibles {#possible-states}
...
#### Exemple en attente {#pending-example}

<AgentStatusPending />

#### Exemple complété {#completed-example}

<AgentStatusCompleted />

## Liste des exécutions d’agent {#listing-agent-runs}
...
## Suivre une exécution en cours {#following-a-run-in-progress}
...
## Traces d’exécution et instantanés {#execution-traces-and-snapshots}
...
## Récupérer les données sources de l&#39;agent {#getting-the-agents-source-data}
...
## Partager des exécutions d’agent {#share-agent-runs}
...
### Définir le modèle {#specifying-a-model}
...
## Agent vs Extract : ce qui a été amélioré {#agent-vs-extract-whats-improved}
...
## Téléversement de CSV dans l’Agent Playground {#csv-upload-in-agent-playground}
...
## Dépannage avec Ask {#troubleshooting-with-ask}
...
### Fonctionnement de la tarification de l’agent {#how-agent-pricing-works}
...
### Tarification des agents parallèles {#parallel-agents-pricing}
...
### Gestion des coûts {#managing-costs}
