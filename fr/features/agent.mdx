...
| `model` | string | Non | Modèle à utiliser : `spark-1-mini` (valeur par défaut) ou `spark-1-pro` |
...
| `maxCredits` | number | Non | Nombre maximal de crédits à dépenser pour cette tâche d’agent. La valeur par défaut est **2 500** si elle n’est pas définie. Si la limite est atteinte, la tâche échoue et **aucune donnée n’est renvoyée**, mais les crédits consommés pour le travail déjà effectué restent facturés. |
...
L’[Agent Playground](https://www.firecrawl.dev/app/agent) prend en charge le téléversement de fichiers CSV pour le traitement par lots. Téléversez un fichier CSV contenant vos données d’entrée (par exemple des noms d’entreprises, des URL ou toute autre entité), rédigez un prompt décrivant les données que vous souhaitez que l’agent trouve pour chaque ligne, définissez vos champs de sortie, puis lancez l’exécution — l’agent traite chaque ligne en parallèle et renseigne les résultats.
