...
Par défaut, chaque session de navigateur commence avec un état vierge. Avec `persistentSession`, vous pouvez enregistrer les cookies, le localStorage et d&#39;autres éléments d&#39;état du navigateur afin qu&#39;ils soient conservés pour les sessions futures. C&#39;est pratique pour rester connecté, conserver les préférences ou reprendre des flux de travail en plusieurs étapes.
...
| `writeMode` | `"readwrite"` | `"readwrite"` enregistre les modifications. `"readonly"` charge les données existantes sans écrire — utile lorsque vous avez besoin de plusieurs lecteurs en parallèle. |

<Note>
  Une seule session peut écrire dans un profil à la fois. Si une autre session est déjà en écriture, vous recevrez une erreur `409`. Vous pouvez tout de même ouvrir le même profil en mode `"readonly"`, ou réessayer plus tard.
</Note>
