...
La qualité d&#39;un monitor à l&#39;échelle du web repose sur deux leviers qui ont des rôles différents :

* **`queries` contrôlent le rappel** — ce que chaque recherche fait remonter. Trop restrictives, et les événements réels ne remontent jamais ; trop larges, et le juge dépense des crédits à filtrer le bruit.
* **`goal` contrôle la précision** — quels résultats récupérés déclenchent réellement une alerte. Le juge évalue chaque nouveau résultat par rapport à l&#39;objectif : c&#39;est donc l&#39;objectif qui fait la différence entre une vraie correspondance et un résultat lié au sujet mais non pertinent.

Réglez les deux. Un objectif parfait ne peut pas déclencher d&#39;alerte sur un résultat que les requêtes n&#39;ont jamais récupéré, et des requêtes larges associées à un objectif vague produisent en permanence des alertes de faible valeur.

**Les bonnes requêtes ressemblent à des termes de recherche, pas à des phrases :**
...
* Gardez chaque requête concise (environ 2 à 6 termes). Une requête large vaut généralement mieux que plusieurs requêtes étroites — des requêtes supplémentaires se partagent le budget `maxResults` sans améliorer la couverture.
* Utilisez une requête par sujet *distinct*. Un même sujet avec plusieurs facettes (&quot;launches, benchmarks, docs&quot;) reste une seule requête ; ne scindez que lorsque l&#39;objectif cite de vraies entités distinctes (par exemple, &quot;OpenAI, Anthropic, and Google&quot;).
* N&#39;utilisez pas les opérateurs `site:` / `-site:` dans les requêtes — utilisez plutôt `includeDomains` / `excludeDomains`.

**Les bons objectifs expriment clairement ce qui constitue une correspondance et n&#39;ajoutent des exclusions que lorsqu&#39;elles font partie de l&#39;intention :**

* Nommez le sujet et ce qui compte comme une correspondance : *&quot;Alertez-moi quand OpenAI annonce un tout nouveau modèle.&quot;*
* Désambiguïsez les termes ambigus : *&quot;Firecrawl (l&#39;API de web scraping)&quot;* évite au juge de partir sur du matériel de lutte contre l&#39;incendie.
* Ajoutez `Ignore ...` uniquement pour les non-correspondances propres à l&#39;intention : *&quot;Ignorez les tribunes d&#39;opinion, les tutoriels et les actualités IA sans rapport.&quot;* Ne reformulez pas le bruit générique — le juge gère déjà la mise en forme, les paramètres de suivi et le bruit lié aux réindexations.
* Si l&#39;intention est large, gardez-la large. Des objectifs trop restrictifs risquent d&#39;écarter de vraies correspondances.

**À quoi ressemble un monitor sain.** Un monitor à l&#39;échelle du web bien réglé ne signale généralement rien — la plupart des vérifications renvoient `new: 0` — et n&#39;alerte que lorsqu&#39;un élément réellement nouveau et conforme à l&#39;objectif apparaît. Lisez le résumé de la vérification et le `searchStatus` de chaque résultat (voir [Statuts et déduplication](#statuses-and-dedup)) pour savoir s&#39;il est bien réglé :

* Un flux régulier de résultats `ignored` signifie que les requêtes font remonter du bruit que l&#39;objectif rejette ensuite — resserrez les requêtes (ou réduisez `searchWindow`) pour arrêter de payer l&#39;évaluation de résultats qui ne déclencheront jamais d&#39;alerte.
* Des `watching` fréquents signifient que l&#39;objectif est ambigu — rendez les critères de correspondance plus précis pour que le juge puisse trancher.
* De longues périodes sans résultat sur un sujet actif signifient que les requêtes sont trop restrictives ou que `searchWindow` est trop étroit — élargissez les termes ou la fenêtre.
* Les alertes que l&#39;utilisateur écarte signifient que l&#39;objectif est trop large — ajoutez un `Ignore ...` propre à l&#39;intention.
